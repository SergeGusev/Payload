...
public static class AppOptionsValidator
{
    ...
    {
        if (!Enum.IsDefined(typeof(BotMode), options.Mode))
        {
    ...
    {
        if (!Enum.IsDefined(typeof(MarketDataWebSocketSubscriptionScope), options.SubscriptionScope))
        {
    ...
    private static void ValidateCryptoAssetSymbols(IReadOnlyCollection<string> assetSymbols, string optionName, List<string> errors)
    {
        ...
        {
            if (string.IsNullOrWhiteSpace(symbol) || symbol.Length > 16 || !symbol.All(IsAsciiLetterOrDigit))
            {
    ...
    private static bool IsValidCertificatePin(string value)
    {
        ...
        try
        {
            var bytes = Convert.FromBase64String(value.Substring(Sha256SubjectPublicKeyInfoPinPrefix.Length));
