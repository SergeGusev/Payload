...
public sealed class ClobV2OrderSigner
{
    ...
    public string Sign(ClobV2Order order, string privateKey, int chainId = ClobV2ExchangeContracts.PolygonChainId)
    {
        ...
        Guard.NotNullOrWhiteSpace(privateKey, nameof(privateKey));
        ...
        int chainId = ClobV2ExchangeContracts.PolygonChainId)
    {
        ...
        Guard.NotNullOrWhiteSpace(expectedSignerAddress, nameof(expectedSignerAddress));
    ...
    private static string ExtractPoly1271InnerSignature(string signature)
    {
        ...
        return "0x" + SubArray(bytes, 0, 65).ToHex();
    }
    ...
    private static string RecoverHashSigner(byte[] hash, string signature)
    {
        ...
        var r = new BouncyBigInteger(1, SubArray(bytes, 0, 32));
        var s = new BouncyBigInteger(1, SubArray(bytes, 32, 32));
        var recovered = EthECKey.RecoverFromSignature(new EthECDSASignature(r, s, new[] { bytes[64] }), hash);
    ...
    private static byte[] OneByteV(byte[] value)
    {
        ...
        return new[] { value[value.Length - 1] };
    }
    ...
    {
        var hex = value.StartsWith("0x", StringComparison.OrdinalIgnoreCase) ? value.Substring(2) : value;
    ...
    private static byte[] ToUnsignedBigEndian(BigInteger value)
    {
        ...
        var lastNonZero = bytes.Length - 1;
