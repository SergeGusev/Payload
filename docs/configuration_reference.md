...
- `CopiedTraderPerformanceReconciliationWalletBatchSize`: separately reserved maximum low-priority reconciliation wallets recomputed in the same transaction; default `5`, allowed range `1` to `250`. Unused high-priority and reconciliation capacity does not spill into the other class, keeping the configured database-load and freshness contract deterministic; the default hard limit is therefore `30` wallets per cycle.
