...
- `BTC/ETH/SOL Up or Down 5m N Child` and the Child ROI modes: plain Child and Child ROI retain `N=1..24`. BTC/SOL Child Progress retain `N=1..24`; ETH Child Progress retains `N=7,12,15,16,17,18,20,22,23`. Child Progress ROI retains BTC `N=1..24`, ETH `N=1,2,4,6,10,20`, and SOL `N=1,2,3,7,8,9,10,11,12,15,16,17,18,20,22,24`. After entry work and database writes, each enabled Child row selects an enabled, unpaused same-asset non-Child, non-Futures parent. Plain Child excludes parent strategies whose names contain `Progress`; Child Progress includes them. PnL modes rank positive settled Paper PnL. ROI modes require at least `10` settled runs, at least `$60` settled stake, and positive PnL, then rank by sample-adjusted ROI `roi_pct * stake_usd / (stake_usd + 120)`. Active assignments store both the selected parent PnL and raw ROI, and copied Paper orders include both values in the raw decision JSON. Migration `20260713_remove_hopeless_progress_strategies` removes the exact 57 retired rows and operational history after an active-Live-order guard.
