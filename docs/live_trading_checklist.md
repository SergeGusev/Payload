...
- Dashboard `Paper Lost` / `Paper Cnt` and `Live Lost` / `Live Cnt` are stored separately per strategy. Only `Paper Lost` applies the loss-counter add-on, and only to Paper entries; `Live Cnt` is maintained from Live settlements when `Live Lost` is above `1`, but it does not increase live stake sizing yet.
- Paper/Live-shadow matching must keep asset, condition, outcome, order type, `postOnly=false`, and limit price within `0.000001`; Paper and Live requested sizes may differ because `Paper Lost` and `Live $` sizing are separate. Shape mismatch disables `LiveStakes` for that strategy and cancels correlated open live orders.
