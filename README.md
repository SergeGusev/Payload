...
- Implemented live trading is BUY-only, explicitly risk-capped, and disabled by default. The legacy Follow Leader live path remains tightly gated; the paper/live shadow test path is limited to explicitly allowed variants, currently `BTC Up or Down 5m Skip 1`, `BTC Up or Down 5m Middle 1 47 bps Instant`, `BTC Up or Down 5m Binance 10 bps`, `BTC Up or Down 5m Binance 17 bps Instant`, `BTC Up or Down 5m Binance 18 bps`, `BTC Up or Down 5m Binance 19 bps`, `BTC Up or Down 5m Binance 20 bps`, `BTC Up or Down 5m Binance 20 bps Instant`, `BTC Up or Down 5m Binance 21 bps`, `BTC Up or Down 5m Binance 22 bps`, `BTC Up or Down 5m Binance 23 bps`, `ETH Up or Down 5m Skip 7 bps Instant`, `SOL Up or Down 5m Binance 24 bps Instant`, and `SOL Up or Down 5m Skip 42 bps Instant`, and submits GTD limit BUY orders with `post_only=false`, a local market-relative cancel deadline, and a CLOB GTD expiration that includes the configured security buffer. Other BTC Middle Instant variants and other ETH/SOL Middle, Binance bps, and Skip variants are Paper-only and are not in the live allowlist.
...
Trading strategies are stored in PostgreSQL table `strategies`. Built-in rows include `Follow leader` (`follow_leader`, id `f0110a0d-1ead-4c00-8b01-000000000001`) and the experimental BTC 5-minute family: 9 `BTC Up or Down 5m Less {Secs}` rows, 9 `BTC Up or Down 5m More {Secs}` rows, 9 `BTC Up or Down 5m Less {Secs}` Gamma rows, and 9 `BTC Up or Down 5m More {Secs}` Gamma rows, with `{Secs}` from `30` through `270` in 30-second steps, plus the other BTC 5-minute comparison, fixed Up/Down, Maker, Maker 50, Middle bps/Instant, Skip bps, Binance, delayed, edge, ensemble, Markov, selector, and pre-open variants. It also includes 1214 ETH/SOL variants: 200 disabled-by-default Binance bps and Binance bps `Instant` rows, 804 Paper-only Middle, Middle `Instant`, Middle `Revert`, and Middle `Revert Instant` rows with `1..100 bps` thresholds, plus Skip `1..5`, Skip bps `1..50`, and Skip bps `1..50 Instant` rows for both `ETH Up or Down 5m` and `SOL Up or Down 5m`; ETH/SOL Skip Revert variants are not seeded. All ETH/SOL variants are Paper-only except the explicitly allowlisted `ETH Up or Down 5m Skip 7 bps Instant` and `SOL Up or Down 5m Binance 24 bps Instant`. It also includes 960 Paper-only fixed-direction pre-open variants across BTC `5m`, `15m`, `1h`, and `4h` markets. Paper, dry-run, and live order rows carry `strategy_id`, so strategy variants can run side by side and be compared by their order/fill outcomes. The dashboard `Strategies` tab aggregates one row per strategy from `strategies`, `paper_orders`, open positions, settlements, BTC strategy run lifecycle rows, and `live_orders`. Its default nested `All` tab shows selectable/copyable strategy names without internal codes, includes Paper open-position mark-to-market `MtM PnL` / `MtM ROI %` beside closed-only `Realized` and `Closed ROI %`, shows decision-health `Avg delay s` / `Max delay s` between planned `entry_due_at_utc` and actual `entered_at_utc`, and shows separate Live outcome metrics: Live orders, filled/open/settled counts, skipped/ignored counts with GTD-unfilled/cancel/reject breakdown, won/lost, settled cost basis, realized PnL, win/loss rate, average win/loss, profit factor, expectancy, ROI, and latest live order/settlement timestamps. The same `Strategies` tab also contains nested `24 hours`, `6 hours`, and `1 hour` tabs with sortable Live state, recent orders, fills, expired/open orders, entered/skipped/settled runs, wins/losses, win rate, ROI, realized PnL, the matching recent Live settled/skipped/ignored-breakdown/won/lost/realized/ROI block, filled cost, average fill price, entry-delay health metrics, top skip reason, and last activity timestamps. Each nested tab supports category filtering plus `Only positive`, `Enabled only`, `Live only`, `Big ROI`, and `Big settles` filters; `Live only` keeps rows with the Live flag enabled even when `Auto Live Pause` is currently suppressing effective live entries, `Big ROI` keeps rows with ROI greater than `10` (`Closed ROI` in `All`, recent `ROI` in period tabs), and `Big settles` keeps rows whose settled count is greater than `100` (`Settled` positions in `All`, recent `Settles` runs in period tabs). The `All` tab lets the `Enabled`, `Live`, and manual `Paused` flags be changed at runtime, shows read-only automatic `Auto Live Pause` state, and exposes editable per-strategy `Paper $`, `Live $`, `Paper Lost`, `Paper Cnt`, `Live Lost`, `Live Cnt`, and live-only `Live bal` values. It also shows closed-outcome quality metrics (`Avg win`, `Avg loss`, `Profit factor`, and `Expectancy`) beside `Win %` so high win-rate but negative-expectancy strategies are visible. The service refreshes enabled, manual pause, and automatic Live pause state from PostgreSQL through a short in-memory cache; disabled strategies stop creating new Follow leader signals or Up/Down entries without a restart, manually paused strategies stay enabled but skip new Paper and Live entries, and auto-Live-paused strategies continue Paper while suppressing new Live/Paper-shadow entries. Existing Paper positions can still be settled, and copied leader exits can still be tracked so disabling or pausing a strategy does not strand already-open accounting.

`Paper Lost` and `Live Lost` are per-strategy Dashboard fields stored in `strategies.paper_lost_coeff` and `strategies.live_lost_coeff`, both defaulting to `1.00`. `Paper Cnt` and `Live Cnt` are the persisted loss counters stored in `strategies.paper_lost_counter` and `strategies.live_lost_counter`, defaulting to `0`; both are visible and editable in the Dashboard. `Paper Lost` values above `1` enable the Paper loss-counter stake add-on: each Paper loss increments `Paper Cnt`, each Paper win decrements it down to zero, and the stake add-on uses `min(Paper Cnt, 5)` times the already computed Paper stake. `Live Cnt` is updated from matched Live settlements when `Live Lost` is above `1`, but `Live Lost` is not yet applied to live stake sizing.

Automatic strategy pausing is Live-only and opt-in per strategy through `LiveTrading:AutoLivePauseStrategies`; the default empty list disables automatic Live pause for every strategy. The service configuration currently has no strategy entries, so no strategy participates in automatic Live pause/resume until a code or id is explicitly added. At startup, the service clears stored `auto_live_paused=true` rows for strategies that are not in the current allowlist, so removed strategies are not stranded by old Auto Live Pause state. Entries may be strategy codes such as `follow_leader` or strategy ids. For allowlisted strategies, pause and resume use different evidence: after a Live settlement, the service checks that strategy's settled Live orders over the last 12 hours; if more than one Live bet exists and the 12-hour Live realized PnL is negative, it sets `strategies.auto_live_paused=true` indefinitely. Live settlements never clear this flag. The strategy continues Paper entries, and after each Paper settlement the service checks that strategy's settled Paper rows over the last 12 hours; when Paper realized PnL becomes positive, it clears `auto_live_paused` and Live entries resume if the manual `Live` flag is still enabled. Paper settlements never set this flag. A one-time schema data migration clears existing `auto_live_paused=true` rows when this default opt-in policy is deployed. The Dashboard `Paused` checkbox remains a manual full Paper+Live pause.

The `BTC Up or Down 5m` paper worker watches BTC 5-minute Gamma markets from `polymarket_gamma_markets`, stores one lifecycle row per enabled market/variant in `strategy_market_paper_runs`, then creates paper entries for each enabled variant's configured Paper stake multiplier when it becomes due. The Maker variants are the exception: `BTC Up or Down 5m Up Maker` and `BTC Up or Down 5m Down Maker` are Paper-only, baseline the selected outcome best ask after market start, keep an in-memory high-water best ask for that market/outcome, and evaluate entries only on 30-second slots (`30s` through `270s`, maximum 9 attempts per 5-minute market). On a slot they create a minimum-size post-only GTD BUY one tick below the current best ask only when that ask exceeds the previously fixed high-water value. `BTC Up or Down 5m Up Maker 50` and `BTC Up or Down 5m Down Maker 50` use the same high-water/slot rules, but place at fixed `0.50` and only when the selected outcome best ask is strictly above `0.50`. The high-water value is fixed at baseline and then updated only when a Maker paper order is actually created; between-slot book moves and no-order slots do not raise it. Flat or falling asks do not create orders, so after a Maker order at `0.55` and a fall to `0.52`, the next Maker order waits until a later 30-second slot where the ask crosses `0.55` and reaches a new high such as `0.56`; Maker orders can happen multiple times in one market and expire at `marketEndUtc`. They can run in `Bot:Mode=Live` when `PaperTrading:RunInLiveMode=true`, but still create only Paper orders, never submit Live/Paper-shadow orders, and are intentionally excluded from the opposite-outcome open-order guard. The same worker also watches enabled ETH/SOL 5-minute Gamma markets for crypto Middle, Binance bps, and non-Revert Skip variants, then settles their filled Paper rows through the same strategy lifecycle tables; live placement remains restricted to the explicit allowlist. With `BtcUpDown5mStrategy:PaperTakerPricingEnabled=true`, the standard non-`Gamma` variants use Gamma only for market/outcome/token mapping and settlement metadata. The worker prices both outcome assets from fresh CLOB/WebSocket order-book depth, falls back to REST CLOB `/book` when cached depth is missing or stale, computes executable ask-depth VWAP, and stores quote/order-book context in run diagnostics when a run is skipped. Middle, Binance, Skip, crypto Middle, crypto Binance bps, and crypto Skip bps `Instant` variants use immediate executable ask-depth pricing, but refuse any required BUY limit above `BtcUpDown5mStrategy:InstantOpeningLimitMaxPrice` (`0.65` by default) with skip reason `instant_price_above_max`. Before creating a new non-Maker BUY entry, the worker skips it with `opposite_outcome_open_order` if the same condition already has an open BUY order on a different outcome; same-outcome entries and SELL exits are not blocked. Paper open orders use GTD BUY sizing, synthetic strategy wallets, fill accounting, and settlement through the shared paper order pipeline; linked live orders are created only by the explicitly allowlisted Paper/Live-shadow path when all live gates pass.
...
Follow leader live orders remain BUY-only, GTD-only, post-only, and capped by `LiveTrading:MaxOrderNotionalUsd` plus live bankroll percentages; current Follow leader Paper pricing uses the leader's historical trade price, so the live preflight rejects these signals until a separate live execution policy is explicitly implemented. Standard BTC taker live stakes, if enabled later, submit BUY GTD limit orders with `postOnly=false` and a bounded expiration. The current Paper/Live-shadow opening-limit path for explicitly allowed variants (`BTC Up or Down 5m Skip 1`, `BTC Up or Down 5m Middle 1 47 bps Instant`, `BTC Up or Down 5m Binance 10 bps`, `BTC Up or Down 5m Binance 17 bps Instant`, `BTC Up or Down 5m Binance 18 bps`, `BTC Up or Down 5m Binance 19 bps`, `BTC Up or Down 5m Binance 20 bps`, `BTC Up or Down 5m Binance 20 bps Instant`, `BTC Up or Down 5m Binance 21 bps`, `BTC Up or Down 5m Binance 22 bps`, `BTC Up or Down 5m Binance 23 bps`, `ETH Up or Down 5m Skip 7 bps Instant`, `SOL Up or Down 5m Binance 24 bps Instant`, and `SOL Up or Down 5m Skip 42 bps Instant`) submits `GTD` BUY limits with `postOnly=false`; by default local cancellation is one minute before market end and the CLOB wire expiration includes `ClobGtdExpirationSecurityBufferSeconds` (`60`). Operational CLI toggles can enable exactly one strategy with `--set-live-stakes-only-code <code>` or exactly a set of strategies with `--set-live-stakes-only-codes code1,code2`; both forms disable Live for every other strategy. Before placement the service refetches the order book, checks clock drift and risk caps, blocks any BUY when the same condition already has an open Paper or Live BUY on a different outcome, enforces live bankroll/strategy-balance caps, signs the CLOB V2 payload locally, and sends `POST /order` with L2 headers. `LiveTrading:MaxOrderNotionalUsd` is a hard emergency ceiling rather than the normal stake-sizing control; intended per-strategy sizing is controlled through Dashboard `Live $` and `Live bal`. Live preflight applies `LiveTrading` market/total exposure caps to open Live orders only; Paper backlog stays governed by Paper controls and does not consume Live safety ceilings. Paper/Live-shadow Paper sizing can include the `Paper Lost` loss-counter add-on, while Live sizing is computed separately from `Live $` and is not increased by `Paper Lost`. Live orders and live events are stored in PostgreSQL; the maintenance loop polls order status and cancels expired/stale orders. Paper/Live-shadow maintenance requires the linked Paper and Live order shape to match on asset, condition, outcome, GTD order type, `postOnly=false`, and price within `0.000001`; requested size is intentionally allowed to differ because Paper and Live stake sizing are separate. Shape mismatches record a discrepancy, disable that strategy's `LiveStakes`, and cancel correlated open live orders. The kill switch pauses new live orders and requests cancel-all.
...
- The default `StrategiesOnlyMode=true` screen shows the strategy tabs below plus the local `Dashboard Errors` tab, database selector, service banner, refresh button, and strategy edit controls.
- Strategies: all configured strategies, including `Follow leader`, with editable enable/live-stake/live-balance controls, Paper orders, open positions, lifecycle runs, entry-delay health, wins/losses, PnL, live-snapshot ROI, split live skip/ignored counts with ignored-reason breakdown, and closed-only ROI.
- Strategies / 24 hours, 6 hours, 1 hour: short strategy slices with recent order/fill/expiry/settlement counts, ROI, average fill price, entry-delay health, split live skip/ignored counts with ignored-reason breakdown, and top skip reason.
