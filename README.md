...
The catalog also seeds `84` ETH-only Optimized Average Premarket rows: 28 Up-trigger, 28 Down-trigger, and 28 neutral thresholds (`1..10`, then `15..100` step `5`). They preserve the ordinary eight-window maximum-average selector, its longer-window tie-break, threshold, and outcome logic. After the ordinary threshold passes, they enter only when that selector chose `3h`; selecting any of the other seven windows skips with `optimized_average_required_window_not_selected`. The rows are enabled for Paper on first insert, are hard-blocked from every Live path even if the database `Live` flag is later toggled, and are excluded from Child/Child ROI parent selection so the experiment does not alter existing mirror strategies.
