...
After any losing strategy settlement, the service calculates that strategy's realized Paper/Live PnL over the previous 12 hours. If the 12-hour total is negative, it sets `strategies.paused=true` and `paused_until_utc=now()+12 hours`; new Follow leader signals and BTC/crypto Up/Down entry orders are skipped during the pause with reason `strategy_paused`. The Dashboard `Paused` checkbox can also be set manually; a manual pause has no expiration until the checkbox is cleared.
