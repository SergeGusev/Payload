...
The historical Max/Min migration retained existing strategy IDs, codes, names, stakes, and runtime flags. It affected 680 direct Reference/Optimized/LowEnter variants plus 168 BpsConfirmed/DiffConfirmed composites, or 848 static variants; its historical 1,095-row static-plus-conditional inventory remains documented in [`Codex/Tasks/REFERENCE_AVERAGE_MAX_MIN_MIGRATION_2026-07-27.md`](Codex/Tasks/REFERENCE_AVERAGE_MAX_MIN_MIGRATION_2026-07-27.md). The 2026-08-13 v4 partial-`24h` denominator policy additionally reaches the 56 ETH single-window 3Hour variants and the exact 28 ETH Maker-GTD variants that share the decision path. The current static signal scope is therefore 932 strategies: BTC 312, ETH 406, and SOL 214. A further 247 Child/Child Progress/Child ROI/Child Progress ROI rows are conditionally downstream-affected because their runtime-selected parent can be an affected non-Paper-only Reference or Confirmed variant, giving an inclusive potential catalog scope of 1,179: BTC 408, ETH 469, and SOL 302. A Child decision can actually change only while that row is active, its PnL/ROI gates select an affected parent, and the corresponding assignment remains active. Because IDs are retained, aggregate historical statistics span multiple algorithms; raw decisions preserve their historical version and new decisions use algorithm version `4` with the versioned decision source. Absolute Premarket uses a separate extrema path and is not included.
