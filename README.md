...
For analyst-friendly querying, schema initialization also creates two read-only views. `polymarket_onchain_trade_details` joins decoded fills to token metadata and exposes maker, taker, maker/taker side, asset amounts, price, size, notional, fee, block time, tx hash, market, outcome, category, and resolved status. `polymarket_onchain_participant_details` joins materialized wallet activity, positions, and performance into one participant row per wallet with executions, buy/sell counts, markets, volume, fees, position counts, exposure, resolved PnL, ROI, win rate, score, and first/last trade time.
