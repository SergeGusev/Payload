...
Paper accounting also settles copied-wallet positions when a market resolution is observed from the market WebSocket or from the periodic closed-Gamma scan. Resolution handling selects only positive-size positions matching the condition/asset, then writes all settlement rows and zeroes all matching positions in one PostgreSQL transaction. A failure in either set-based statement rolls back both tables. The WebSocket path no longer runs the full copied-trader performance rebuild; `PaperAccountingWorker` performs that rebuild once per accounting cycle, every `PaperTrading:CopiedTraderPerformanceRefreshSeconds` (`30` seconds by default). Settlement warnings report separate open-position load, batch preparation, persistence, cache, and total durations.
