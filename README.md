...
WebSocket market-data updates also dispatch into paper trading so pending orders can fill and paper positions can be re-marked without waiting for the next scanner loop. The receive path captures the exact open Paper-order ids for the updated asset; deferred fill evaluation uses only those ids and the frame receipt time, so a queued older frame cannot fill an order created later or expire an order merely because queue processing was delayed. Before the exposure cache completes its initial load, the state is treated as unknown: coalescing is disabled and the updater performs the existing lazy load. The worker reads open Paper orders and positive-size positions from the shared exposure snapshot and filters them by the updated asset in memory; it does not reload the complete `paper_orders` or `paper_positions` tables for each WebSocket update. Changed marks for one asset are persisted in one PostgreSQL batch transaction before the exposure cache is updated, instead of opening one connection and issuing one upsert per copied wallet. Runtime position scans, settlement, copied-leader exits, and subscription discovery likewise use the server-side positive-size query, while individual BUY accounting reads one indexed `(copied_trader_wallet, asset_id)` position. Stale WebSocket snapshots are ignored after `MarketDataWebSocket:StaleAfterSeconds`.
...
Paper accounting also settles copied-wallet positions when a market resolution is observed from the market WebSocket or from the periodic closed-Gamma scan. Resolution handling selects only positive-size positions matching the condition/asset, then writes all settlement rows and zeroes all matching positions in one PostgreSQL transaction. A failure in either set-based statement rolls back both tables. Copied-trader scoring is outside this settlement path: order, fill, position, and settlement changes enqueue only their affected copied wallets at high priority, and `PaperCopiedTraderPerformanceWorker` recomputes at most `PaperTrading:CopiedTraderPerformanceWalletBatchSize` wallets per cycle (`25` by default). A bounded low-priority lexical reconciliation sweep seeds up to `PaperTrading:CopiedTraderPerformanceReconciliationSeedWalletBatchSize` historical wallets per cycle (`100` by default), covering bootstrap, missed events, and later category metadata changes without scanning and rewriting every copied trader during settlement. The worker interval is `PaperTrading:CopiedTraderPerformanceRefreshSeconds` (`30` seconds by default) and can be disabled with `PaperTrading:CopiedTraderPerformanceProjectionEnabled`. Settlement warnings report separate open-position load, batch preparation, persistence, cache, and total durations.
