...
When `OnChainIngestion:BackgroundSignalCandidateRefreshEnabled` is true, another background worker converts recent on-chain wallet fills into `polymarket_onchain_signal_candidates` and `polymarket_onchain_signal_candidate_reasons`. This is a read-only decision-prep layer, not order placement. Each candidate represents one maker or taker wallet side from `polymarket_onchain_wallet_fills`, enriched with token metadata, category, market status, notional, wallet/category performance, score, ROI, win rate, and sample quality. Candidates are marked `Accepted` only when the fill is a BUY, the notional meets the configured minimum, the market/category metadata is known, the market is active/unresolved, and the wallet/category performance passes the configured sample, score, ROI, and win-rate gates. Otherwise the table records `Rejected` plus explicit reason codes such as missing category, missing performance, weak score, unsupported side, or too-small fill. The worker processes recent rows in bounded batches (`SignalCandidateBatchSize`, default `250`) over a rolling lookback (`SignalCandidateLookbackHours`, default `24`) so it can keep up with new blocks without backfilling the full historical table.
