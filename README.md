...
The catalog seeds `94` Optimized Average Premarket rows. The ETH grid contains 28 Up-trigger, 28 Down-trigger, and 28 neutral thresholds (`1..10`, then `15..100` step `5`); the BTC grid contains only the 10 Down-trigger thresholds `1..10`, each targeting `Up`. They preserve the ordinary eight-window maximum-average selector, its longer-window tie-break, threshold, and outcome logic. After the ordinary threshold passes, they enter only when that selector chose `3h`; selecting any of the other seven windows skips with `optimized_average_required_window_not_selected`. The rows are enabled for Paper on first insert, are hard-blocked from every Live path even if the database `Live` flag is later toggled, and are excluded from Child/Child ROI parent selection so the experiment does not alter existing mirror strategies. No BTC Up/neutral or SOL Optimized rows are seeded.

The BTC catalog also seeds `304` `LowerEnter` Premarket clones for every currently unserved BTC 5-minute Premarket source strategy: `298` Regular and `6` Progress rows. Each clone preserves its source strategy's signal, direction, timing, linked confirmation inputs, progression state rules, stake calculation, and guaranteed-depth BUY FAK execution, while accepting a Paper entry only when the actual average fill price is at most `0.50` inclusive. The clones have stable source mappings, independent strategy IDs and progression state, are grouped into dedicated `LowerEnter` Dashboard categories, and are always Paper-only. They cannot submit Live or Live-shadow orders and cannot be selected as Child or Child ROI parents. The existing `LowEnter Average Premarket` neutral Reference Average grid remains a separate already-covered family and is not duplicated by these clones.
...
`ETH Up or Down 5m Up/Down N bps Optimized Average Premarket` and neutral `ETH Up or Down 5m N bps Optimized Average Premarket` are 84 parallel Paper experiments over the full 28-threshold grid. `BTC Up or Down 5m Down N bps Optimized Average Premarket` adds exactly 10 Paper experiments for `N=1..10`; a qualifying BTC Down move targets `Up`. These strategies do not force the 3-hour average or remove any window. They first run the complete ordinary Reference Average decision, including maximum selection, longer-window tie-break, trigger direction, and `N bps` threshold; only then do they keep the entry when the ordinary selected window label is `3h`. Diagnostics record the selected and required windows and whether they matched. These rows cannot create Live-shadow or Live orders and cannot become parents for Child strategies.

`BTC/ETH/SOL Up or Down 5m N bps LowEnter Average Premarket` adds 84 neutral Reference Average Paper experiments: 28 thresholds per asset (`1..10`, then `15..100` step `5`). Each row keeps the ordinary maximum-average signal and contrarian outcome selection, then accepts the simulated FAK entry only when the actual executable ask-depth average fill price is at most `0.50`; exactly `0.50` is allowed, while a higher VWAP skips with `execution_price_above_strategy_cap`. Diagnostics retain the evaluated VWAP, inclusive cap, and cap result. New rows are enabled for Paper with `Live=false`, are enforced as Paper-only even if `Live` is toggled manually, and appear in separate per-asset `Bps LowEnter Average Premarket` Dashboard categories. They start collecting new history after deployment and do not rewrite the history of their source strategies.
