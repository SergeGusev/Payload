...
Two fixed ETH children follow only actual same-market entries of `ETH Up or Down 5m 1 Diff Confirmed Average Premarket` (`b7c50005-0000-4000-8204-000000000001`). `ETH 5m 1 Diff Confirmed Average Premarket LossDiff 4+` increments its own durable value after each settled parent loss and resets it to zero after a settled parent win; it copies a subsequent parent entry while the pre-entry value is at least `4`. `ETH 5m 1 Diff Confirmed Average Premarket LossDiff 13+ Positive` increments after a parent loss and decrements after a parent win with a hard zero floor; it copies a subsequent parent entry while the pre-entry value is at least `13`. Only the exact parent's post-cutoff settled outcomes count: child outcomes and the editable `paper_lost_counter`/`live_lost_counter` stake controls do not affect either value. The ordered migration creates both values once at zero with one database UTC cutoff and never imports earlier parent history or resets state on an ordinary restart. Each child order has its own accounting identity but copies the parent's already frozen token/outcome, BUY side, target amount, price cap, FAK time-in-force, and other pre-submit constraints without a newer market read. Below-threshold child decisions are retained as auditable skips. Each child independently follows its own effective `live_stakes` setting after the parent entry: a parent kept on Paper is durably persisted first, an eligible Live child then uses the existing Live-shadow path, and no parent Live order is required or created. Existing parent-Live behavior remains unchanged. Both catalog rows are enabled and unpaused for Paper with `live_stakes=false`; this implementation does not apply the migration, generate historical child records, enable Live, deploy, restart the service, or change any current runtime setting.
