...
- Implemented live trading is BUY-only, tiny-size, and disabled by default. The legacy Follow Leader live path remains tightly gated; the paper/live shadow test path is limited to explicitly allowed variants, currently `BTC Up or Down 5m Skip 1`, `BTC Up or Down 5m Binance 10 bps`, `BTC Up or Down 5m Binance 17 bps Instant`, `BTC Up or Down 5m Binance 18 bps`, `BTC Up or Down 5m Binance 19 bps`, `BTC Up or Down 5m Binance 20 bps`, `BTC Up or Down 5m Binance 20 bps Instant`, `BTC Up or Down 5m Binance 21 bps`, `BTC Up or Down 5m Binance 22 bps`, `BTC Up or Down 5m Binance 23 bps`, and `SOL Up or Down 5m Binance 24 bps Instant`, and submits GTD limit BUY orders with `post_only=false`, a local market-relative cancel deadline, and a CLOB GTD expiration that includes the configured security buffer. Other ETH/SOL Middle, Binance bps, and Skip variants are Paper-only and are not in the live allowlist.
...
`ETH Up or Down 5m Skip` and `SOL Up or Down 5m Skip` mirror the non-Revert BTC Skip family: `Skip 1..5` uses consecutive previous ETH/SOL close-book results, `Skip 1..50 bps` gates on the previous streak's archived ETH/SOL start-to-close Binance move from `crypto_up_down_5m_odds_ticks`, and matching `Instant` variants price from executable ask depth. ETH/SOL Skip Revert variants are intentionally not seeded, and all ETH/SOL Skip rows remain Paper-only.
