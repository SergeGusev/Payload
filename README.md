...
Preview-only mode calculates the exact eligible row and strategy totals and logs a bounded ID sample without changing data. Eligibility dependencies are expressed directly as one relational PostgreSQL query so the planner can use anti-joins instead of invoking the diagnostic blocker function once per candidate; the function remains the independent parity oracle in PostgreSQL integration tests. Apply mode must be enabled separately after reviewing that preview; every batch then rechecks the exact logged run-ID allowlist inside one serializable transaction, adds UTC-day/skip-reason lifetime rollups and permanent runtime deduplication tombstones, suppresses only the corresponding Dashboard delete events, queues reconciliation, and deletes the same number of raw skipped rows or rolls back. Lifetime Dashboard and direct strategy-performance skip counts include the rollups, while `1h`/`6h`/`24h` views remain raw because the retention floor is older than those windows. The serializable recheck protects dependencies visible before commit, but the current tombstone does not prevent a new Paper/Live/shadow dependency committed after the raw run was deleted. Therefore `ApplyEnabled` must remain `false` until a separately approved late-dependency policy either rejects that write explicitly or atomically restores/preserves the archived run. A disposable PostgreSQL integration suite and a separate production read-only preview are still mandatory after that gap is closed.
