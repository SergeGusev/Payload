...
- Every Paper execution change is governed by the mandatory [Paper/Live execution parity contract](docs/architecture/PAPER_LIVE_PARITY.md); no proven Live equivalent means no Paper trade or Paper PnL claim.
...
Both retention switches remain `false` in checked-in configuration. Before production apply can be approved, deploy the schema/code, run a read-only preview that proves there are no legacy four-column tombstones (`archive_format_version IS NULL`) and that every versioned archive group exactly matches its rollup count/min/max, then run the focused PostgreSQL suite against the deployed schema. Existing legacy tombstones cannot be losslessly backfilled after their raw rows have already been removed.
