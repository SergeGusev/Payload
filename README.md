...
After deployment, verify that critical-shard receive gaps, side-effect queue delay/overflow counters, `PaperTradingMarketDataUpdater` phase errors, and PostgreSQL lock waits remain low across several busy 5-minute boundaries. Use that runtime evidence before changing worker concurrency or the `250`-event Dashboard projection batch.
