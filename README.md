...
Existing `RealizedPnlUsd` and Dashboard PnL/ROI values remain gross, before platform fees. `FeeUsd` is stored separately, and nullable net PnL is produced only when all contributing fees are `Calculated` or `VenueReported`; partial, unavailable, and legacy coverage must remain unknown rather than silently using zero. Aggregate Dashboard PnL/ROI therefore remains the existing gross metric and cannot be presented as net profitability.
