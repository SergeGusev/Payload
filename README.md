...
- Implemented live trading is BUY-only, explicitly risk-capped, and disabled by default. The legacy Follow Leader live path remains tightly gated. For opening-limit BTC/ETH/SOL 5-minute strategy variants, the Dashboard/DB `Live` flag (`strategies.live_stakes`) is the runtime switch for the paper/live shadow path: checking `Live` makes the strategy eligible for linked live-shadow orders when the normal live gates pass; every live-shadow submit uses BUY FAK market amount with `postOnly=false` and no GTD expiration, while Paper simulation can still use its strategy-specific GTD/FAK model. Unchecking `Live` disables new live-shadow entries. `Auto Live Pause` can temporarily suppress effective Live entries while preserving the manual `Live` flag.
...
`5`, it selects recently ended local Gamma markets. When provisional order-book
...
The BTC capped Gamma comparison variants have been removed from the active seed set. The uncapped Gamma comparison rows remain available for Paper comparison with `Live` off by default.

Trading strategies are stored in PostgreSQL table `strategies`. Built-in rows include `Follow leader` (`follow_leader`, id `f0110a0d-1ead-4c00-8b01-000000000001`) and the experimental BTC 5-minute family: 9 `BTC Up or Down 5m Less {Secs}` rows, 9 `BTC Up or Down 5m More {Secs}` rows, 9 `BTC Up or Down 5m Less {Secs}` Gamma rows, and 9 `BTC Up or Down 5m More {Secs}` Gamma rows, with `{Secs}` from `30` through `270` in 30-second steps, plus the other BTC 5-minute comparison, fixed Up/Down, Up/Down Simple, Maker, Maker 50, Middle bps/Instant, Skip bps, fixed Up/Down bps Instant, Binance, delayed, edge, ensemble, Markov, selector, Diff Instant, Diff Premarket, Diff Progress, Diff Limit Progress Premarket, Diff Real Limit Progress Premarket, Diff Reference Average Premarket, AdjustedDiff Instant, ShiftDiff Instant, and their Revert copies. It also includes ETH/SOL variants: Binance bps and Binance bps `Instant` rows, Paper-first Middle and Middle `Instant` rows, Skip `1..5`, fixed Up/Down bps `Instant`, the targeted `ETH Up or Down 5m Down 9 bps` stats-entry variant plus `ETH Up or Down 5m Down {1..50} bps Premarket` `-30s` variants and selected `-10s`/`-5s` production-candidate Premarket variants, Up/Down Simple, Diff/AdjustedDiff/ShiftDiff Instant rows, Diff Progress rows, Diff Limit Progress Premarket rows, Diff Real Limit Progress Premarket rows, Diff Reference Average Premarket rows, and BTC/ETH/SOL Up/Down `1..10` Diff Premarket plus matching BTC/SOL Up/Down `1..10` Diff Revert Premarket rows; ETH Up/Down `1..10` Diff Revert Premarket rows have been removed from production and are not seeded, and ETH/SOL Middle Revert, Skip Revert, and Diff/AdjustedDiff/ShiftDiff Revert Instant variants are not seeded. Diff rows cover BTC/ETH/SOL `Up N Diff Instant` and `Down N Diff Instant` thresholds `1..10` in steps of 1, then `15..150` in steps of 5; Diff Progress rows cover BTC/ETH/SOL `N Diff Up Progress` and `N Diff Down Progress` thresholds `1..50`; Diff Limit Progress Premarket and Diff Real Limit Progress Premarket rows cover BTC/ETH/SOL limits `1..5`; Diff Reference Average Premarket rows cover BTC/ETH/SOL thresholds `1..10`, `15`, `20`, `25`, and `30`; AdjustedDiff rows cover `Up N AdjustedDiff Instant` and `Down N AdjustedDiff Instant` thresholds `1..10`, `15`, and `20`; ShiftDiff rows cover shift `1..6` and threshold `1..12`. BTC Diff/AdjustedDiff/ShiftDiff Revert rows and SOL Diff Revert Premarket rows use the same trigger side and threshold as their parent row, but buy the opposite parent outcome. All opening-limit BTC/ETH/SOL variants, including Diff/AdjustedDiff/ShiftDiff/Diff Progress/Diff Limit Progress/Diff Real Limit Progress/Diff Reference Average, Simple, and Maker, can enter the Paper/Live-shadow path when their Dashboard `Live` flag is enabled and normal live gates pass. ETH/SOL variants are seeded with runtime `Live` disabled by default. Fifteen-minute strategy rows and PreOpen fixed-direction strategy rows were removed from production and are no longer seeded; missing strategy rows are treated as disabled by the worker, and the insert guard skips deleted 15m and PreOpen run rows. Paper, dry-run, and live order rows carry `strategy_id`, so strategy variants can run side by side and be compared by their order/fill outcomes. The dashboard `Strategies` tab aggregates one row per strategy from `strategies`, `paper_orders`, open positions, settlements, BTC strategy run lifecycle rows, and `live_orders`. Its default nested `All` tab shows selectable/copyable strategy names without internal codes, includes Paper open-position mark-to-market `MtM PnL` / `MtM ROI %` beside closed-only `Realized` and `Closed ROI %`, shows decision-health `Avg delay s` / `Max delay s` between planned `entry_due_at_utc` and actual `entered_at_utc`, and shows separate Live outcome metrics: Live orders, filled/open/settled counts, skipped/ignored counts with GTD-unfilled/cancel/reject breakdown, won/lost, settled cost basis, realized PnL, win/loss rate, average win/loss, profit factor, expectancy, ROI, and latest live order/settlement timestamps. The same `Strategies` tab also contains nested `24 hours`, `6 hours`, and `1 hour` tabs with sortable Live state, recent orders, fills, expired/open orders, entered/skipped/settled runs, wins/losses, win rate, ROI, realized PnL, the matching recent Live settled/skipped/ignored-breakdown/won/lost/realized/ROI block, filled cost, average fill price, entry-delay health metrics, top skip reason, and last activity timestamps. Each nested tab supports category filtering plus `Only positive`, `Enabled only`, `Live only`, `Big ROI`, and `Big settles` filters; `Live only` keeps rows with the Live flag enabled even when `Auto Live Pause` is currently suppressing effective live entries, `Big ROI` keeps rows with ROI greater than `10` (`Closed ROI` in `All`, recent `ROI` in period tabs), and `Big settles` keeps rows whose settled count is greater than `100` (`Settled` positions in `All`, recent `Settles` runs in period tabs). The `All` tab lets the `Enabled`, `Live`, and manual `Paused` flags be changed at runtime, shows read-only automatic `Auto Live Pause` state, and exposes editable per-strategy `Paper $`, `Live $`, `Paper Lost`, `Paper Cnt`, `Live Lost`, `Live Cnt`, and live-only `Live bal` values. It also shows closed-outcome quality metrics (`Avg win`, `Avg loss`, `Profit factor`, and `Expectancy`) beside `Win %` so high win-rate but negative-expectancy strategies are visible. The service refreshes enabled, manual pause, live stakes, and automatic Live pause state from PostgreSQL through a short in-memory cache; disabled strategies stop creating new Follow leader signals or Up/Down entries without a restart, manually paused strategies stay enabled but skip new Paper and Live entries, unchecked Live flags disable new Live/Paper-shadow entries, and auto-Live-paused strategies continue Paper while suppressing new Live/Paper-shadow entries. Existing Paper positions can still be settled, and copied leader exits can still be tracked so disabling or pausing a strategy does not strand already-open accounting.

The `Hide progress` checkbox in each `Strategies` tab hides rows whose visible strategy name contains `Progress`.

Fixed-outcome `Up/Down N bps Instant` variants appear in dedicated `Up Bps` and `Down Bps` Dashboard strategy categories per asset and interval instead of falling into `Other`. Up/Down Simple variants for BTC/ETH/SOL share one Dashboard category named `Simple`. Diff Instant variants appear in `Diff Up` and `Diff Down` categories per asset; Diff Progress variants share one `Diff Progress` category per asset; Diff Limit Progress Premarket variants share `Up Or Down 5 min Diff Limit Progress`; Diff Real Limit Progress Premarket variants share `Up Or Down 5 min Diff Real Limit Progress`; Diff Reference Average Premarket variants share `Up Or Down 5 min Diff Reference Average`; AdjustedDiff Instant variants appear in `AdjustedDiff Up` and `AdjustedDiff Down`; ShiftDiff variants appear by asset and shift. Revert copies use matching `... Revert` categories.

Current Premarket seed rows for BTC/ETH/SOL are the reference-average `Up/Down N bps Reference Average Premarket` family with thresholds `1..10` and `15..100` step `5`; they are grouped in one Dashboard category per asset. BTC/ETH/SOL also seed `N Diff Reference Average Premarket` rows with thresholds `1..10`, `15`, `20`, `25`, and `30`, plus `Up/Down 1..10 Diff Premarket` rows; BTC/SOL also seed matching `Up/Down 1..10 Diff Revert Premarket` rows, while ETH Diff Revert Premarket rows have been removed and are not seeded. ETH Down reference-average rows additionally use a separate `...down_reference_average_bps...` code/id family so they do not share the old ETH Down previous-result Premarket category or historical metrics. The old ETH `-30s` previous-result Premarket rows remain catalogued for history/settlement but are disabled by schema initialization, while the selected ETH previous-result `-10s` and `-5s` rows remain as legacy timed candidates.
...
Automatic strategy pausing is Live-only and opt-in per strategy through `LiveTrading:AutoLivePauseStrategies`; an empty list disables automatic Live pause for every strategy. The service configuration currently leaves this list empty, so automatic Live pause is disabled globally and startup clears any stored `auto_live_paused=true` rows. Entries may be strategy codes such as `follow_leader` or strategy ids. If strategies are later allowlisted again, pause and resume use different evidence: after a Live settlement, the service checks that strategy's settled Live orders over the last 12 hours; if more than one Live bet exists and the 12-hour Live realized PnL is negative, it sets `strategies.auto_live_paused=true`, stores `auto_live_paused_at_utc`, and stores `auto_live_pause_window_start_utc` as the start of the 12-hour Live-loss window. Live settlements never clear this flag. The strategy continues Paper entries, and after each Paper settlement the service checks all settled Paper rows from the stored Live-loss window start through the current settlement. When that anchored Paper realized PnL is positive and at least one of those Paper settlements happened after `auto_live_paused_at_utc`, it clears `auto_live_paused` and Live entries resume if the manual `Live` flag is still enabled. Paper settlements never set this flag. The Dashboard `Paused` checkbox remains a manual full Paper+Live pause.
...
variant's configured Paper stake multiplier when it becomes due. The Maker
variants use a maker-style slot rule: `BTC Up or Down 5m Up Maker` and
`BTC Up or Down 5m Down Maker` baseline the selected outcome best ask after
market start, keep an in-memory high-water best ask for that market/outcome, and
evaluate entries only on 30-second slots (`30s` through `270s`, maximum 9
attempts per 5-minute market). On a slot they create a minimum-size post-only GTD
BUY one tick below the current best ask only when that ask exceeds the previously
fixed high-water value. `BTC Up or Down 5m Up Maker 50` and
`BTC Up or Down 5m Down Maker 50` use the same high-water/slot rules, but place
at fixed `0.50` and only when the selected outcome best ask is strictly above
`0.50`. The high-water value is fixed at baseline and then updated only when a
Maker paper order is actually created; between-slot book moves and no-order slots
do not raise it. Flat or falling asks do not create orders, so after a Maker
order at `0.55` and a fall to `0.52`, the next Maker order waits until a later
30-second slot where the ask crosses `0.55` and reaches a new high such as
`0.56`; Maker orders can happen multiple times in one market and expire at
`marketEndUtc`.
...
`CURR Up or Down 5m N Diff Limit Progress Premarket` variants share
`Up Or Down 5 min Diff Limit Progress`; they exist for BTC, ETH, and SOL with
`N=1..5`. They also evaluate 30 seconds before the 5-minute market opens, keep
...
`CURR Up or Down 5m N Diff Real Limit Progress Premarket` variants share
`Up Or Down 5 min Diff Real Limit Progress`; they are seeded for BTC, ETH, and
SOL with `N=1..5`. They use the same premarket timing, UTC-day persistence,
...
Middle, Binance bps, non-Revert Skip, and fixed Up/Down bps
...
Binance, Skip, crypto Middle, crypto Binance bps, and crypto Skip bps `Instant`
...
that cap and can enter at any valid executable ask price below `1.00`. Simple
variants use the same FAK path with a fixed `0.50` cap and skip above the cap
instead of placing a resting order. Diff-family Instant variants use the same FAK
...
`BTC Up or Down 5m Skip 1..50 bps` variants inspect the immediately previous BTC 5-minute close-book result, walk backward through the current streak of identical inferred outcomes, sum each streak market's archived Binance BTC start-to-close absolute bps move, then buy the opposite outcome when the cumulative streak move reaches the configured threshold. For example, two consecutive `Up` results with `1 bps` each produce a current streak move of `2 bps`, so the `1` and `2` variants can enter while `3` still skips. Standard variants submit a fixed `0.50` GTD BUY. Matching `Instant` variants keep the same previous-result and cumulative previous-BTC-move gate, but submit BUY FAK taker entries from the selected outcome's executable ask depth using the same instant sizing/pricing path as Binance instant variants. The shared Skip bps streak calculation also upserts one row per target market into `btc_up_down_5m_result_streak_diagnostics`, with `close_book_streak_result_count` for the identical-outcome run length and `cumulative_abs_move_bps` for the accumulated bps over markets with BTC samples. Missing latest previous close-book evidence or missing latest previous BTC start/end samples waits for a later cycle; a completed below-threshold cumulative move skips with `btc_previous_market_move_below_bps_threshold`.

`BTC Up or Down 5m Up 1..50 bps Instant` and `BTC Up or Down 5m Down 1..50 bps Instant` reuse the same previous-result streak and cumulative BTC start-to-close bps gate as Skip bps Instant, but keep only one fixed countertrend side. The former BTC 15-minute fixed Up/Down bps Instant rows were removed from production and are no longer seeded because observed 15-minute volume/liquidity was too thin for current live use. The `Up` variants enter only when the previous streak points to buying `Up` after a `Down` streak; if the countertrend side is `Down`, they skip with `btc_previous_market_move_fixed_outcome_mismatch`. The `Down` variants mirror that behavior and enter only when the countertrend side is `Down`. Accepted entries use the executable ask-depth FAK path with an effective max BUY price of `1.00`, so the old `InstantOpeningLimitMaxPrice` default of `0.65` no longer blocks these fixed Up/Down bps entries.

`ETH Up or Down 5m Skip` and `SOL Up or Down 5m Skip` mirror the non-Revert BTC Skip family: `Skip 1..5` uses consecutive previous ETH/SOL close-book results, `Skip 1..50 bps` gates on the previous streak's archived ETH/SOL start-to-close Binance move from `crypto_up_down_5m_odds_ticks`, and matching `Instant` variants submit BUY FAK taker entries from executable ask depth. `ETH/SOL Up 1..50 bps Instant` and `ETH/SOL Down 1..50 bps Instant` reuse that same crypto streak/move gate, but keep only the requested fixed countertrend side before using the same uncapped fixed-outcome FAK path with effective max BUY price `1.00`. The former ETH/SOL 15-minute fixed Up/Down bps Instant rows were removed from production and are no longer seeded. ETH/SOL Skip Revert variants are intentionally not seeded; seeded ETH/SOL Skip/fixed rows are runtime-Live capable through the same Dashboard `Live` flag and normal live gates as other opening-limit rows.

Temporary guards: ETH Skip entries whose selected countertrend direction is `Up` are skipped before Paper, Live-shadow, or Live order creation with skip reason `eth_skip_up_direction_temporarily_disabled`; this applies to ETH `Skip`, `Skip bps`, and `Skip bps Instant` rows. `SOL Up or Down 5m Skip 42 bps Instant` also skips selected `Up` entries with `sol_skip_42_up_direction_temporarily_disabled`. Other SOL Skip rows and fixed ETH/SOL `Up/Down bps Instant` rows remain governed by their normal rules.
...
`ETH Up or Down 5m Binance` and `SOL Up or Down 5m Binance` bps variants mirror the BTC bps and `Instant` behavior, but read Binance `ETHUSDT` or `SOLUSDT` from `BinanceCryptoReference` and use the first archived crypto market-start reference from `crypto_up_down_5m_odds_ticks`. Standard variants place a Paper GTD BUY capped at `0.50`; `Instant` variants submit BUY FAK taker entries from selected-outcome executable ask depth with REST CLOB fallback when the cache is missing or stale. Their below-threshold skip reason is `crypto_reference_move_below_bps_threshold`. `ETH Up or Down 5m Down 9 bps` keeps the same previous-result fixed Down signal as the matching Instant strategy. The legacy selected ETH Premarket variants remain only at `-10s` for `40..42 bps` and `-5s` for `30..38 bps`; each samples the previous ETH reference price the same number of seconds before previous market close and enters the next market the same number of seconds before open.

`BTC/ETH/SOL Up or Down 5m Up/Down N bps Reference Average Premarket` reference-average variants run 30 seconds before the 5-minute market open. `N` covers `1..10` in steps of `1`, then `15..100` in steps of `5`. For `Up` trigger rows, the strategy chooses the largest full in-memory `middle` average from the `24h`, `12h`, `6h`, `3h`, `90m`, `45m`, `20m`, and `10m` crypto reference windows, compares the current Binance reference price with that maximum average, and buys `Down` when the current price is above the selected average by at least `N` bps. `Down` trigger rows mirror that rule and buy `Up` when the current price is below the selected maximum average by at least `N` bps. ETH Down reference-average rows also use distinct strategy codes from legacy ETH Down previous-result Premarket rows; the legacy `... Down N bps Premarket` rows stay in the catalog only for historical runs and settlement. These rows simulate the same taker BUY from executable ask depth using worst price `1 - tick`: fills are recorded at ask-depth VWAP, partial fills keep only the filled notional, and zero-fill cases are skipped/rejected instead of being treated as a buy at the cap. Their Live-shadow entry sends a BUY `FAK` market amount with the same worst-price cap; any unfilled remainder is cancelled by the exchange and a zero-fill response is stored as a rejected live entry. These rows are seeded with runtime `Live` disabled by default, except a first migration can copy an existing ETH Down legacy runtime flag to the matching new reference-average row before disabling the legacy row; enabled rows can enter the Paper/Live-shadow path when their Dashboard `Live` flags are enabled and all live gates pass.
...
- Strategies: all configured strategies, including `Follow leader`, with editable enable/live-stake/live-balance controls, per-row Paper/Live order navigation, Paper orders, open positions, lifecycle runs, entry-delay health, wins/losses, PnL, live-snapshot ROI, split live skip/ignored counts with ignored-reason breakdown, and closed-only ROI.
