...
Temporary guard: ETH Skip entries whose selected countertrend direction is `Up` are skipped before Paper, Live-shadow, or Live order creation with skip reason `eth_skip_up_direction_temporarily_disabled`. This applies to ETH `Skip`, `Skip bps`, and `Skip bps Instant` rows only; SOL Skip rows and fixed ETH `Up/Down bps Instant` rows remain governed by their normal rules.
