...
`BTC/ETH/SOL Up or Down 5m Up/Down Paired Maker GTD First Accepting` adds three equal-share pairs (six Paper-only legs). A dedicated Gamma worker polls only exact five-minute slugs in the configured day-ahead lead band and records the first service-observed snapshot with `acceptingOrders=true`; this is not claimed to be the venue's exact transition timestamp. Before either leg is attempted, both fresh S0 books define one common CLOB-normalized share quantity. Up uses `floor_to_tick(min(bestAsk-tick, 0.50))`; Down uses the same formula capped at `0.49`, so the two frozen limits total at most `0.99`. Each leg then receives its own fresh S0/S1 PostOnly acceptance sequence and is persisted independently. New `paired_maker_gtd_paper_v2` reads prove freshness with a bounded ordered local request/client-receipt/response/evaluation bracket. The authoritative venue snapshot timestamp remains mandatory audit evidence but may be old on a quiet unchanged book; it no longer acts as the receipt-freshness clock. Exact v1 orders remain grandfathered for lifecycle completion. There is no pair atomicity or rollback: a resting or filled leg remains until its original effective expiry even if its peer is rejected or never fills. Wire GTD expiry is market end and Paper expiry is one minute earlier. Later full-fill inference uses the same optimistic exact-token TouchNoDepth trigger, but this family additionally requires the same market-data service session and uninterrupted owning-shard component plus per-asset subscription generation since acceptance; service restart, owning-shard reconnect, or asset reassignment fails closed. Queue, depth, event size, and aggressor remain ignored. By explicit user approval, this exact group-`8224` family contributes to ordinary Paper metrics under the mandatory label `optimistic TouchNoDepth Paper; not Live-equivalent; may overstate fills`. Maker rebates are not modeled or included in Paper PnL, and Live submission is disabled.
