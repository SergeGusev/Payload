...
BTC/ETH/SOL Dashboard categories; they exist for BTC, ETH, and SOL with
...
`BTC/ETH/SOL Up or Down 5m N Child` and `N Child Progress` mirror the currently selected same-asset non-Child, non-Futures parent strategy for `N = 1..24` hour lookback windows. Plain Child excludes parent strategies whose names contain `Progress`; Child Progress can select them. All Child modes exclude Futures strategies from parent selection. The original Child families select the enabled, unpaused parent with the highest positive settled Paper PnL in the lookback window. The matching `N Child ROI` and `N Child Progress ROI` clones use the same mirroring rules, but first require the candidate parent to have at least `10` settled runs, at least `$60` settled stake, and positive PnL in the lookback window; they then rank by sample-adjusted ROI: `roi_pct * stake_usd / (stake_usd + 120)`. Child rows remain Paper-only by default and copy the accepted parent entry's market, outcome, notional, price, and share size.
