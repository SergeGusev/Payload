...
After the final high-priority and reconciliation wallet selection, the repository analyzes its session-local selector before rebuilding the derived performance rows. This gives PostgreSQL the actual small batch cardinality so it can use the existing wallet-leading Paper indexes instead of choosing full source-table scans; the step changes only temporary planner statistics and does not delete or compact Paper, Live, or Live-shadow history.
