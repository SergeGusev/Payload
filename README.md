...
WebSocket market-data updates also dispatch into paper trading so pending orders can fill and paper positions can be re-marked without waiting for the next scanner loop. The dispatch path reads open Paper orders and positive-size positions from the shared exposure snapshot and filters them by the updated asset in memory; it does not reload the complete `paper_orders` or `paper_positions` tables for each WebSocket update. Runtime position scans, settlement, copied-leader exits, and subscription discovery likewise use the server-side positive-size query, while individual BUY accounting reads one indexed `(copied_trader_wallet, asset_id)` position. Stale WebSocket snapshots are ignored after `MarketDataWebSocket:StaleAfterSeconds`.
