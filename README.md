...
Paper open-order lifecycle and position mark-to-market refresh run in separate hosted workers. Both use `PaperTrading:OpenOrderProcessingIntervalSeconds`, default `5`, but each independently awaits its own pass: a slow position-mark pass cannot delay the next open-order expiry/fill cycle, while mark calculations and their WebSocket-cache-first/REST-fallback lookup remain unchanged.
