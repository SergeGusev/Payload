...
distinct Paper order IDs in deterministic chunks of at most the existing
500-row candidate maximum, unions the exact matches inside the same
repeatable-read read-only transaction, and never sends the complete unbounded
preflight ID array to one PostgreSQL command. The final query still sorts and
