...
- Implemented live trading is BUY-only, explicitly risk-capped, and disabled by default. Every current catalog strategy honors its own Dashboard/DB `Live` flag (`strategies.live_stakes`) when the normal live gates pass. The legacy Follow Leader live path remains tightly gated, with its existing signal-source scheduling unchanged. FAK strategies retain the immutable BUY FAK cash amount and maximum price with `postOnly=false`; Maker GTD and inherited Child GTD entries retain their frozen share-based order type, post-only flag, price, and expiry. Dynamic Child Live copies the parent's original requested amount and normalization independently of the parent's Live flag. Unchecking `Live` disables new live entries.
...
Their own Live checkbox enables the same venue-minimum FAK intent through the
existing Live gates; no configured-stake multiplier or Paper-fill gate is added.
...
The service runs startup safety checks, the scanner, signal engine, paper/live maintenance engines, heartbeat writer, daily analytics report generator, market WebSocket client, and localhost-only IPC server. It writes rolling logs under its output `logs` directory by default. `ServiceLogging:Directory` can select another absolute directory; checked-in Production configuration uses `D:\1\logs`. If that configured directory is unavailable during logger setup, the service reports the failure to stderr and falls back to its output `logs` directory so the logging path alone does not prevent startup. Standard `System.Net.Http.HttpClient` request lifecycle messages are retained only at Warning or above. Routine strategy skips keep a compact Information record with the strategy, market, reason, diagnostics length and SHA-256 fingerprint; their complete diagnostics are emitted at Debug when that level is enabled and remain in the unchanged persisted run. Maker-GTD diagnostics required for lifecycle evidence remain complete at Information or above.
...
The exact ETH Up 8 bps pair has a separate command and approval boundary. Its
default preview is also server-enforced read-only:

```powershell
dotnet run --project src/PolyCopyTrader.Service/PolyCopyTrader.Service.csproj -- --backfill-eth-up8-lossdiff-history
```

Apply requires `--apply --approved-contract-digest
sha256:b83c8ba3ee3b432a551aeeb1ed76d33d30e76ff4765c63831341ad9f12fe7be5`.
It is frozen to parent `b7c50005-0000-4000-8137-000000000108`, cutoff
`2026-08-27T21:18:38.378192Z`, exact source and full-chain digests, 46 Reset 3+
and 21 Positive 16+ memberships, one complete chain per short transaction, and
marker `20260828_eth_up8_lossdiff_history_backfill_batched_v1`. Twenty-six Reset
chains have only persisted parent-chain evidence; no missing book snapshot is
invented. The command preserves strategy flags, LossDiff state/events, parent
rows, and all Live or venue state. Before every batch it rereads the frozen
parent source and compares the source, full-chain, selected-membership, and
protected-state digests against the invocation baseline. Existing PostgreSQL
projection and copied-trader-performance triggers remain enabled: 402 primary
history rows synchronously produce the normal projection/refresh work, which the
running workers may consume and coalesce. The already committed marker remains
bound to the execution-time contract digest `sha256:ab25b0935aea29c666905707a6b3487135336142ec93f8c195ad65954e123cb8`;
the newer digest gates only the corrected command implementation and its
...
`MarketDataWebSocket:MaxSubscribedAssets=0` means no local cap; prefer `SubscriptionScope` for semantic narrowing because a numeric cap can exclude the BTC/ETH/SOL assets the strategy needs. Closed zero-size Paper positions are retained in PostgreSQL history but are excluded from operational subscriptions; only positive-size positions contribute asset ids. The WebSocket supervisor shards the desired asset ids across multiple `ClientWebSocket` connections instead of using one huge all-active subscription. BTC/ETH/SOL 5-minute Up/Down asset ids from the active registry are isolated into the dedicated `PolymarketMarketWebSocket:crypto-updown-5m-critical` shard before the remaining operational assets are allocated, so Diff result capture does not depend on old paper-position assets sharing a huge shard. `MarketDataWebSocket:ShardMaxAssets` defaults to `3000`, `MaxShardConnections` defaults to `64`, and all outcomes for the same market/condition are kept on the same shard. Shard assignment is stable while the Gamma full scan is still discovering pages: new token ids are sent to existing shard connections with dynamic subscribe messages when there is capacity, instead of restarting all existing shards on every page. Each shard sends `PING` heartbeats, reconnects with exponential backoff, and resubscribes after reconnect. Repeated connect/close flaps without an accepted market update continue increasing the delay up to `MarketDataWebSocket:ReconnectMaxDelaySeconds`; the first update accepted by the side-effect queue (`Enqueued` or `Coalesced`) resets it to `ReconnectBaseDelaySeconds`. Malformed JSON, `PING`/`PONG`, zero-update payloads, rejected/dropped updates, and dispatch failures do not reset it. Subscription messages are sent in `MarketDataWebSocket:SubscriptionBatchSize` chunks to avoid huge single payloads. Each shard timestamps a complete received frame immediately and places it in a bounded ordered transport-to-dispatch queue before Maker admission and application dispatch. `MarketDataWebSocket:ReceiveDispatchQueueCapacity` defaults to `64`; the one-producer/one-consumer FIFO never silently drops or coalesces a received frame. A full queue applies backpressure and reports prolonged saturation, while consumer failure follows the existing connection error/reconnect path. This local handoff does not change reconnect delays/reset policy and does not claim to explain a remote close status such as `1013`. Frame dispatch updates only the in-memory registry/cache and enqueues side effects. It logs every bulk frame of at least `100` updates and warns when dispatch itself takes at least one second, including queued/coalesced/failure, payload-size, and duration fields.

For each connection generation, the complete initial subscription sequence is a barrier: dynamic subscribe/unsubscribe writes wait behind it, and desired-asset changes received during initialization are reconciled before the generation can become ready. After the initial sequence finishes, the shard remains `Connecting` and does not start heartbeat traffic until the first complete non-empty inbound text frame is received. `MarketDataWebSocket:FirstFrameTimeoutSeconds`, default `10`, starts at that point; if no qualifying frame arrives, the exact socket is aborted, the diagnostic identifies `Reason=FirstFrameTimeout` and `Phase=FirstFrameWait`, and the existing reconnect/backoff path resumes. Qualification is transport readiness at complete frame receipt, before Maker admission, parsing, or application dispatch; it does not imply successful parsing, a Live-trading guarantee, or elimination of remote `1013` closes. The existing post-readiness watchdog and reconnect-backoff reset rules are unchanged.

The general side-effect worker serializes Paper fills, resolution recording, optional raw persistence, and trade diagnostics outside the receive loop. It never waits for Paper position-mark persistence: the existing dedicated `PaperPositionMarkWorker` refreshes marks independently at `PaperTrading:OpenOrderProcessingIntervalSeconds`, using the existing WebSocket-cache-first/REST-fallback lookup and single-position compare-and-set write. A slow mark refresh can therefore make unrealized valuation temporarily stale, but it cannot delay market-data order, fill, expiry, settlement, fee, diagnostic, or exposure-cache lifecycle work. Batch mark persistence remains available as a directly callable best-effort operation and retains its complete-state compare-and-set and `SKIP LOCKED` behavior, but the DI-wired general market-data handler does not call it. During a same-asset backlog, every nonreplaceable work item still performs its complete order, fill, expiry, settlement, fee, diagnostic, and exposure-cache lifecycle work. Exact-family ETH Maker-GTD v1/v2 orders use a separate evidence worker: every eligible `book`, `price_change`, `last_trade_price`, and `best_bid_ask` receipt is retained in per-asset FIFO order with its original source/receipt timestamps, source event id, fingerprint, prices, and book snapshot. That dedicated path performs only the existing exact-family Maker fill evaluation; it does not update position marks, settle markets, or handle unrelated Paper orders. Once exact Maker order ids have been copied to this lane, the same quote may still be coalesced on the general lane when no ordinary open Paper order requires it. Replaceable general quotes without ordinary open Paper orders are coalesced per asset to their latest bid; `last_trade_price`, resolutions, unknown/uninitialized exposure state, and updates for assets with ordinary open Paper orders are never coalesced. Enabling `MarketDataWebSocket:PersistMarketDataEvents` or `MarketDataWebSocket:PersistOrderBookSnapshots` disables general quote coalescing. `MarketDataWebSocket:SideEffectMaxPendingUpdatesPerAsset`, default `32`, remains a soft bound for replaceable general work; the Maker evidence lane has no drop or coalescing bound. Queue metrics report the dedicated Maker pending/enqueued/processed/failed counts separately. `MarketDataWebSocket:SideEffectDiagnosticQueueCapacity`, default `256`, may discard routine sampled frames when full but retains parse/API errors and other important diagnostics. Queue metrics are logged every `MarketDataWebSocket:SideEffectMetricsIntervalSeconds`, default `30`; queue delay or processing above `MarketDataWebSocket:SideEffectSlowProcessingMilliseconds`, default `1000`, produces an immediate first warning. Repeats with the same component/event/category/phase are summarized at that interval with counts and maxima, and the first later non-slow completion reports recovery; errors and Maker expiry-drain warnings remain immediate. General and dedicated Maker-GTD slow warnings classify the threshold breach as `QueueDelay`, `Processing`, or `QueueDelayAndProcessing`; they retain the available durations, event/asset and pending-count evidence and report the active phase/operation plus the slowest observed phase/operation and duration. The slowest phase is finalized across transitions, so a later fast phase cannot hide an earlier slow repository call or Maker publication/serialization wait. This change does not alter Maker-GTD evidence admission, ordering, publication, fill, expiry, drain, failure, or scheduling semantics; its dedicated-lane extension is telemetry-only. On service shutdown the WebSocket stops first and both workers drain accepted work.

When one accepted exact-family Maker-GTD event touches multiple resting orders, the dedicated updater loads their linked runs in one bounded lookup and processes distinct exposure-cache wallet keys with at most `MarketDataWebSocket:MakerGtdWalletMaximumConcurrency` concurrent groups, default `8` and valid from `1` through `16`. The concurrency key uses the cache's existing `Trim().ToUpperInvariant()` wallet identity, so publications that target one cache key remain sequential and retain matching-order order. Within each group, working position state remains separated by the exact persisted wallet string with ordinal equality: differently persisted case or whitespace variants are not merged in PostgreSQL, while repeated orders for one exact string compose the same position. The updater awaits every group before completing the event, so the dedicated queue still preserves its event-level per-asset FIFO, publication fence, expiry drain, and no-drop guarantees. Once an order is atomically terminal as Filled or Expired, its id is pruned from already queued later items; an item is skipped only when none of its originally eligible order ids remains Pending. A failed cache-key group records failure for its own remaining orders without suppressing already started independent groups; every order still uses the existing independent atomic persistence transaction.
...
The BTC runtime catalog contains `LowerEnter` Premarket clones for every surviving supported BTC 5-minute Premarket source strategy. The four approved dependent LowerEnter targets—Shift Progress `N=3`, Real Limit Progress `N=4,5`, and non-Revert Futures Basis `N=1`—are retired with their sources. Each surviving clone preserves its source strategy's signal, direction, timing, linked confirmation inputs, progression state rules, and stake calculation, but submits its simulated BUY FAK with a hard maximum order price of `0.50`. Paper consumes only immediately executable asks at or below that price and cancels the remainder, matching the order semantics available to Live FAK. The clones have stable source mappings, independent strategy IDs and progression state, are grouped into dedicated `LowerEnter` Dashboard categories, and honor their own Live checkbox using the same frozen FAK price cap. Their existing exclusion from Child and Child ROI parent selection remains unchanged. The existing `LowEnter Average Premarket` neutral Reference Average grid remains a separate already-covered family and is not duplicated by these clones.
...
`BTC/ETH/SOL Up or Down 5m N bps Futures Basis Premarket` variants run 30 seconds before the 5-minute market open. The retained grid uses `N` values `1`, `2`, `3`, `5`, `8`, `10`, `15`, and `20`, except that the non-Revert BTC and SOL `N=1` rows are retired; their Revert counterparts remain. They select the three live OKX linear USD fixed-expiry contracts with the nearest distinct expiries at or after the target market end and compare every best-bid/ask mid with the simultaneous OKX `<asset>-USD` index. The `N bps` threshold applies only to the nearest expiry. The second and third expiries confirm only its strict sign: both must be positive when the nearest basis is positive, or both negative when it is negative. Zero or an opposite sign skips with `futures_basis_confirmation_sign_mismatch`; a nearest absolute basis below `N` still skips with `futures_basis_move_below_bps_threshold`. Matching Revert variants perform the same three-expiry confirmation and only then invert the trade direction. All three contracts and their quotes must be present and fresh; there is no two-contract or perpetual fallback. Entries use the same executable ask-depth FAK Paper/Live-shadow path as the reference-average Premarket rows, and the seeded runtime `Live` flag is disabled by default.

`BTC/ETH/SOL Up or Down 5m N Child` modes mirror a selected same-asset non-Child, non-Futures, non-Paper-only parent strategy. Plain Child and Child ROI retain every `N=1..24` except the retired BTC Child `N=1` and SOL Child ROI `N=1`. After the staged negative-Gross catalog retirement, Child Progress retains only SOL `N=15,17,19,20,21,22,23,24`; Child Progress ROI retains BTC `N=4,5,6,7,8,9,22`, ETH `N=1,20`, and SOL `N=9`. Plain Child excludes parent strategies whose names contain `Progress`; Child Progress can select them. The parent-selection, minimum-sample, ROI, and Live-off Paper execution rules are unchanged for every surviving row. Each child independently honors its own Live checkbox and copies the selected parent's original requested amount and normalization, not the parent's filled notional or the child's Live stake setting. The inherited token, side, price cap, order type, post-only flag, and expiry remain unchanged; the child retains its own identifiers and Live balance checks. Existing LossDiff sizing is unchanged. Migration `20260713_remove_hopeless_progress_strategies` removed the earlier exact 57 rows. The additional exact 217 variants and the follow-up exact 15 targets remain retired from runtime catalogs; existing Product rows and history are preserved until their separately authorized cleanup.
...
Four fixed ETH LossDiff children are grouped under two exact parents. The existing `ETH 5m 1 Diff Confirmed Average Premarket LossDiff 4+` and `ETH 5m 1 Diff Confirmed Average Premarket LossDiff 13+ Positive` children follow only actual same-market entries of `ETH Up or Down 5m 1 Diff Confirmed Average Premarket` (`b7c50005-0000-4000-8204-000000000001`). The new `ETH 5m Up 8 bps Reference Average Premarket LossDiff 3+` and `ETH 5m Up 8 bps Reference Average Premarket LossDiff 16+ Positive` children follow only actual same-market entries of `ETH Up or Down 5m Up 8 bps Reference Average Premarket` (`b7c50005-0000-4000-8137-000000000108`). A Reset child increments its durable value after each settled exact-parent loss and resets it to zero after an exact-parent win; a Positive child increments after a loss and decrements after a win with a hard zero floor. A subsequent actual parent entry is copied only when the durable pre-entry value is at least the child's fixed threshold (`4`, `13`, `3`, or `16`). Only the corresponding exact parent's post-cutoff settled outcomes count: child outcomes and the editable `paper_lost_counter`/`live_lost_counter` stake controls do not affect these values. Each ordered migration creates its pair once at zero with one database UTC cutoff and never imports earlier parent history or resets state on an ordinary restart. Each child order has its own accounting identity but copies the parent's already frozen token/outcome, BUY side, target amount, price cap, FAK time-in-force, and other pre-submit constraints without a newer market read. Below-threshold child decisions are retained as auditable skips. Each child independently follows its own effective `live_stakes` setting after the parent entry: a parent kept on Paper is durably persisted first, an eligible Live child then uses the existing Live-shadow path, and no parent Live order is required or created. Existing parent-Live behavior remains unchanged. All four catalog rows are enabled and unpaused for Paper with `live_stakes=false`. The two approval-gated history commands remain isolated: `--backfill-eth-lossdiff-history` serves only the older 4+/13+ pair, while `--backfill-eth-up8-lossdiff-history` serves only the Up8 3+/16+ pair. Neither command changes current LossDiff state/events, strategy flags, Live settings, or venue state.
...
`CryptoReferencePriceRetentionWorker` automatically keeps a fixed **48 hours** of this standalone history for the exact symbols `BTC`, `ETH`, and `SOL`. It starts with the normally deployed service, independently of `CryptoReferencePriceHistory.Enabled`; no separate cleanup command is needed. Each transaction deletes at most `1000` rows with `sampled_at_utc < nowUtc - 48h` for one asset, rotating BTC/ETH/SOL and waiting at least `10s` after each completed batch. Rows exactly at the cutoff, newer/future rows and other symbols are preserved. Locked rows are skipped; transaction-local statement/lock limits are `5s`/`250ms`, and a failed batch is logged and followed by at least `60s` backoff. Existing old rows drain gradually, and restart resumes from the remaining rows. Logs include asset, UTC cutoff, affected count and duration; no extra database event table is used. Existing 24-hour cache loading/calculation windows are unchanged, and retention does not automatically expand if those settings are later increased. Odds archives, correlation samples, orders, strategies and other tables are untouched. Deletion makes space reusable by PostgreSQL; it does not promise immediate shrinkage of database files on disk.

Focused retention verification uses the existing test project. Set `$retentionRun` to an owned, marked `D:\CodexTemp\runs\...` session and point `TEMP`, `TMP`, and `TMPDIR` to its `temp` directory. For SQL tests, `POLYCOPYTRADER_TEST_POSTGRES_CONNECTION` must point only to a disposable native loopback PostgreSQL database named `pct_codex_skip_v2_<14-digit timestamp>_<8 lowercase hex digits>`, with its cluster at that marked run's `db\pgdata`; never use a user database. The integration fixture verifies that identity before schema initialization or writes.

```powershell
dotnet test tests/PolyCopyTrader.Tests/PolyCopyTrader.Tests.csproj --artifacts-path "$retentionRun/artifacts" --results-directory "$retentionRun/results" --filter "FullyQualifiedName~CryptoReferencePriceRetentionWorkerTests|FullyQualifiedName~CryptoReferencePriceRetentionPostgresIntegrationTests|FullyQualifiedName~CryptoReferencePriceAverageCacheTests|FullyQualifiedName~CryptoReferencePriceExtremaCacheTests"
```

After deploying the updated service from `master`, verify the retention batch logs and the remaining old-row count before calling the backlog cleanup complete. Source tests alone do not prove that the server is running the new policy.

`OkxExpiryFuturesReference` reads only public OKX data for BTC, ETH, and SOL. The all-fixed-expiry ticker source and each asset's `<asset>-USD` index run independent one-second success loops, while the live instrument catalog is refreshed separately, so one slow endpoint does not pace the other sources. A timeout, HTTP `429`, or HTTP `5xx` receives at most one idempotent retry after `100..250ms` jitter; cancellation, parse/contract errors and other statuses do not retry. Each attempt retains the configured two-second timeout, successful receipt time is recorded only after parsing, and the five-second stale gate is unchanged. Repeated failures for the same source are summarized instead of writing one warning/API-error row per second, with the first failure and recovery still explicit. For each Futures Basis Premarket decision it selects the three live linear USD fixed-expiry contracts with the earliest distinct expiries at or after the target 5-minute market end, with no maximum horizon and no perpetual fallback. The nearest contract supplies the signed `N bps` threshold signal; the next two must both confirm that raw basis sign before the standard or Revert direction rule is applied. Raw decision JSON retains the legacy singular fields for the nearest contract and also records all three instruments, expiry ranks and roles, prices, quote timestamps, bases, signs, horizons, and confirmation result. Existing strategy IDs, codes, runtime flags, and stored history remain unchanged. New rows use `okx_three_expiry_confirmed_futures_basis_bps_premarket` or its Revert counterpart as `decision_source`, so they can be separated from both the former Binance history and the one-expiry OKX history.
...
The Binance reference clients for **BTC, ETH and SOL** use the last valid trade
price without a price-age expiry while the corresponding socket is open and its
connection lifetime has not been cancelled. The legacy
`BinanceBtcUsdReference:StaleAfterSeconds` and
`BinanceCryptoReference:StaleAfterSeconds` settings remain accepted/validated for
configuration compatibility but no longer expire prices in these two getters.
An observed close, error, abort or service stop makes that connection's price
unavailable to new current-reference-price reads. After reconnect, each asset
requires its own first valid trade from the new connection: an ETH trade cannot
make the previous SOL price available, and Ping/Pong cannot supply a quote.

Both clients set native .NET `KeepAliveInterval` and `KeepAliveTimeout` to
**10 seconds**. After channel communication inactivity, .NET sends a Ping and
aborts the socket if its matching Pong is not processed within the timeout; the
existing reconnect backoff then applies. This is a connection check, not a
per-asset no-trade timer. The existing sequential receive/processing loop remains:
a long local processing delay can prevent reading a Pong and abort an otherwise
healthy connection. This does not guarantee network-only failure detection or
detection of every outage within 20 seconds. See the
[.NET keepalive documentation](https://learn.microsoft.com/en-us/dotnet/fundamentals/networking/websockets#keep-reading-to-process-pongs).

Price, source and original source/fetched timestamps are never refreshed merely
by reading the cache, by keepalive traffic or by reconnecting. Latest-price
publication, sampled-cache cadence and history sampling schedules are unchanged;
a periodic history sample during quiet connected trading may retain the same
quote and its original timestamps. Age-based ETH/SOL stale/recovery events are
no longer triggered by the stream. Existing diagnostic phase/counter collection
and sampled logs remain, separate from business availability. This change does
not alter already frozen/submitted orders, execution/book-freshness rules,
accounting, or the independent `BinanceTimedClose` quote-age/market-end checks;
an old reference quote does not thereby become a valid closing price.

The behavior runs in the existing service with no new configuration or process.
Focused verification uses `BinanceReferenceConnectionPolicyTests` and
`BinanceCryptoReferenceDiagnosticsTests`, plus parser, odds-archive, timed-close
and frozen-FAK/Live-shadow regressions in the normal test project; no running
service, exchange connection or application database is needed.
...
Paper open-order lifecycle and position mark-to-market refresh run in separate hosted workers. Both use `PaperTrading:OpenOrderProcessingIntervalSeconds`, default `5`, but each independently awaits its own pass: a slow position-mark pass cannot delay the next open-order expiry/fill cycle, while mark calculations and their WebSocket-cache-first/REST-fallback lookup remain unchanged. Exact-family Maker-GTD expiry also uses a starvation-free receipt handoff: market-data receipts already active when expiry requests admission finish first, later receipts cannot overtake that queued expiry, and receipts remain concurrent when no expiry is pending. While holding that exclusive admission, an expired order asks the dedicated Maker evidence lane to prioritize its asset and finish every already accepted eligible pre-expiry update before terminal evaluation; the legacy general-queue check remains as a compatibility fallback. The dedicated lane preserves FIFO within the asset, never interrupts the current in-flight item, never drops, coalesces, or fabricates a Maker evidence update, and resumes normal round-robin scheduling as soon as no matching update remains. A slow drain logs the order, asset, duration, and remaining matching count.

Exact-family Maker-GTD receipt admission and its accepted pre-expiry side-effect drain now share one absolute preflight deadline at `effective_expires_at_utc + MarketDataWebSocket:StaleAfterSeconds` (default `30` seconds); the deadline is not reset between phases or orders. Work completed before it follows the existing fill and continuity rules. If admission or drain is still incomplete at the deadline, only the wait is canceled: the underlying receipt/queue work remains intact, while the still-Pending order and linked Resting run are atomically finalized as Expired/Skipped with `maker_gtd_evidence_unavailable` and phase detail `receipt_admission_timeout` or `side_effect_drain_timeout`. Atomic eligibility prevents late work from filling or reopening the terminal order, and caller/service cancellation remains a propagated cancellation rather than a terminal mutation. This bounded fail-closed rule can omit a genuine pre-expiry touch that completes after the deadline and can therefore understate Paper fills, positions, and PnL; Live behavior is unchanged.
...
After the final high-priority and reconciliation wallet selection, the repository analyzes its session-local selector before rebuilding the derived performance rows. This gives PostgreSQL the actual small batch cardinality so it can use the wallet-leading partial index `ix_paper_positions_open_wallet` instead of choosing full source-table scans. The aggregate materializes the selected wallets' orders, positive-size positions, and settlements once. It resolves the latest Gamma category once per distinct condition needed by those rows, reads every order's fills through the existing `ix_paper_fills_order_time` lookup, reduces each source to wallet/category metrics, and only then produces category and `OVERALL` rows with `GROUPING SETS`. The `paper_positions` contribution still contains only currently open rows with `size_shares > 0`; settled-position counts, outcomes, and settlement PnL still come from `paper_position_settlements`, while fill counts, costs, proceeds, and fill PnL still come from `paper_fills`. A non-empty stored settlement category remains authoritative; Gamma is used for it only when the stored category is null or empty. Closed zero-size Paper positions remain stored as history, and these derived-read optimizations do not delete or compact any Paper, Live, or Live-shadow history.

Only the copied-trader performance aggregate command uses a dedicated 180-second database timeout. All other projection commands retain the connection default, and an aggregate failure still rolls back the projection transaction while leaving durable in-flight work for the existing retry path.
...
FAK live-shadow submissions retain the frozen BUY cash amount, `postOnly=false`, price cap, and no GTD expiration. Maker and inherited Child GTD submissions retain the frozen share-based GTD intent, including post-only flag and local/wire expiry. A zero-fill FAK response is stored as a rejected live entry instead of an open order; resting GTD shadows stay pending for reconciliation and actual venue fills. Checking any current strategy's Dashboard `Live` checkbox sets `strategies.live_stakes=true` and makes that strategy eligible for new live entries when all live gates pass; unchecking it stops new entries without a service redeploy. Operational CLI toggles can still enable exactly one strategy with `--set-live-stakes-only-code <code>` or exactly a set with `--set-live-stakes-only-codes code1,code2`; both forms intentionally disable Live for every other strategy.

The decision obtains its required order-book evidence before freezing the intent. Live submission does not fetch a newer book to resize, reprice, or apply a Paper-fill acceptance gate. Preflight checks clock drift and risk caps, blocks a Live BUY when the same condition already has an open Live BUY on a different outcome, enforces live bankroll/strategy-balance caps, signs the CLOB V2 payload locally, and sends `POST /order` with L2 headers. `LiveTrading:MaxOrderNotionalUsd` remains a hard emergency ceiling. Normal Live sizing uses Dashboard `Live $` and the existing `Live Lost` rules; dynamic Child uses the parent's original requested amount and normalization, while Follow Market retains its venue-minimum amount. The child's own Live balance applies. Existing LossDiff sizing is unchanged. Live preflight applies market/total exposure caps to open Live orders only; Paper backlog remains governed by Paper controls.

Live orders and events are persisted, and maintenance polls status and cancels expired/stale open orders. Reconciliation compares asset, condition, outcome, frozen order type and post-only flag. FAK comparison uses the original maximum price and target notional. New GTD comparison uses the original limit, normalized size/notional, and local expiry metadata, so later actual fill prices cannot change the submitted intent. Shape mismatches record a discrepancy, disable that strategy's `LiveStakes`, and cancel correlated open live orders. The kill switch pauses new live orders and requests cancel-all. The labeled optimistic Maker TouchNoDepth model applies only with Live off; its history is unchanged.
...
index. The legacy `PaperRun` payload exclusion then drives deterministic chunks
of at most the existing 500-row candidate maximum through one lateral existence
probe per distinct Paper order ID. Each probe uses the deployed
`ix_historical_parity_paper_run_order` expression index inside the same
repeatable-read read-only transaction, so PostgreSQL does not choose a global
audit-table `DISTINCT ... ANY(...)` scan. The final query still sorts and
limits only the strategy-local candidate keys before loading the full rows.
This avoids scanning the global chronological `LegacyUnknown` fill index to
discover the current strategy and does not change eligibility, parity
exclusions, keyset ordering, batch size, command timeout, or financial
calculations.
...
The conditional apply accepts exactly two dependency shapes. `FullChain` requires the unchanged exact fill/run/zero-size-position/settlement chain and accepts either settlement source `BtcUpDown5mGammaClosedMarket` at exactly the run settlement time or exact `MarketWebSocket` with settlement time at or before the run settlement time; every identity, economic, uniqueness, and accounting guard remains mandatory, and the apply updates fill, run, position, and settlement fee/net fields. `RunOnlyLegacy` requires exactly one unchanged, settled, economically self-consistent run and no position or settlement rows; it updates only the fill and run and never synthesizes missing accounting rows. The audit exclusion is expressed as four source-specific anti-joins for `PaperSellFill`, current `PaperRun` linkage, legacy `PaperRun` payload linkage, and `PaperPosition`/`PaperSettlement` evidence. This preserves the same exclusion set while allowing the legacy payload lookup to use `ix_historical_parity_paper_run_order`; batch size, transaction boundaries, timeouts, financial formulas, and retry behavior are unchanged.
