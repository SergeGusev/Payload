...
Only the existing `WITH event_rows` copied-trader performance aggregate uses a dedicated 180-second database command timeout. All other projection commands retain the connection default, and an aggregate failure still rolls back the projection transaction while leaving durable in-flight work for the existing retry path.
