...
Automatic strategy pausing after losing settlements is disabled. Losing Paper/Live settlements are logged, but the service does not set `strategies.paused=true` or `paused_until_utc` from PnL. On startup, schema initialization clears legacy timed pauses where `paused_until_utc` is set. The Dashboard `Paused` checkbox remains available for manual pauses; a manually paused strategy skips new Follow leader signals and BTC/crypto Up/Down entry orders with reason `strategy_paused` until the checkbox is cleared.
