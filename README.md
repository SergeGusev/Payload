...
All four retention/compaction switches remain `false` in checked-in configuration. Before either production apply path can be approved, deploy the schema/code, run a read-only preview that proves there are no legacy four-column tombstones (`archive_format_version IS NULL`) and that every versioned archive group exactly matches its rollup count/min/max, then run the focused PostgreSQL suite against the deployed schema. Direct compaction additionally requires an exact candidate/dependency count and a query-plan/size check for recent tombstone reconciliation before high-volume enablement; any new supporting production index is a separate concurrent-DDL approval. Existing legacy tombstones cannot be losslessly backfilled after their raw rows have already been removed.
