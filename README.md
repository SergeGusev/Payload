...
`BTC/ETH/SOL Up or Down 5m N bps LowEnter Average Premarket` adds 84 neutral Reference Average Paper experiments: 28 thresholds per asset (`1..10`, then `15..100` step `5`). Each row keeps the ordinary Max/Min envelope signal and contrarian outcome selection, then submits the simulated BUY FAK with a hard maximum order price of `0.50`. Paper fills only ask levels at or below `0.50`, permits a partial fill, and cancels the unfilled remainder; VWAP is recorded only as an execution outcome and never authorizes or rejects that same order. New rows are enabled for Paper with `Live=false`, are enforced as Paper-only even if `Live` is toggled manually, and appear in separate per-asset `Bps LowEnter Average Premarket` Dashboard categories. They start collecting new history after deployment and do not rewrite the history of their source strategies.

`ETH Up or Down 5m N bps Reference Average Maker GTD Premarket` adds 28 independent Paper-only Maker experiments for neutral ETH thresholds `1..10`, then `15..100` step `5`; the original FAK rows are unchanged. Each clone preserves the source Reference Average signal, selected outcome, timing, stake sizing, and risk gates. New `maker_gtd_paper_v2` placements make at most ten fresh attempts and compute the maximum-resting BUY candidate `floor_to_tick(min(bestAsk - tick, 0.99))` (the highest tick-aligned price strictly below `bestAsk` when the venue book is tick-aligned), freeze one share-based `GTD` intent with `postOnly=true`, and obtain a second fresh book observation solely to emulate the venue's PostOnly acceptance check. A BUY limit at or above that second best ask is rejected and the next attempt starts with a new intent; a lower limit becomes one `Resting` order. The wire GTD expiration is market end, giving the Paper order an effective expiry one minute earlier. While resting, the explicit optimistic `TouchNoDepth` model marks the whole order filled at its own limit when an authoritative later exact-token `last_trade_price` or current best ask is at or below the limit. Equality counts, while queue position, depth, event size, and aggressor side are intentionally ignored. A continuously observed order with no trigger expires unfilled; stale, missing, restarted, or reconnected evidence is classified unavailable. By explicit user approval on 2026-08-09, only this exact 28-strategy family—asset `ETH`, those neutral thresholds, behavior `ReferenceAverageBpsThresholdMakerGtdPremarket`, catalog ID `b7c50005-0000-4000-8223-{100+threshold, zero-padded to 12 digits}`, execution source `eth_reference_average_maker_gtd_paper`, cap `0.99`, and `PaperOnly=true`—is a closed ordinary-Paper exception: its orders, positions, fills, PnL, win rate, and performance intentionally enter standard Paper metrics. Exact-family `maker_gtd_paper_v1` records already persisted with the earlier one-tick-improvement formula are grandfathered for lifecycle completion and historical accounting; persisted contract version/formula fields distinguish them from v2. New v2 placement fails closed unless the exact family predicates pass. Every result must be labeled `optimistic TouchNoDepth Paper; not Live-equivalent; may overstate fills`. Live submission remains disabled, and no alias, clone, descendant, future strategy, different execution source, predicate mismatch, or changed execution semantic inherits the exception.

`BTC/ETH/SOL Up or Down 5m Up/Down Paired Maker GTD First Accepting` adds three equal-share pairs (six Paper-only legs). A dedicated Gamma worker polls only exact five-minute slugs in the configured day-ahead lead band and records the first service-observed snapshot with `acceptingOrders=true`; this is not claimed to be the venue's exact transition timestamp. Before either leg is attempted, both fresh S0 books define one common CLOB-normalized share quantity. Up uses `floor_to_tick(min(bestAsk-tick, 0.50))`; Down uses the same formula capped at `0.49`, so the two frozen limits total at most `0.99`. Each leg then receives its own fresh S0/S1 PostOnly acceptance sequence and is persisted independently. New `paired_maker_gtd_paper_v4` PostOnly GTD placements retain the bounded local request/client-receipt/response/evaluation S0/S1 freshness proof. The authoritative venue snapshot timestamp remains mandatory audit evidence but may be old on a quiet unchanged book. Exact v1/v2/v3 orders remain grandfathered under their former effective-expiry-at-market-end-minus-60-seconds and stated/wire-expiration-at-market-end semantics for lifecycle completion. There is no pair atomicity or rollback: a resting or filled leg remains until its original effective expiry even if its peer is rejected or never fills. New v4 legs remain eligible for Paper execution through market end and use market end plus 60 seconds as their stated/wire expiration because of the venue GTD security threshold; every other approved predicate and lifecycle rule remains unchanged. Under `paired_touch_no_depth_gap_recovery_v1`, service restart, owning-shard reconnect, asset reassignment, or delivery failure pauses TouchNoDepth inference and creates a new exact-asset observation fence. The first authoritative frame confirms the fence and cannot fill; only a later authoritative event in the same unchanged session/component/generation, with receipt and venue timestamp strictly after the fence and a different fingerprint, may fill. Missing gap events, cached state, REST snapshots, and pre-fence queued events are never backfilled. A segment confirmed before expiry may finish as expired-unfilled; absent or post-expiry confirmation remains evidence-unavailable. Queue, depth, event size, and aggressor remain ignored. By explicit user approval, this exact group-`8224` family contributes to ordinary Paper metrics under the mandatory label `optimistic TouchNoDepth Paper; not Live-equivalent; may overstate fills`; fill and expiry evidence additionally states `observation gaps are not backfilled; only future authoritative events after confirmed recovery are eligible`. Maker rebates are not modeled or included in Paper PnL, and Live submission is disabled.

The Max/Min migration retains existing strategy IDs, codes, names, stakes, and runtime flags. Its static catalog impact is 680 direct Reference/Optimized/LowEnter variants plus 168 BpsConfirmed/DiffConfirmed composites, for 848 signal-affected variants: BTC 312, ETH 322, and SOL 214. A further 247 Child/Child Progress/Child ROI/Child Progress ROI rows are conditionally downstream-affected because their runtime-selected parent can be one of the non-Paper-only affected Reference or Confirmed variants; this tier contains BTC 96, ETH 63, and SOL 88 rows. A Child decision can actually change only while that row is active, its PnL/ROI gates select an affected parent, and the corresponding assignment remains active. The inclusive potential catalog blast radius is therefore 1,095 strategies: BTC 408, ETH 385, and SOL 302. The 56 ETH single-window 3Hour variants pass through the same diagnostics builder but keep identical signals because `Amin == Amax`; Absolute Premarket uses a separate extrema path. Because IDs are retained, aggregate historical statistics span both algorithms; raw decisions distinguish the new regime with algorithm version `2` and the versioned decision source. The exact 1,095-row ID/code/name inventory, separated into static and conditional tiers, is in [`Codex/Tasks/REFERENCE_AVERAGE_MAX_MIN_MIGRATION_2026-07-27.md`](Codex/Tasks/REFERENCE_AVERAGE_MAX_MIN_MIGRATION_2026-07-27.md).
...
`BTC/ETH/SOL Up or Down 5m N Child` and `N Child Progress` mirror the currently selected same-asset non-Child, non-Futures, non-Paper-only parent strategy. Plain Child and Child ROI retain every `N=1..24`; BTC/SOL Child Progress also retain every `N=1..24`, while ETH Child Progress retains `N=7,12,15,16,17,18,20,22,23`. Child Progress ROI retains every BTC `N=1..24`, ETH `N=1,2,4,6,10,20`, and SOL `N=1,2,3,7,8,9,10,11,12,15,16,17,18,20,22,24`. Plain Child excludes parent strategies whose names contain `Progress`; Child Progress can select them. All Child modes exclude Futures and explicitly Paper-only experimental strategies from parent selection. The original Child families select the enabled, unpaused parent with the highest positive settled Paper PnL in the lookback window. The ROI modes first require at least `10` settled runs, at least `$60` settled stake, and positive PnL in the lookback window; they then rank by sample-adjusted ROI: `roi_pct * stake_usd / (stake_usd + 120)`. Child rows remain Paper-only by default and copy the accepted parent entry's market, outcome, notional, price, and share size. Schema migration `20260713_remove_hopeless_progress_strategies` purges the exact 57 retired Progress rows and their operational history after verifying there are no active Live orders.
...
`PaperFakFeeBackfill` performs the retained historical pure-Paper FAK migration online. It uses a fixed pre-fee-deployment cutoff, scans only BUY fills whose persisted execution source is exactly `btc_updown5m_fak_taker_paper` or `btc_updown5m_child_mirror_fak_paper`, forces the proven `Taker` role, and evaluates them through the same current fee calculator used for new fills. At the start of each sweep, the worker calculates lifetime `Gross realized (audit)` directly from the retained run/fill/settlement accounting rows, freezes eligible strategies in descending Gross order, and breaks ties by strategy ID. It keyset-pages every eligible fill of one strategy before moving to the next, so the most successful strategy is attempted first and normally reaches complete Net PnL and Net ROI coverage first. This rank changes scheduling only: the Gross values, fee/net formulas, cutoff, and eligibility rules are not rewritten. The default cadence is one batch of 50 rows every 15 seconds after a five-minute startup delay; the worker yields while Paper-entry persistence or market-data side-effect queues have pending work. At the previewed `2,196,391`-fill scale, the no-contention lower bound is about 7.6 days. Each short apply is atomic, conditional on the historical row and its settled accounting chain still matching, and idempotent on retry. Gross fields and their timestamps are not rewritten; calculated fee provenance is prefixed with `historical-current-paper-model-v1`, while run and settlement net PnL are populated separately. Temporary CLOB lookup failures and apply conflicts are retried on a later ranked sweep and are never persisted as zero fees; they do not prevent the worker from continuing to lower-ranked strategies in the current sweep.

The historical worker deliberately excludes GTD, Maker, ambiguous sources, already-accounted rows, and every `paper_live_shadow_actual_fill`. A Live-shadow fee belongs to the linked aggregate Live execution and is copied into Paper by the normal reconciliation path; rewriting only its Paper row would make Paper and Live accounting disagree. Dependency mismatches are reported as deferred conflicts rather than guessed or silently counted as complete. `ReachedEnd` means only that one keyset sweep ended; it does not prove that no deferred or unavailable legacy rows remain.
