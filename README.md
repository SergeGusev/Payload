...
`BTC Up or Down 5m Skip 0.1..5 bps` variants inspect only the immediately previous BTC 5-minute close-book result, buy the opposite outcome, and require the previous market's archived Binance BTC move from market start to a close-time sample to be at least the configured absolute bps threshold. Standard variants submit a fixed `0.50` GTD BUY. Matching `Instant` variants keep the same previous-result and previous-BTC-move gate, but price from the selected outcome's executable ask depth using the same instant sizing/pricing path as Binance instant variants. Missing previous close-book evidence or missing previous BTC start/end samples waits for a later cycle; a completed below-threshold move skips with `btc_previous_market_move_below_bps_threshold`.
