...
`ETH Up or Down 5m Skip` and `SOL Up or Down 5m Skip` mirror the non-Revert BTC Skip family: `Skip 1..5` uses consecutive previous ETH/SOL close-book results, `Skip 1..50 bps` gates on the previous streak's archived ETH/SOL start-to-close Binance move from `crypto_up_down_5m_odds_ticks`, and matching `Instant` variants price from executable ask depth. ETH/SOL Skip Revert variants are intentionally not seeded; seeded ETH/SOL Skip rows are runtime-Live capable through the same Dashboard `Live` flag and normal live gates as other opening-limit rows.
