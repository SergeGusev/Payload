...
Paper open-order lifecycle and position mark-to-market refresh run in separate hosted workers. Both use `PaperTrading:OpenOrderProcessingIntervalSeconds`, default `5`, but each independently awaits its own pass: a slow position-mark pass cannot delay the next open-order expiry/fill cycle, while mark calculations and their WebSocket-cache-first/REST-fallback lookup remain unchanged. Exact-family Maker-GTD expiry also uses a starvation-free receipt handoff: market-data receipts already active when expiry requests admission finish first, later receipts cannot overtake that queued expiry, and receipts remain concurrent when no expiry is pending. While holding that exclusive admission, an expired order now asks the side-effect queue to prioritize its asset and finish every already accepted eligible pre-expiry update before terminal evaluation. The queue preserves FIFO within the asset, never interrupts the current in-flight item, never drops or fabricates an update, and resumes normal round-robin scheduling as soon as no matching update remains. A slow drain logs the order, asset, duration, and remaining matching count. A genuinely non-completing in-flight handler is not discarded or timed out and therefore remains fail-closed.
