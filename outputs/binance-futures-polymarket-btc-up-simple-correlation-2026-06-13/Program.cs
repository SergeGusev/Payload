...
var xlsxPath = Path.Combine(options.OutputDir, "binance-futures-polymarket-btc-up-simple-correlation-2026-06-13.xlsx");
var htmlPath = Path.Combine(options.OutputDir, "binance-futures-polymarket-btc-up-simple-correlation-2026-06-13.html");
var csvPath = Path.Combine(options.OutputDir, "binance-futures-polymarket-btc-up-simple-correlation-2026-06-13.csv");
...
Console.WriteLine($"polymarket_result_days={reportRows.Count(row => row.Outcomes is { TotalCount: > 0 })}");
...
static async Task<IReadOnlyList<DailyReportRow>> LoadRowsAsync(ReportOptions options)
{
    ...
    var polymarketOutcomes = await LoadPolymarketOutcomesAsync(options);
    ...
    await Parallel.ForEachAsync(dates, parallelOptions, async (date, cancellationToken) =>
    {
        try
        {
            ...
            polymarketOutcomes.TryGetValue(date, out var outcomes);
...
static async Task<IReadOnlyDictionary<DateOnly, OutcomeCounts>> LoadPolymarketOutcomesAsync(ReportOptions options)
{
    ...
    var builder = new NpgsqlConnectionStringBuilder(connectionString)
    {
        ...
        Timeout = 10,
        CommandTimeout = 120,
        ApplicationName = "BinanceFuturesPolymarketBtcUpSimpleCorrelation"
    };
    ...
    const string sql = """
        ...
        polling_fallback AS (
            SELECT
                ...
                observation.market_start_utc,
                observation.winning_outcome
            FROM crypto_up_down_5m_result_polling_observations observation
            WHERE observation.asset_symbol = 'BTC'
              AND observation.market_start_utc >= @StartUtc
              AND observation.market_start_utc < @EndUtc
              AND observation.status = 'Resolved'
              AND observation.winning_outcome IN ('Up', 'Down')
              AND NOT EXISTS (
                  SELECT 1
                  FROM crypto_up_down_5m_websocket_resolved_markets ledger_row
                  WHERE ledger_row.asset_symbol = observation.asset_symbol
                    AND ledger_row.market_start_utc = observation.market_start_utc
              )
        ),
        paper_run_inferred AS (
            SELECT
                market_key,
                market_start_utc,
                CASE
            ...
            FROM (
                SELECT
                    COALESCE(NULLIF(condition_id, ''), market_id) AS market_key,
                    COALESCE(market_start_utc, market_end_utc - interval '5 minutes') AS market_start_utc,
                    CASE
                ...
                WHERE status = 'Settled'
                  ...
                  AND COALESCE(market_start_utc, market_end_utc - interval '5 minutes') >= @StartUtc
                  AND COALESCE(market_start_utc, market_end_utc - interval '5 minutes') < @EndUtc
            ) inferred_rows
            WHERE inferred_winning_outcome IS NOT NULL
            GROUP BY market_key, market_start_utc
        ),
        paper_run_fallback AS (
            SELECT
                'BTC'::text AS asset_symbol,
                inferred.market_start_utc,
                inferred.winning_outcome
            FROM paper_run_inferred inferred
            WHERE inferred.winning_outcome IN ('Up', 'Down')
              AND NOT EXISTS (
                  SELECT 1
                  FROM ledger ledger_row
                  WHERE ledger_row.market_start_utc = inferred.market_start_utc
              )
              AND NOT EXISTS (
                  SELECT 1
                  FROM polling_fallback polling_row
                  WHERE polling_row.market_start_utc = inferred.market_start_utc
              )
        ),
        resolved AS (
            SELECT * FROM ledger
            UNION ALL
            SELECT * FROM polling_fallback
            UNION ALL
            SELECT * FROM paper_run_fallback
        )
        SELECT
            to_char((market_start_utc AT TIME ZONE 'utc')::date, 'YYYY-MM-DD') AS day_utc,
            count(*) FILTER (WHERE winning_outcome = 'Up')::integer AS up_count,
            count(*) FILTER (WHERE winning_outcome = 'Down')::integer AS down_count,
    ...
    var results = new Dictionary<DateOnly, OutcomeCounts>();
    await using var command = connection.CreateCommand();
    command.Transaction = transaction;
    command.CommandText = sql;
    command.Parameters.Add("StartUtc", NpgsqlTypes.NpgsqlDbType.TimestampTz).Value = options.Start.ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc);
    command.Parameters.Add("EndUtc", NpgsqlTypes.NpgsqlDbType.TimestampTz).Value = options.EndExclusive.ToDateTime(TimeOnly.MinValue, DateTimeKind.Utc);
    ...
    await using (var reader = await command.ExecuteReaderAsync())
    {
        while (await reader.ReadAsync())
        {
            ...
            results[date] = new OutcomeCounts(
                reader.GetInt32(1),
                reader.GetInt32(2),
                0,
                reader.GetInt32(3));
...
static ReportSummary BuildSummary(IReadOnlyList<DailyReportRow> rows)
{
    ...
    var missingPolymarketResultDays = rows.Count(row => row.Outcomes is null || row.Outcomes.TotalCount == 0);
    ...
    DateTimeOffset capturedAtUtc)
{
    ...
    html.AppendLine("<h1>BTCUSDT Binance Futures sentiment vs Polymarket BTC 5m Up/Down results</h1>");
    ...
    AppendMeta(html, "Result method", "Polymarket BTC 5m ledger/polling results, with settled paper-run inference fallback");
    html.AppendLine("</div>");
    ...
    DateTimeOffset capturedAtUtc)
{
    ...
    sheetData.Append(Row(rowIndex++, Text("A1", "Binance Futures BTCUSDT forecast vs Polymarket BTC 5m Up/Down results", Styles.Title)));
    ...
    sheetData.Append(Row(rowIndex++, Text("A5", "Forecast method", Styles.SummaryLabel), Text("B5", "First daily metrics count_long_short_ratio: >1 Up, <1 Down", Styles.SummaryValue), Text("D5", "Result method", Styles.SummaryLabel), Text("E5", "Polymarket BTC 5m ledger/polling results, with settled paper-run inference fallback", Styles.SummaryValue)));
    sheetData.Append(Row(rowIndex++, Text("A6", "Data source", Styles.SummaryLabel), Text("B6", "Binance daily metrics plus production PostgreSQL Polymarket result/paper-settlement history", Styles.SummaryValue)));
    rowIndex++;
    ...
    var headers = new[]
    {
        ...
        "Polymarket Up count",
        "Polymarket Down count",
        "Polymarket tie/other count",
        "Polymarket result count",
        "Up minus Down",
        "Actual majority",
        "Forecast hit"
    };
    ...
    string csvPath)
{
    ...
    {
        "Binance Futures BTCUSDT forecast vs Polymarket BTC 5m Up/Down results",
        ...
        $"Missing Polymarket result days: {summary.MissingPolymarketResultDays}",
        $"Tie days: {summary.TieDays}",
        $"XLSX: {xlsxPath}",
        $"HTML: {htmlPath}",
        $"CSV: {csvPath}",
        "Forecast method: first daily metrics count_long_short_ratio row; >1 Up, <1 Down.",
        "Result method: production PostgreSQL Polymarket BTC 5m result ledger/polling rows, with settled paper-run inference fallback where direct result rows are missing.",
        "Source: official Binance Data Collection futures/um daily metrics archive plus production PostgreSQL Polymarket result/paper-settlement history."
    };
...
internal sealed record ReportSummary(
    ...
    int MissingPolymarketResultDays,
    int TieDays,
