...
var skipGuard = args.Contains("--skip-guard", StringComparer.OrdinalIgnoreCase);

await WriteLineAsync($"PreOpen strategy cleanup started at {startedAt:O}");
await WriteLineAsync($"Dry run: {dryRun}");
await WriteLineAsync($"Verify only: {verifyOnly}");
await WriteLineAsync($"Skip guard install: {skipGuard}");
...
{
    ApplicationName = "DeletePreOpenStrategies"
};
...
if (verifyOnly)
{
    ...
    await WriteLineAsync($"PreOpen residual verification finished at {DateTimeOffset.UtcNow:O}");
    return;
}
...
if (skipGuard)
{
    await WriteLineAsync("Skipping missing-strategy guard install because --skip-guard was requested.");
}
else
{
    await InstallProductionMissingStrategyGuardAsync(connection);
}
...
SET enabled = false,
    ...
    updated_at_utc = now()
WHERE id IN (SELECT id FROM cleanup_preopen_strategy_ids)
...
WITH victim AS (
    ...
    WHERE d.strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
       OR d.correlation_id IN (SELECT correlation_id FROM cleanup_preopen_shadow_correlation_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    ...
    WHERE d.strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
       OR d.paper_order_id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
       OR d.live_order_id IN (SELECT id FROM cleanup_preopen_live_order_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    ...
    WHERE r.id IN (SELECT id FROM cleanup_preopen_run_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    SELECT l.id
    FROM live_orders l
    WHERE l.strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
       OR l.paper_order_id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    SELECT f.id
    FROM paper_fills f
    WHERE f.paper_order_id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    ...
    WHERE s.copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_preopen_safe_wallets)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    ...
    WHERE p.copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_preopen_safe_wallets)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    SELECT o.id
    FROM paper_orders o
    WHERE o.id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    ...
    WHERE r.signal_id IN (SELECT id FROM cleanup_preopen_signal_delete_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    SELECT s.id
    FROM signals s
    WHERE s.id IN (SELECT id FROM cleanup_preopen_signal_delete_ids)
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    SELECT s.id
    FROM strategies s
    WHERE s.id IN (SELECT id FROM cleanup_preopen_strategy_ids)
    LIMIT @batch_size
),
deleted AS (
...
await WriteLineAsync($"PreOpen strategy cleanup finished at {DateTimeOffset.UtcNow:O}");

async Task InstallProductionMissingStrategyGuardAsync(NpgsqlConnection db)
{
    await WriteLineAsync("Installing missing-strategy guard for deleted PreOpen runs...");
    await ExecuteNonQueryAsync(db, "SET lock_timeout = '60s';");
    try
    {
...
""");
    ...
    {
        await ExecuteNonQueryAsync(db, "SET lock_timeout = '3s';");
    }

    await WriteLineAsync("Missing-strategy guard installed.");
}
...
async Task CreateTargetTablesAsync(NpgsqlConnection db)
{
    ...
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_preopen_strategy_ids;
CREATE TEMP TABLE cleanup_preopen_strategy_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_strategy_ids (id)
SELECT id
FROM strategies
WHERE lower(code) LIKE '%_preopen_%'
   OR lower(name) LIKE '% preopen %'
   OR lower(name) LIKE '% pre-open %'
   OR lower(description) LIKE '%preopen%'
   OR lower(description) LIKE '%pre-open%';
""");

    await WriteLineAsync("Preparing target paper order ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_preopen_paper_order_ids;
CREATE TEMP TABLE cleanup_preopen_paper_order_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_paper_order_ids (id)
SELECT DISTINCT id
FROM paper_orders
WHERE strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
   OR lower(copied_trader_wallet) LIKE 'strategy:%preopen%';
""");

    await WriteLineAsync("Preparing target signal ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_preopen_run_ids;
CREATE TEMP TABLE cleanup_preopen_run_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_run_ids (id)
SELECT DISTINCT id
FROM strategy_market_paper_runs
WHERE strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
   OR paper_order_id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
   OR lower(COALESCE(category, '')) LIKE '%preopen%';

DROP TABLE IF EXISTS cleanup_preopen_signal_ids;
CREATE TEMP TABLE cleanup_preopen_signal_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_signal_ids (id)
SELECT DISTINCT signal_id
FROM paper_orders
WHERE id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
ON CONFLICT DO NOTHING;

INSERT INTO cleanup_preopen_signal_ids (id)
SELECT DISTINCT id
FROM signals
WHERE lower(trader_wallet) LIKE 'strategy:%preopen%'
ON CONFLICT DO NOTHING;

INSERT INTO cleanup_preopen_signal_ids (id)
SELECT DISTINCT signal_id
FROM strategy_market_paper_runs
WHERE id IN (SELECT id FROM cleanup_preopen_run_ids)
...
""");
    ...
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_preopen_live_order_ids;
CREATE TEMP TABLE cleanup_preopen_live_order_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_live_order_ids (id)
SELECT DISTINCT id
FROM live_orders
WHERE strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
   OR paper_order_id IN (SELECT id FROM cleanup_preopen_paper_order_ids);
""");
    ...
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_preopen_shadow_correlation_ids;
CREATE TEMP TABLE cleanup_preopen_shadow_correlation_ids (
    correlation_id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_shadow_correlation_ids (correlation_id)
SELECT DISTINCT correlation_id
FROM paper_live_shadow_decisions
WHERE strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
   OR paper_order_id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
   OR live_order_id IN (SELECT id FROM cleanup_preopen_live_order_ids)
ON CONFLICT DO NOTHING;

INSERT INTO cleanup_preopen_shadow_correlation_ids (correlation_id)
SELECT DISTINCT correlation_id
FROM live_orders
WHERE id IN (SELECT id FROM cleanup_preopen_live_order_ids)
...
""");
    ...
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_preopen_wallet_candidates;
CREATE TEMP TABLE cleanup_preopen_wallet_candidates (
    copied_trader_wallet text PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_wallet_candidates (copied_trader_wallet)
...
WHERE copied_trader_wallet LIKE 'strategy:%preopen%'
ON CONFLICT DO NOTHING;
...
DROP TABLE IF EXISTS cleanup_preopen_safe_wallets;
CREATE TEMP TABLE cleanup_preopen_safe_wallets (
    copied_trader_wallet text PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_safe_wallets (copied_trader_wallet)
SELECT copied_trader_wallet
FROM cleanup_preopen_wallet_candidates candidate
WHERE NOT EXISTS (
    SELECT 1
    FROM paper_orders orders
    WHERE orders.copied_trader_wallet = candidate.copied_trader_wallet
      AND orders.id NOT IN (SELECT id FROM cleanup_preopen_paper_order_ids)
);
""");
}
...
{
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_preopen_signal_delete_ids;
CREATE TEMP TABLE cleanup_preopen_signal_delete_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_preopen_signal_delete_ids (id)
SELECT target.id
FROM cleanup_preopen_signal_ids target
WHERE EXISTS (
    SELECT 1
    FROM signals signal
...
async Task WriteTargetSummaryAsync(NpgsqlConnection db, string stage)
{
    ...
    {
        ("target strategies in temp", "SELECT count(*)::int FROM cleanup_preopen_strategy_ids"),
        ("remaining target strategies", "SELECT count(*)::int FROM strategies WHERE id IN (SELECT id FROM cleanup_preopen_strategy_ids)"),
        ("actual PreOpen strategies by code/name/description", """
SELECT count(*)::int
FROM strategies
WHERE lower(code) LIKE '%_preopen_%'
   OR lower(name) LIKE '% preopen %'
   ...
   OR lower(description) LIKE '%pre-open%';
"""),
        ("target paper_orders in temp", "SELECT count(*)::int FROM cleanup_preopen_paper_order_ids"),
        ("remaining target paper_orders", "SELECT count(*)::int FROM paper_orders WHERE id IN (SELECT id FROM cleanup_preopen_paper_order_ids)"),
        ("remaining target paper_fills", "SELECT count(*)::int FROM paper_fills WHERE paper_order_id IN (SELECT id FROM cleanup_preopen_paper_order_ids)"),
        ("target run ids in temp", "SELECT count(*)::int FROM cleanup_preopen_run_ids"),
        ("target signal ids in temp", "SELECT count(*)::int FROM cleanup_preopen_signal_ids"),
        ("remaining target signals", "SELECT count(*)::int FROM signals WHERE id IN (SELECT id FROM cleanup_preopen_signal_ids)"),
        ("remaining target signal_rejections", "SELECT count(*)::int FROM signal_rejections WHERE signal_id IN (SELECT id FROM cleanup_preopen_signal_ids)"),
        ("remaining target runs", "SELECT count(*)::int FROM strategy_market_paper_runs WHERE id IN (SELECT id FROM cleanup_preopen_run_ids)"),
...
WHERE strategy_id IN (SELECT id FROM cleanup_preopen_strategy_ids)
   OR correlation_id IN (SELECT correlation_id FROM cleanup_preopen_shadow_correlation_ids);
"""),
        ("wallet candidates", "SELECT count(*)::int FROM cleanup_preopen_wallet_candidates"),
        ("safe wallets", "SELECT count(*)::int FROM cleanup_preopen_safe_wallets"),
        ("remaining safe-wallet paper_positions", "SELECT count(*)::int FROM paper_positions WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_preopen_safe_wallets)"),
        ("remaining safe-wallet settlements", "SELECT count(*)::int FROM paper_position_settlements WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_preopen_safe_wallets)"),
        ("remaining safe-wallet performance rows", "SELECT count(*)::int FROM paper_copied_trader_performance WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_preopen_safe_wallets)")
    };
    ...
    if (await TempTableExistsAsync(db, "cleanup_preopen_signal_delete_ids"))
    {
        await WriteScalarAsync(db, "  deletable signals", "SELECT count(*)::int FROM cleanup_preopen_signal_delete_ids");
    }
...
LEFT JOIN LATERAL (
    ...
    FROM paper_orders orders
    WHERE orders.copied_trader_wallet = candidate.copied_trader_wallet
      AND orders.id IN (SELECT id FROM cleanup_preopen_paper_order_ids)
...
LEFT JOIN cleanup_preopen_safe_wallets safe
...
""";
    ...
    if (openLiveOrders > 0)
    {
        throw new InvalidOperationException("Refusing cleanup because open/unsettled PreOpen live orders exist. Cancel/settle them before deleting history.");
    }
}

async Task WriteResidualPatternScanAsync(NpgsqlConnection db)
{
    await WriteLineAsync("Residual PreOpen pattern scan:");
    var summaries = new (string Label, string Sql)[]
    {
        ("strategies by PreOpen code/name/description", """
SELECT count(*)::int
FROM strategies
WHERE lower(code) LIKE '%_preopen_%'
   OR lower(name) LIKE '% preopen %'
   ...
   OR lower(description) LIKE '%pre-open%';
"""),
        ("paper_orders synthetic PreOpen wallets", """
SELECT count(*)::int
FROM paper_orders
WHERE lower(copied_trader_wallet) LIKE 'strategy:%preopen%';
"""),
        ("signals synthetic PreOpen wallets", """
SELECT count(*)::int
FROM signals
WHERE lower(trader_wallet) LIKE 'strategy:%preopen%';
"""),
        ("paper_positions synthetic PreOpen wallets", """
SELECT count(*)::int
FROM paper_positions
WHERE lower(copied_trader_wallet) LIKE 'strategy:%preopen%';
"""),
        ("paper_position_settlements synthetic PreOpen wallets", """
SELECT count(*)::int
FROM paper_position_settlements
WHERE lower(copied_trader_wallet) LIKE 'strategy:%preopen%';
"""),
        ("paper_copied_trader_performance synthetic PreOpen wallets", """
SELECT count(*)::int
FROM paper_copied_trader_performance
WHERE lower(copied_trader_wallet) LIKE 'strategy:%preopen%';
"""),
        ("strategy_market_paper_runs PreOpen categories", """
SELECT count(*)::int
FROM strategy_market_paper_runs
WHERE lower(category) LIKE '%preopen%';
""")
    };
