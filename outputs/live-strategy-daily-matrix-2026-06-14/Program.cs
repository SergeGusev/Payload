...
var outputPath = args.Length > 1
    ...
    : Path.Combine(outputDir, "live-strategy-daily-matrix-2026-06-14.xlsx");
...
{
    Console.WriteLine($"strategy={strategy.Code}\tlive_realized_pnl={strategy.LiveRealizedPnl:0.########}");
}
...
static async Task<List<StrategyRow>> LoadCurrentLiveStrategiesAsync(NpgsqlConnection connection, NpgsqlTransaction transaction)
{
    ...
    {
        rows.Add(new StrategyRow(reader.GetString(0), reader.GetString(1), reader.GetDecimal(2)));
    }
    ...
    decimal expectedGrandTotal)
{
    ...
    for (var index = 0; index < strategies.Count; index++)
    {
        if (index > 0 && strategies[index - 1].LiveRealizedPnl > strategies[index].LiveRealizedPnl)
        {
            throw new InvalidOperationException("Strategy columns are not sorted by ascending Live realized PnL.");
        }

        var columnName = ColumnName(index + 2);
        var header = GetText(cells[$"{columnName}1"]);
        if (!string.Equals(header, strategies[index].Name, StringComparison.Ordinal))
        {
            throw new InvalidOperationException($"Strategy header mismatch in column {columnName}.");
        }

        var strategyTotalCell = cells[$"{columnName}{totalRowIndex}"];
        var cachedStrategyTotal = decimal.Parse(strategyTotalCell.CellValue?.Text ?? "0", CultureInfo.InvariantCulture);
        if (Math.Abs(cachedStrategyTotal - strategies[index].LiveRealizedPnl) > 0.00000001m)
        {
            throw new InvalidOperationException(
                $"Strategy total mismatch for {strategies[index].Code}: expected {strategies[index].LiveRealizedPnl}, got {cachedStrategyTotal}.");
        }
    }
...
internal sealed record StrategyRow(string Code, string Name, decimal LiveRealizedPnl);
