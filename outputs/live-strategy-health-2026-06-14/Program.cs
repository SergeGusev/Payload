...
var builder = new NpgsqlConnectionStringBuilder(connectionString)
{
    ...
    Timeout = 10,
    CommandTimeout = 60,
    ApplicationName = "LiveStrategyHealthDiagnostic"
};
...
SELECT
    ...
    paused,
    auto_live_paused,
    (enabled AND NOT paused AND live_stakes AND NOT auto_live_paused AND live_available_balance >= live_stake_amount) AS effective_live_ready,
    live_stake_amount,
...
Console.WriteLine();
Console.WriteLine("== Per-strategy Live activity summary ==");
await PrintRowsAsync("""
SELECT
    strategy.code,
    count(live_order.id) FILTER (WHERE live_order.created_at_utc >= now() - interval '1 hour') AS created_1h,
    count(live_order.id) FILTER (WHERE live_order.created_at_utc >= now() - interval '6 hours') AS created_6h,
    count(live_order.id) FILTER (WHERE live_order.created_at_utc >= now() - interval '12 hours') AS created_12h,
    count(live_order.id) FILTER (WHERE live_order.created_at_utc >= now() - interval '24 hours') AS created_24h,
    count(live_order.id) FILTER (WHERE live_order.settled_at_utc >= now() - interval '24 hours') AS settled_24h,
    count(live_order.id) FILTER (WHERE live_order.status IN ('Submitted', 'Live', 'Delayed', 'Unmatched', 'CancelRequested')) AS open_like_now,
    ...
    max(live_order.created_at_utc) AS latest_live_created_utc,
    max(live_order.submitted_at_utc) AS latest_live_submitted_utc,
    max(live_order.updated_at_utc) AS latest_live_updated_utc,
    max(live_order.settled_at_utc) AS latest_live_settled_utc
...
Console.WriteLine();
Console.WriteLine("== Per-strategy paper run health, last 6h ==");
await PrintRowsAsync("""
SELECT
    strategy.code,
    count(run.id) AS runs_6h,
    count(run.id) FILTER (WHERE run.status = 'Open' AND run.entry_due_at_utc <= now()) AS open_due_now,
    count(run.id) FILTER (WHERE run.status = 'Open' AND run.entry_due_at_utc > now()) AS open_future,
    count(run.id) FILTER (WHERE run.status IN ('Entered', 'Settled')) AS entered_or_settled,
    ...
    max(run.updated_at_utc) AS latest_run_updated_utc,
...
Console.WriteLine();
Console.WriteLine("== Top paper-run skip/status reasons by strategy, last 6h ==");
await PrintRowsAsync("""
WITH grouped AS (
    SELECT
        strategy.code,
        ...
        row_number() OVER (
            PARTITION BY strategy.code
            ORDER BY count(*) DESC, max(run.updated_at_utc) DESC NULLS LAST, run.status, coalesce(run.skip_reason, '')
        ) AS reason_rank
...
SELECT
    ...
    skip_reason,
    runs,
    latest_entry_due_utc,
    latest_updated_utc
FROM grouped
WHERE reason_rank <= 5
ORDER BY code, runs DESC, latest_updated_utc DESC NULLS LAST;
""");

Console.WriteLine();
Console.WriteLine("== Latest paper run per current Live strategy ==");
await PrintRowsAsync("""
SELECT DISTINCT ON (strategy.code)
    strategy.code,
    run.status,
    coalesce(run.skip_reason, '') AS skip_reason,
    ...
    run.updated_at_utc,
    run.settled_at_utc,
    left(coalesce(run.skip_diagnostics_json::text, ''), 240) AS skip_diagnostics_prefix
FROM strategy_market_paper_runs run
INNER JOIN strategies strategy ON strategy.id = run.strategy_id
WHERE strategy.live_stakes
ORDER BY strategy.code, run.updated_at_utc DESC;
""");

Console.WriteLine();
Console.WriteLine("== Latest paper/live-shadow decision per current Live strategy ==");
await PrintRowsAsync("""
SELECT DISTINCT ON (strategy.code)
    strategy.code,
    decision.status,
...
WHERE strategy.live_stakes
ORDER BY strategy.code, decision.decision_created_at_utc DESC;
""");

Console.WriteLine();
Console.WriteLine("== Recent non-Simple paper runs, last 3h ==");
await PrintRowsAsync("""
SELECT
    strategy.code,
    run.market_slug,
    ...
    run.entered_at_utc,
    run.updated_at_utc,
    left(coalesce(run.skip_diagnostics_json::text, ''), 180) AS skip_diagnostics_prefix
FROM strategy_market_paper_runs run
INNER JOIN strategies strategy ON strategy.id = run.strategy_id
WHERE strategy.live_stakes
  AND strategy.code <> 'btc_up_down_5m_up_simple'
  AND run.updated_at_utc >= now() - interval '3 hours'
ORDER BY run.updated_at_utc DESC, strategy.code
LIMIT 80;
""");

Console.WriteLine();
Console.WriteLine("== Full diagnostics for latest non-Simple skips ==");
await PrintRawRowsAsync("""
SELECT
    strategy.code,
    run.market_slug,
    run.skip_reason,
    run.entry_due_at_utc,
    run.updated_at_utc,
    coalesce(run.skip_diagnostics_json::text, '') AS diagnostics_json
FROM strategy_market_paper_runs run
INNER JOIN strategies strategy ON strategy.id = run.strategy_id
WHERE strategy.live_stakes
  AND strategy.code <> 'btc_up_down_5m_up_simple'
  AND run.skip_reason IN (
      'btc_previous_market_move_below_bps_threshold',
      'btc_previous_close_book_orderbook_unavailable',
      'btc_previous_market_move_fixed_outcome_mismatch',
...
async Task PrintRawRowsAsync(string sql)
{
    await using var command = connection.CreateCommand();
    command.Transaction = transaction;
    command.CommandText = sql;
    ...
    while (await reader.ReadAsync())
    {
        ...
        {
            Console.WriteLine($"{reader.GetName(i)}={FormatValue(reader.GetValue(i))}");
        }
    }
