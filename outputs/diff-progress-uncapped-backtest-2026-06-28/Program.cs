...
internal static class Program
{
    ...
    private const decimal FormerMaxMultiplier = 10m;
    ...
    private static async Task<int> Main()
    {
        ...
        var outputDir = Path.Combine(repoRoot, "outputs", "diff-progress-uncapped-backtest-2026-06-28");
        ...
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-uncapped-strategies.csv"),
            BuildStrategyCsv(strategies),
            Encoding.UTF8);
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-uncapped-daily-by-strategy.csv"),
            BuildDailyCsv(dailyRows),
            Encoding.UTF8);
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-uncapped-asset-summary.csv"),
            BuildAssetSummaryCsv(assetSummaries),
            Encoding.UTF8);
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-uncapped-summary.md"),
            BuildMarkdownSummary(groupedCandles, strategies, assetSummaries),
            Encoding.UTF8);

        Console.WriteLine("Wrote uncapped Diff Progress backtest outputs to " + outputDir);
        ...
        List<DailyStrategyMetrics> dailyRows)
    {
        ...
        {
            var candleDay = candle.OpenTimeUtc.UtcDateTime.Date;
            ...
            foreach (var strategy in strategies)
            {
                ...
                strategy.Evaluate(candle, rawDiff);
            }
    ...
    private static string BuildStrategyCsv(IEnumerable<StrategyBacktest> strategies)
    {
        ...
        builder.AppendLine("asset,strategy_name,trigger_side,target_outcome,threshold,markets_evaluated,betting_episodes,entries,settled_entries,flat_entries,wins,losses,win_rate_pct,total_stake_units,settled_stake_units,pnl_units,roi_pct,avg_stake_units,max_multiplier,above_10_multiplier_entries,max_drawdown_units,active_days,positive_days,negative_days,flat_days,first_entry_utc,last_entry_utc");
        foreach (var strategy in strategies
            ...
            .ThenByDescending(item => item.RoiPct))
        {
            ...
            builder.AppendCsv(strategy.Above10MultiplierEntries);
    ...
    {
        var builder = new StringBuilder();
        builder.AppendLine("asset,strategy_name,trigger_side,target_outcome,threshold,day_utc,entries,settled_entries,flat_entries,wins,losses,total_stake_units,settled_stake_units,pnl_units,above_10_multiplier_entries,max_multiplier");
            ...
            .ThenBy(item => item.DayUtc))
        {
            ...
            builder.AppendCsv(row.Above10MultiplierEntries);
    ...
    {
        var builder = new StringBuilder();
        builder.AppendLine("asset,strategies,markets,first_market_utc,last_market_utc,entries,settled_entries,flat_entries,wins,losses,win_rate_pct,total_stake_units,settled_stake_units,pnl_units,roi_pct,best_pnl_strategy,best_pnl_units,best_pnl_roi_pct,best_roi_strategy,best_roi_pnl_units,best_roi_pct,worst_pnl_strategy,worst_pnl_units,worst_pnl_roi_pct");
    ...
    {
        var builder = new StringBuilder();
        builder.AppendLine("# Uncapped Diff Progress Backtest");
        builder.AppendLine();
        builder.AppendLine("Source: `outputs/binance-diff-time-chart-2026-06-28/binance-diff-timeseries.csv`.");
        builder.AppendLine("Model: evaluate each 5m market using the previous resolved 5m outcomes since `00:00 UTC`; reset counters at every UTC day boundary in both waiting and betting modes; enter when side-specific `Diff > N`; buy the opposite outcome; stake multiplier is uncapped `Diff - N`.");
        ...
        builder.AppendLine("The first partial UTC day in the six-month CSV is skipped per asset so daily counters start from midnight.");
        builder.AppendLine("The `above_10_multiplier_entries` metric counts entries that would have been capped by the previous max multiplier of 10.");
        builder.AppendLine();
        builder.AppendLine("## Asset Summary");
        builder.AppendLine();
        builder.AppendLine("| Asset | Markets | Entries | Settled | Flat | PnL units | ROI | Best PnL strategy | Worst PnL strategy |");
        builder.AppendLine("|---|---:|---:|---:|---:|---:|---:|---|---|");
        foreach (var summary in assetSummaries)
        {
            builder.AppendLine(CultureInfo.InvariantCulture,
                $"| {summary.AssetSymbol} | {summary.Markets} | {summary.Entries} | {summary.SettledEntries} | {summary.FlatEntries} | {summary.PnlUnits:0.##} | {summary.RoiPct:0.##}% | {EscapeMarkdown(summary.BestPnlStrategy)} ({summary.BestPnlUnits:0.##}) | {EscapeMarkdown(summary.WorstPnlStrategy)} ({summary.WorstPnlUnits:0.##}) |");
        }
    ...
    {
        builder.AppendLine("| Strategy | Entries | Settled | Win % | Stake | PnL | ROI | Max DD | Above10 |");
        builder.AppendLine("|---|---:|---:|---:|---:|---:|---:|---:|---:|");
        foreach (var strategy in strategies)
        {
            builder.AppendLine(CultureInfo.InvariantCulture,
                $"| {EscapeMarkdown(strategy.Name)} | {strategy.Entries} | {strategy.SettledEntries} | {strategy.WinRatePct:0.##}% | {strategy.SettledStakeUnits:0.##} | {strategy.PnlUnits:0.##} | {strategy.RoiPct:0.##}% | {strategy.MaxDrawdownUnits:0.##} | {strategy.Above10MultiplierEntries} |");
        }
    }
    ...
    private sealed class StrategyBacktest
    {
        ...
        public int Above10MultiplierEntries { get; private set; }
        ...
        public void Evaluate(Candle candle, int rawDiffBeforeMarket)
        {
            ...
            var multiplier = uncappedMultiplier;
            if (multiplier <= 0)
            {
                return;
            }
            ...
            if (multiplier > FormerMaxMultiplier)
            {
                Above10MultiplierEntries++;
            }
            ...
            dayMetrics.RecordEntry(multiplier, multiplier > FormerMaxMultiplier);
        ...
        DateTime dayUtc)
    {
        ...
        public void RecordEntry(decimal multiplier, bool above10)
        {
            Entries++;
            TotalStakeUnits += multiplier;
            MaxMultiplierSeen = Math.Max(MaxMultiplierSeen, multiplier);
