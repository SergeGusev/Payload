...
internal static class Program
{
    ...
    private static async Task<int> Main()
    {
        ...
        var outputDir = Path.Combine(repoRoot, "outputs", "diff-progress-backtest-2026-06-28");
        ...
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-backtest-strategies.csv"),
            BuildStrategyCsv(strategies),
            Encoding.UTF8);
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-backtest-daily-by-strategy.csv"),
            BuildDailyCsv(dailyRows),
            Encoding.UTF8);
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-backtest-asset-summary.csv"),
            BuildAssetSummaryCsv(assetSummaries),
            Encoding.UTF8);
        await File.WriteAllTextAsync(
            Path.Combine(outputDir, "diff-progress-backtest-summary.md"),
            BuildMarkdownSummary(groupedCandles, strategies, assetSummaries),
            Encoding.UTF8);

        Console.WriteLine("Wrote Diff Progress backtest outputs to " + outputDir);
    ...
    {
        var builder = new StringBuilder();
        builder.AppendLine("# Diff Progress Backtest");
        builder.AppendLine();
        builder.AppendLine("Source: `outputs/binance-diff-time-chart-2026-06-28/binance-diff-timeseries.csv`.");
        builder.AppendLine("Model: evaluate each 5m market using the previous resolved 5m outcomes since `00:00 UTC`; reset counters at every UTC day boundary in both waiting and betting modes; enter when side-specific `Diff > N`; buy the opposite outcome; stake multiplier is `min(Diff - N, 10)`.");
    ...
    private sealed class StrategyBacktest
    {
        ...
        public void Evaluate(Candle candle, int rawDiffBeforeMarket)
        {
            ...
            var multiplier = Math.Min(uncappedMultiplier, MaxMultiplier);
            if (multiplier <= 0)
            {
                return;
            }
