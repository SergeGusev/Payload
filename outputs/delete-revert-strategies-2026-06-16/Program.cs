...
const string TargetPattern = "revert";
...
var strategyLimit = GetNonNegativeIntArg(args, "--strategy-limit", 100);
...
await WriteLineAsync($"Revert strategy cleanup started at {startedAt:O}");
await WriteLineAsync($"Target strategy code/name regex: {TargetPattern}");
...
{
    ApplicationName = "DeleteRevertStrategies"
};
...
if (diagnoseLiveOrdersOnly)
{
    ...
    await WriteLineAsync($"Revert live order diagnostics finished at {DateTimeOffset.UtcNow:O}");
    return;
}
...
if (verifyOnly)
{
    ...
    await WriteLineAsync($"Revert residual verification finished at {DateTimeOffset.UtcNow:O}");
    return;
}
...
await WriteResidualPatternScanAsync(connection);
await WriteLineAsync($"Revert strategy cleanup finished at {DateTimeOffset.UtcNow:O}");
...
INSERT INTO cleanup_revert_strategy_ids (id, code, name, enabled, live_stakes)
SELECT id, code, name, enabled, live_stakes
FROM strategies
WHERE code ~* '{TargetPattern}'
   OR name ~* '{TargetPattern}'
ORDER BY code
{strategyLimitClause};
...
async Task WriteTargetSummaryAsync(NpgsqlConnection db, string stage)
{
    ...
    var summaries = new (string Label, string Sql)[]
    {
        ...
        ("remaining strategies by code/name regex", $"SELECT count(*)::int FROM strategies WHERE code ~* '{TargetPattern}' OR name ~* '{TargetPattern}';"),
...
{
    await WriteScalarAsync(db, "Target strategies by code/name regex", $"SELECT count(*)::int FROM strategies WHERE code ~* '{TargetPattern}' OR name ~* '{TargetPattern}';");
...
WHERE strategy.code ~* '{TargetPattern}'
   OR strategy.name ~* '{TargetPattern}';
""");
...
JOIN strategies strategy
    ON strategy.id = live_order.strategy_id
WHERE (strategy.code ~* '{TargetPattern}' OR strategy.name ~* '{TargetPattern}')
  AND (
...
""";
    ...
    if (openLiveOrders > 0)
    {
        throw new InvalidOperationException("Refusing cleanup because open/unsettled Revert live orders exist. Cancel/settle them before deleting history.");
    }
}

async Task WriteResidualPatternScanAsync(NpgsqlConnection db)
{
    await WriteLineAsync("Residual Revert pattern scan:");
    var summaries = new (string Label, string Sql)[]
    {
        ("strategies by code/name regex", $"SELECT count(*)::int FROM strategies WHERE code ~* '{TargetPattern}' OR name ~* '{TargetPattern}';"),
