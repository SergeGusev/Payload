...
const decimal DefaultPaperPnlThreshold = -100m;
...
var strategyLimit = GetNonNegativeIntArg(args, "--strategy-limit", 25);
...
var paperPnlThreshold = GetDecimalArg(args, "--paper-pnl-threshold", DefaultPaperPnlThreshold);
...
var paperPnlThresholdSql = paperPnlThreshold.ToString(System.Globalization.CultureInfo.InvariantCulture);

await WriteLineAsync($"Negative Paper PnL strategy cleanup started at {startedAt:O}");
await WriteLineAsync($"Target strategy Paper total PnL threshold: < {paperPnlThresholdSql}");
await WriteLineAsync("Target excludes follow_leader because it is the system non-synthetic copy-trading strategy.");
...
await WriteLineAsync($"Strategy limit: {(strategyLimit > 0 ? strategyLimit.ToString() : "all")}");
...
{
    ApplicationName = "DeleteNegativePaperPnlStrategies"
};
...
var strategyLimitClause = strategyLimit > 0
    ? $"LIMIT {strategyLimit.ToString(System.Globalization.CultureInfo.InvariantCulture)}"
    : string.Empty;
...
if (diagnoseLiveOrdersOnly)
{
    ...
    await WriteLineAsync($"Negative Paper PnL live order diagnostics finished at {DateTimeOffset.UtcNow:O}");
    return;
}
...
if (verifyOnly)
{
    ...
    await WriteLineAsync($"Negative Paper PnL residual verification finished at {DateTimeOffset.UtcNow:O}");
    return;
}
...
SET enabled = false,
    ...
    updated_at_utc = now()
WHERE id IN (SELECT id FROM cleanup_revert_strategy_ids)
...
WITH victim AS (
    ...
    WHERE d.strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
       OR d.correlation_id IN (SELECT correlation_id FROM cleanup_revert_shadow_correlation_ids)
...
WITH victim AS (
    ...
    WHERE d.correlation_id IN (SELECT correlation_id FROM cleanup_revert_shadow_correlation_ids)
    FOR UPDATE SKIP LOCKED
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    DELETE FROM cleanup_revert_run_ids target
    WHERE target.id IN (
        SELECT id
        FROM cleanup_revert_run_ids
        LIMIT @batch_size
    )
    RETURNING target.id
),
...
WITH victim AS (
    SELECT l.id
    FROM live_orders l
    WHERE l.id IN (SELECT id FROM cleanup_revert_live_order_ids)
    FOR UPDATE SKIP LOCKED
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    DELETE FROM cleanup_revert_paper_fill_ids target
    WHERE target.id IN (
        SELECT id
        FROM cleanup_revert_paper_fill_ids
        LIMIT @batch_size
    )
    RETURNING target.id
),
...
WITH victim AS (
    DELETE FROM cleanup_revert_settlement_ids target
    WHERE target.id IN (
        SELECT id
        FROM cleanup_revert_settlement_ids
        LIMIT @batch_size
    )
    RETURNING target.id
),
...
WITH victim AS (
    DELETE FROM cleanup_revert_position_ids target
    WHERE target.id IN (
        SELECT id
        FROM cleanup_revert_position_ids
        LIMIT @batch_size
    )
    RETURNING target.id
),
...
WITH victim AS (
    ...
    WHERE r.strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
    FOR UPDATE SKIP LOCKED
    LIMIT @batch_size
),
deleted AS (
...
await DeleteInBatchesAsync(connection, "strategy_market_paper_runs late paper_order", """
WITH victim AS (
    SELECT r.id
    FROM strategy_market_paper_runs r
    JOIN cleanup_revert_paper_order_ids target
        ON target.id = r.paper_order_id
    FOR UPDATE SKIP LOCKED
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    SELECT f.id
    FROM paper_fills f
    WHERE f.paper_order_id IN (SELECT id FROM cleanup_revert_paper_order_ids)
    FOR UPDATE SKIP LOCKED
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    DELETE FROM cleanup_revert_paper_order_ids target
    WHERE target.id IN (
        SELECT id
        FROM cleanup_revert_paper_order_ids
        LIMIT @batch_size
    )
    RETURNING target.id
),
...
WITH victim AS (
    ...
    WHERE r.signal_id IN (SELECT id FROM cleanup_revert_signal_delete_ids)
    FOR UPDATE SKIP LOCKED
    LIMIT @batch_size
),
deleted AS (
...
WITH victim AS (
    DELETE FROM cleanup_revert_signal_delete_ids target
    WHERE target.id IN (
        SELECT id
        FROM cleanup_revert_signal_delete_ids
        LIMIT @batch_size
    )
    RETURNING target.id
),
...
WITH victim AS (
    SELECT s.id
    FROM strategies s
    WHERE s.id IN (SELECT id FROM cleanup_revert_strategy_ids)
    FOR UPDATE SKIP LOCKED
    LIMIT @batch_size
),
deleted AS (
...
await WriteResidualPatternScanAsync(connection);
await WriteLineAsync($"Negative Paper PnL strategy cleanup finished at {DateTimeOffset.UtcNow:O}");
...
DROP TABLE IF EXISTS cleanup_revert_strategy_ids;
CREATE TEMP TABLE cleanup_revert_strategy_ids (
    id uuid PRIMARY KEY,
    code text NOT NULL,
    name text NOT NULL,
...
INSERT INTO cleanup_revert_strategy_ids (id, code, name, enabled, live_stakes, paper_pnl, realized_pnl_usd, unrealized_pnl_usd)
WITH position_mapped AS (
    SELECT
        strategy_by_wallet.id AS strategy_id,
        position_row.unrealized_pnl_usd
    FROM paper_positions position_row
    INNER JOIN strategies strategy_by_wallet
...
run_agg AS (
    SELECT
        ...
        COALESCE(sum(COALESCE(run.realized_pnl_usd, 0)), 0) AS realized_pnl_usd
    FROM strategy_market_paper_runs run
    WHERE run.status = 'Settled'
    GROUP BY run.strategy_id
),
strategy_pnl AS (
    SELECT
        strategy.id,
    ...
    WHERE strategy.code <> 'follow_leader'
      AND (COALESCE(run_agg.runs_count, 0) > 0 OR COALESCE(position_agg.unrealized_pnl_usd, 0) <> 0)
),
target AS (
    SELECT
        id,
        ...
        realized_pnl_usd,
        unrealized_pnl_usd,
        realized_pnl_usd + unrealized_pnl_usd AS paper_pnl
    FROM strategy_pnl
)
SELECT id, code, name, enabled, live_stakes, paper_pnl, realized_pnl_usd, unrealized_pnl_usd
FROM target
WHERE paper_pnl < {paperPnlThresholdSql}
ORDER BY paper_pnl ASC, code
{strategyLimitClause};

ANALYZE cleanup_revert_strategy_ids;

DROP TABLE IF EXISTS cleanup_revert_wallets;
CREATE TEMP TABLE cleanup_revert_wallets (
    copied_trader_wallet text PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_wallets (copied_trader_wallet)
SELECT 'strategy:' || code
FROM cleanup_revert_strategy_ids
ON CONFLICT DO NOTHING;

ANALYZE cleanup_revert_wallets;
""");

    await WriteLineAsync("Preparing target paper order ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_revert_paper_order_ids;
CREATE TEMP TABLE cleanup_revert_paper_order_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_paper_order_ids (id)
SELECT paper_order.id
FROM paper_orders paper_order
WHERE paper_order.strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
ON CONFLICT DO NOTHING;

INSERT INTO cleanup_revert_paper_order_ids (id)
SELECT paper_order.id
FROM paper_orders paper_order
JOIN cleanup_revert_wallets target
    ON target.copied_trader_wallet = paper_order.copied_trader_wallet
ON CONFLICT DO NOTHING;

ANALYZE cleanup_revert_paper_order_ids;
""");

    await WriteLineAsync("Preparing target paper fill ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_revert_paper_fill_ids;
CREATE TEMP TABLE cleanup_revert_paper_fill_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_paper_fill_ids (id)
SELECT fill.id
...
ANALYZE cleanup_revert_paper_fill_ids;
""");

    await WriteLineAsync("Preparing target run ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_revert_run_ids;
CREATE TEMP TABLE cleanup_revert_run_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_run_ids (id)
SELECT run.id
FROM strategy_market_paper_runs run
WHERE run.strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
ON CONFLICT DO NOTHING;
...
ANALYZE cleanup_revert_run_ids;
""");

    await WriteLineAsync("Preparing target signal ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_revert_signal_ids;
CREATE TEMP TABLE cleanup_revert_signal_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_signal_ids (id)
...
FROM strategy_market_paper_runs run
JOIN cleanup_revert_run_ids target
    ON target.id = run.id
...
WHERE decision.strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
...
WHERE live_order.strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
ON CONFLICT DO NOTHING;

INSERT INTO cleanup_revert_signal_ids (id)
SELECT dry_order.signal_id
FROM dry_run_orders dry_order
WHERE dry_order.strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
ON CONFLICT DO NOTHING;

ANALYZE cleanup_revert_signal_ids;
""");

    await WriteLineAsync("Preparing target live order ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_revert_live_order_ids;
CREATE TEMP TABLE cleanup_revert_live_order_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_live_order_ids (id)
...
ANALYZE cleanup_revert_live_order_ids;
""");

    await WriteLineAsync("Preparing target correlation ids...");
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_revert_shadow_correlation_ids;
CREATE TEMP TABLE cleanup_revert_shadow_correlation_ids (
    correlation_id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_shadow_correlation_ids (correlation_id)
...
SELECT decision.correlation_id
FROM paper_live_shadow_decisions decision
JOIN cleanup_revert_live_order_ids target
...
ANALYZE cleanup_revert_shadow_correlation_ids;
""");

    await WriteLineAsync("Preparing safe synthetic wallet ids...");
    await ExecuteNonQueryAsync(db, $"""
DROP TABLE IF EXISTS cleanup_revert_wallet_candidates;
CREATE TEMP TABLE cleanup_revert_wallet_candidates (
    copied_trader_wallet text PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_wallet_candidates (copied_trader_wallet)
SELECT copied_trader_wallet
FROM cleanup_revert_wallets
ON CONFLICT DO NOTHING;

DROP TABLE IF EXISTS cleanup_revert_safe_wallets;
CREATE TEMP TABLE cleanup_revert_safe_wallets (
    copied_trader_wallet text PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_safe_wallets (copied_trader_wallet)
SELECT copied_trader_wallet
FROM cleanup_revert_wallet_candidates;

DROP TABLE IF EXISTS cleanup_revert_settlement_ids;
CREATE TEMP TABLE cleanup_revert_settlement_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_settlement_ids (id)
SELECT settlement.id
FROM paper_position_settlements settlement
JOIN cleanup_revert_safe_wallets target
    ON target.copied_trader_wallet = settlement.copied_trader_wallet;

DROP TABLE IF EXISTS cleanup_revert_position_ids;
CREATE TEMP TABLE cleanup_revert_position_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_position_ids (id)
SELECT position.id
FROM paper_positions position
JOIN cleanup_revert_safe_wallets target
    ON target.copied_trader_wallet = position.copied_trader_wallet;

DROP TABLE IF EXISTS cleanup_revert_performance_keys;
CREATE TEMP TABLE cleanup_revert_performance_keys (
    copied_trader_wallet text NOT NULL,
    category text NOT NULL,
    PRIMARY KEY (copied_trader_wallet, category)
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_performance_keys (copied_trader_wallet, category)
...
{
    await ExecuteNonQueryAsync(db, """
DROP TABLE IF EXISTS cleanup_revert_signal_delete_ids;
CREATE TEMP TABLE cleanup_revert_signal_delete_ids (
    id uuid PRIMARY KEY
) ON COMMIT PRESERVE ROWS;

INSERT INTO cleanup_revert_signal_delete_ids (id)
SELECT target.id
FROM cleanup_revert_signal_ids target
WHERE EXISTS (
    SELECT 1
    FROM signals signal
...
ANALYZE cleanup_revert_signal_delete_ids;
""");
}
...
{
    await WriteLineAsync("Target strategy rows sample:");
    await WriteScalarAsync(db, "  target strategy rows selected", "SELECT count(*)::int FROM cleanup_revert_strategy_ids;");
    await using var command = new NpgsqlCommand("""
SELECT code, name, enabled, live_stakes, paper_pnl, realized_pnl_usd, unrealized_pnl_usd
FROM cleanup_revert_strategy_ids
ORDER BY paper_pnl ASC, code
LIMIT 50;
""", db);
    ...
    {
        count++;
        await WriteLineAsync($"  {reader.GetString(0)} | paper_pnl={reader.GetDecimal(4):0.########} | realized={reader.GetDecimal(5):0.########} | unrealized={reader.GetDecimal(6):0.########} | enabled={reader.GetBoolean(2)} | live_stakes={reader.GetBoolean(3)} | {reader.GetString(1)}");
    }
    ...
    {
        await WriteLineAsync("  ...sample limited to 50 rows.");
    }
}

string BuildRemainingTargetCountSql()
{
    return $"""
WITH position_mapped AS (
    SELECT
...
strategy_pnl AS (
    SELECT
        strategy.id,
        strategy.code,
        COALESCE(run_agg.realized_pnl_usd, 0) + COALESCE(position_agg.unrealized_pnl_usd, 0) AS paper_pnl
    FROM strategies strategy
    LEFT JOIN position_agg ON position_agg.strategy_id = strategy.id
    LEFT JOIN run_agg ON run_agg.strategy_id = strategy.id
    WHERE strategy.code <> 'follow_leader'
...
async Task WriteTargetSummaryAsync(NpgsqlConnection db, string stage)
{
    ...
    {
        ("target strategies in temp", "SELECT count(*)::int FROM cleanup_revert_strategy_ids"),
        ($"remaining strategies with Paper total PnL < {paperPnlThresholdSql}", BuildRemainingTargetCountSql()),
        ("target paper_pnl sum", "SELECT COALESCE(sum(paper_pnl), 0)::numeric(28,8) FROM cleanup_revert_strategy_ids"),
        ("target min paper_pnl", "SELECT COALESCE(min(paper_pnl), 0)::numeric(28,8) FROM cleanup_revert_strategy_ids"),
        ("target max paper_pnl", "SELECT COALESCE(max(paper_pnl), 0)::numeric(28,8) FROM cleanup_revert_strategy_ids"),
        ("target wallet candidates", "SELECT count(*)::int FROM cleanup_revert_wallet_candidates"),
        ("safe wallets", "SELECT count(*)::int FROM cleanup_revert_safe_wallets"),
        ("target paper_orders in temp", "SELECT count(*)::int FROM cleanup_revert_paper_order_ids"),
        ("remaining target paper_orders", "SELECT count(*)::int FROM paper_orders WHERE id IN (SELECT id FROM cleanup_revert_paper_order_ids)"),
        ("target paper_fills in temp", "SELECT count(*)::int FROM cleanup_revert_paper_fill_ids"),
        ("remaining target paper_fills", "SELECT count(*)::int FROM paper_fills WHERE paper_order_id IN (SELECT id FROM cleanup_revert_paper_order_ids)"),
        ("target run ids in temp", "SELECT count(*)::int FROM cleanup_revert_run_ids"),
        ("remaining target runs", "SELECT count(*)::int FROM strategy_market_paper_runs WHERE id IN (SELECT id FROM cleanup_revert_run_ids)"),
        ("target signal ids in temp", "SELECT count(*)::int FROM cleanup_revert_signal_ids"),
        ("remaining target signals", "SELECT count(*)::int FROM signals WHERE id IN (SELECT id FROM cleanup_revert_signal_ids)"),
        ("remaining target signal_rejections", "SELECT count(*)::int FROM signal_rejections WHERE signal_id IN (SELECT id FROM cleanup_revert_signal_ids)"),
        ("remaining target dry_run_orders", "SELECT count(*)::int FROM dry_run_orders WHERE strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)"),
        ("target live_orders in temp", "SELECT count(*)::int FROM cleanup_revert_live_order_ids"),
        ("remaining target live_orders", "SELECT count(*)::int FROM live_orders WHERE id IN (SELECT id FROM cleanup_revert_live_order_ids)"),
        ("remaining target shadow decisions", """
SELECT count(*)::int
FROM paper_live_shadow_decisions
WHERE strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
   OR paper_order_id IN (SELECT id FROM cleanup_revert_paper_order_ids)
   OR live_order_id IN (SELECT id FROM cleanup_revert_live_order_ids)
   OR signal_id IN (SELECT id FROM cleanup_revert_signal_ids);
"""),
        ("remaining target shadow discrepancies", """
SELECT count(*)::int
FROM paper_live_shadow_discrepancies
WHERE strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
   OR correlation_id IN (SELECT correlation_id FROM cleanup_revert_shadow_correlation_ids);
"""),
        ("remaining safe-wallet paper_positions", "SELECT count(*)::int FROM paper_positions WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_safe_wallets)"),
        ("remaining safe-wallet settlements", "SELECT count(*)::int FROM paper_position_settlements WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_safe_wallets)"),
        ("remaining safe-wallet performance rows", "SELECT count(*)::int FROM paper_copied_trader_performance WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_safe_wallets)"),
        ("target settlement ids in temp", "SELECT count(*)::int FROM cleanup_revert_settlement_ids"),
        ("target position ids in temp", "SELECT count(*)::int FROM cleanup_revert_position_ids"),
        ("target performance keys in temp", "SELECT count(*)::int FROM cleanup_revert_performance_keys")
    };
    ...
    if (await TempTableExistsAsync(db, "cleanup_revert_signal_delete_ids"))
    {
        await WriteScalarAsync(db, "  deletable signals", "SELECT count(*)::int FROM cleanup_revert_signal_delete_ids");
    }
...
FROM cleanup_revert_wallet_candidates candidate
LEFT JOIN LATERAL (
    SELECT count(*)::int
    FROM paper_orders orders
    WHERE orders.copied_trader_wallet = candidate.copied_trader_wallet
      AND orders.id IN (SELECT id FROM cleanup_revert_paper_order_ids)
...
{
    await WriteScalarAsync(db, "Target strategies in temp", "SELECT count(*)::int FROM cleanup_revert_strategy_ids;");
...
FROM live_orders
WHERE (
      strategy_id IN (SELECT id FROM cleanup_revert_strategy_ids)
      OR id IN (SELECT id FROM cleanup_revert_live_order_ids)
      OR paper_order_id IN (SELECT id FROM cleanup_revert_paper_order_ids)
  )
  AND (
...
{
    await WriteLineAsync("Residual negative Paper PnL scan:");
    var summaries = new (string Label, string Sql)[]
    {
        ($"strategies with Paper total PnL < {paperPnlThresholdSql}", BuildRemainingTargetCountSql()),
        ("paper_orders synthetic wallets", "SELECT count(*)::int FROM paper_orders WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_wallets);"),
        ("paper_positions synthetic wallets", "SELECT count(*)::int FROM paper_positions WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_wallets);"),
        ("paper_position_settlements synthetic wallets", "SELECT count(*)::int FROM paper_position_settlements WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_wallets);"),
        ("paper_copied_trader_performance synthetic wallets", "SELECT count(*)::int FROM paper_copied_trader_performance WHERE copied_trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_wallets);")
    };
    ...
    await WriteScalarBestEffortAsync(
        ...
        "SELECT count(*)::int FROM signals WHERE trader_wallet IN (SELECT copied_trader_wallet FROM cleanup_revert_wallets);",
...
static int GetNonNegativeIntArg(string[] args, string name, int defaultValue)
{
    for (var i = 0; i < args.Length - 1; i++)
    {
        if (string.Equals(args[i], name, StringComparison.OrdinalIgnoreCase)
...
static decimal GetDecimalArg(string[] args, string name, decimal defaultValue)
{
    for (var i = 0; i < args.Length - 1; i++)
    {
        if (string.Equals(args[i], name, StringComparison.OrdinalIgnoreCase)
