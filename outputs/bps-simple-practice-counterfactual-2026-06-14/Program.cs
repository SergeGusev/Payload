...
var outputPath = args.Length > 1
    ...
    : Path.Combine(outputDir, "bps-simple-practice-counterfactual-2026-06-14.xlsx");
...
var builder = new NpgsqlConnectionStringBuilder(connectionString)
{
    ...
    Timeout = 10,
    CommandTimeout = 600,
    ApplicationName = "BpsSimplePracticeCounterfactual"
};
...
var dailyByStrategyCsvPath = Path.Combine(outputDir, "bps-simple-practice-daily-by-strategy.csv");
var dailyTotalsCsvPath = Path.Combine(outputDir, "bps-simple-practice-daily-totals.csv");
var strategyTotalsCsvPath = Path.Combine(outputDir, "bps-simple-practice-strategy-totals.csv");
...
Console.WriteLine($"simple_practice_pnl={grandTotal.SimplePracticePnl:0.########}");
Console.WriteLine($"practice_vs_current={grandTotal.PracticeVsCurrent:0.########}");
...
static EvaluatedRow EvaluateRow(SourceRow row)
{
    ...
    var simplePractice = CalculateSimplePracticePnl(row, pricing, currentPnl);
...
static SimplePracticeResult CalculateSimplePracticePnl(SourceRow row, PricingInfo pricing, decimal currentPnl)
{
    ...
    if (string.IsNullOrEmpty(selectedOutcome))
    {
        return new SimplePracticeResult(false, 0m, false, null, null, null, "missing_selected_outcome");
    }
    ...
    {
        return new SimplePracticeResult(false, 0m, false, simpleCapPrice, null, null, "invalid_simple_practice_cap_price");
    }

    if (rawMarketPrice is not > 0m || rawMarketPrice.Value >= 1m)
    {
        return new SimplePracticeResult(false, 0m, false, rawMarketPrice, null, null, "missing_market_price");
    }
    ...
    {
        return new SimplePracticeResult(false, 0m, true, simpleCapPrice, null, null, "missing_market_result");
    }
    ...
    {
        return new SimplePracticeResult(false, 0m, true, simpleCapPrice, sizing.NotionalUsd, sizing.SizeShares, "invalid_simple_practice_sizing");
    }
    ...
    return new SimplePracticeResult(true, pnl, true, simpleCapPrice, sizing.NotionalUsd, sizing.SizeShares, "computed_resting_at_half");
}
...
static AggregateStats Aggregate(IEnumerable<EvaluatedRow> rows)
{
    ...
    foreach (var row in rows)
    {
        ...
        stats.SimplePracticePnl += row.SimplePracticePnl;
...
static AggregateStats SumStats(IEnumerable<AggregateStats> source)
{
    ...
    foreach (var stats in source)
    {
        ...
        total.SimplePracticePnl += stats.SimplePracticePnl;
    }
...
static string[] CsvValues(DateOnly? date, string? strategyCode, string? strategyName, AggregateStats stats)
{
    ...
    values.AddRange(new[]
    {
        ...
        FormatDecimal(stats.SimplePracticePnl),
        FormatDecimal(stats.PracticeVsCurrent),
...
static Worksheet BuildDailyTotalsWorksheet(IReadOnlyList<DailyTotalRow> rows, AggregateStats grandTotal)
{
    ...
    AppendTitleRow(sheetData, "A", 1U, "Daily totals across Up/Down bps strategies");
    ...
    return BuildWorksheet(sheetData, BuildStatsColumns(includeStrategy: false), $"A3:K{Math.Max(3, rowIndex)}");
}
...
static Worksheet BuildDailyByStrategyWorksheet(IReadOnlyList<DailyStrategyRow> rows)
{
    ...
    return BuildWorksheet(sheetData, BuildStatsColumns(includeStrategy: true), $"A3:M{Math.Max(3, rowIndex - 1)}");
}
...
static Worksheet BuildStrategyTotalsWorksheet(IReadOnlyList<StrategyTotalRow> rows, AggregateStats grandTotal)
{
    ...
    AppendStatsRow(sheetData, rowIndex, null, "TOTAL", null, grandTotal, totalStyle: true);
    return BuildWorksheet(sheetData, BuildStatsColumns(includeStrategy: true, includeDate: false), $"A3:L{Math.Max(3, rowIndex)}");
}
    ...
    AggregateStats grandTotal)
{
    ...
    var rows = new[]
    {
        ...
        ("SimplePracticePnl", "Counterfactual keeps the bps signal but applies Simple entry practice: executable ask at or below 0.50 remains current-equivalent; ask above 0.50 becomes a resting 0.50 BUY and is modeled as filled"),
        ("PracticeVsCurrent", "SimplePracticePnl minus CurrentPnl"),
    ...
    bool totalStyle = false)
{
    ...
    {
        stats.CurrentPnl,
        stats.SimplePracticePnl,
        stats.PracticeVsCurrent,
    ...
    AggregateStats grandTotal)
{
    ...
    var totalPractice = dailyTotals.Sum(row => row.Stats.SimplePracticePnl);
    if (Math.Abs(totalCurrent - grandTotal.CurrentPnl) > 0.00000001m ||
        Math.Abs(totalPractice - grandTotal.SimplePracticePnl) > 0.00000001m)
    {
...
internal sealed class AggregateStats
{
    ...
    public decimal SimplePracticePnl { get; set; }

    public decimal PracticeVsCurrent => SimplePracticePnl - CurrentPnl;
}
