...
SELECT
    ...
    snapshot.observed_runs_count,
    snapshot.entered_runs_count,
    snapshot.skipped_runs_count,
    snapshot.paper_condition_skipped_runs_count,
    snapshot.paper_not_accepted_runs_count,
    snapshot.settled_runs_count,
    snapshot.settled_positions_count,
    snapshot.won_positions_count,
    snapshot.lost_positions_count,
    snapshot.stake_usd,
    snapshot.realized_pnl_usd,
    snapshot.unrealized_pnl_usd,
    snapshot.total_pnl_usd,
    snapshot.win_rate_pct,
    snapshot.loss_rate_pct,
    snapshot.avg_win_pnl_usd,
    snapshot.avg_loss_pnl_usd,
    snapshot.profit_factor,
    snapshot.expectancy_pnl_usd,
    snapshot.roi_pct,
    snapshot.closed_roi_pct,
    snapshot.avg_entry_delay_seconds,
    snapshot.max_entry_delay_seconds,
    snapshot.avg_countertrend_score_bps,
    snapshot.avg_countertrend_signal_bps,
    snapshot.last_countertrend_signal_bps,
    snapshot.live_orders_count,
    snapshot.live_filled_orders_count,
    snapshot.live_open_orders_count,
    snapshot.live_settled_orders_count,
    snapshot.live_skipped_orders_count,
    snapshot.live_condition_skipped_orders_count,
    snapshot.live_technical_skipped_orders_count,
    snapshot.live_ignored_orders_count,
    snapshot.live_ignored_gtd_unfilled_count,
    snapshot.live_ignored_cancelled_orders_count,
    snapshot.live_ignored_rejected_orders_count,
    snapshot.live_won_orders_count,
    snapshot.live_lost_orders_count,
    snapshot.live_stake_usd,
    snapshot.live_realized_pnl_usd,
    snapshot.live_win_rate_pct,
    snapshot.live_loss_rate_pct,
    snapshot.live_avg_win_pnl_usd,
    snapshot.live_avg_loss_pnl_usd,
    snapshot.live_profit_factor,
    snapshot.live_expectancy_pnl_usd,
...
ORDER BY
    CASE WHEN strategy.code = 'follow_leader' THEN 0 ELSE 1 END,
    strategy.code
LIMIT @Limit;
""";
...
SELECT
    ...
    snapshot.paper_not_accepted_runs_count,
    snapshot.settled_runs_count,
    snapshot.won_runs_count,
    snapshot.lost_runs_count,
    snapshot.filled_cost_usd,
    snapshot.realized_pnl_usd,
