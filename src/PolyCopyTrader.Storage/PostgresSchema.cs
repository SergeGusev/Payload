...
public static class PostgresSchema
{
    ...
    [
        ...
        "strategy_market_paper_skip_tombstones_v2",
...
SELECT
    ...
    'Paper-only Maker GTD clone of ETH Up or Down 5m ' || threshold_bps::text || ' bps Reference Average Premarket. It preserves the source ETH reference-average envelope signal, including Max/Min boundaries selected from every usable configured window whether complete or incomplete; gaps and incomplete coverage alone do not block the signal. Bps use the oldest available real bucket in the 24h window even when that denominator window is incomplete; no other denominator is substituted. Direction, timing, stake sizing, and risk checks are unchanged. Before market open, make at most ten attempts to create one post-only GTD BUY using the maximum-resting formula floor_to_tick(min(bestAsk - tick, cap)), capped at 0.99; for a tick-aligned venue book this is the highest tick-aligned price strictly below the current best ask. After the first accepted order, stop retrying. The resting order expires one minute before market end. By explicit user approval, this exact family and maximum-resting price policy are a closed ordinary-Paper exception. The optimistic, non-Live-equivalent TouchNoDepth model marks the complete order filled at its limit on a later selected-token trade or current best ask at or below the BUY limit while queue position, depth, and observed size are ignored; this may overstate fills. Its records intentionally contribute to ordinary Paper orders, PnL, win rate, and performance. Every result must be labeled optimistic TouchNoDepth Paper; not Live-equivalent; may overstate fills. Live submission is disabled, and no alias, clone, descendant, future strategy, different execution source, predicate mismatch, or changed execution semantic inherits this exception.',
    true,
    false,
    1.00,
    1.00,
...
SELECT
    ...
    '30 seconds before ' || asset_symbol || ' 5m market open, compare the latest Binance ' || asset_symbol || '/USDT reference price with the envelope formed by the smallest and largest usable in-memory reference averages across every configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m window. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. If the current price is above the maximum boundary by at least ' || threshold_value::text || ' bps, BUY Down; if it is below the minimum boundary by at least ' || threshold_value::text || ' bps, BUY Up. Otherwise skip. Simulate a Paper FAK taker BUY with a maximum order price of 0.50, fill only immediately executable asks at or below that price, and cancel the remainder. Live execution is not supported for this Paper experiment.',
    true,
    false,
    1.00,
    1.00,
...
SELECT
    ...
    '30 seconds before ETH 5m market open, compare the latest Binance ETH/USDT reference price with the usable in-memory 3h reference average only; other configured windows do not participate. The 3h average is eligible whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. If the current price is above that 3h average by at least ' || threshold_value::text || ' bps, BUY Down; if it is below that 3h average by at least ' || threshold_value::text || ' bps, BUY Up. Otherwise skip. ' || description_suffix,
    true,
    false,
    1.00,
    1.00,
...
SELECT
    ...
    '30 seconds before ETH 5m market open, evaluate the latest Binance ETH/USDT reference price against every usable in-memory reference average across the configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m windows. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. ' ||
        CASE
            WHEN trigger_name = 'Up'
            THEN 'Use the largest usable reference average as the maximum boundary. If the current price moves Up by at least ' || threshold_value::text || ' bps from that maximum boundary, BUY Down.'
            WHEN trigger_name = 'Down'
            THEN 'Use the smallest usable reference average as the minimum boundary. If the current price moves Down by at least ' || threshold_value::text || ' bps from that minimum boundary, BUY Up.'
            ELSE 'Use the envelope formed by the smallest and largest usable reference averages. If the current price is above the maximum boundary by at least ' || threshold_value::text || ' bps, BUY Down; if it is below the minimum boundary by at least ' || threshold_value::text || ' bps, BUY Up.'
        END ||
        CASE
            WHEN trigger_name = 'Up'
...
SELECT
    ...
    '30 seconds before BTC 5m market open, evaluate the latest Binance BTC/USDT reference price against every usable in-memory reference average across the configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m windows. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. ' ||
        CASE
            WHEN trigger_name = 'Up'
            THEN 'Use the largest usable reference average as the maximum boundary. If the current price moves Up by at least ' || threshold_value::text || ' bps from that maximum boundary, BUY Down.'
            WHEN trigger_name = 'Down'
...
SELECT
    ...
    '30 seconds before SOL 5m market open, evaluate the latest Binance SOL/USDT reference price against every usable in-memory reference average across the configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m windows. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. ' ||
        CASE
            WHEN trigger_name = 'Up'
            THEN 'Use the largest usable reference average as the maximum boundary. If the current price moves Up by at least ' || threshold_value::text || ' bps from that maximum boundary, BUY Down.'
            WHEN trigger_name = 'Down'
...
SELECT
    ...
    CASE
        ...
        THEN 'Thirty seconds before ' || asset_symbol || ' 5m market open, compare the latest Binance ' || asset_symbol || '/USDT reference price with the largest usable in-memory reference average across every configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m window. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. If the current price moves Up by at least ' || threshold_name || ' bps from that maximum boundary, BUY Down from current premarket executable ask depth using the worst-price cap. Otherwise skip. Paper entry simulates the same taker BUY, while Live-shadow submits a market BUY amount so available liquidity is taken immediately and any remainder is cancelled.'
        ELSE 'Thirty seconds before ' || asset_symbol || ' 5m market open, compare the latest Binance ' || asset_symbol || '/USDT reference price with the smallest usable in-memory reference average across every configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m window. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. If the current price moves Down by at least ' || threshold_name || ' bps from that minimum boundary, BUY Up from current premarket executable ask depth using the worst-price cap. Otherwise skip. Paper entry simulates the same taker BUY, while Live-shadow submits a market BUY amount so available liquidity is taken immediately and any remainder is cancelled.'
    END,
    true,
    false,
    1.00,
...
-- Install the Live-state side of the retention gate while the first upgrade
-- lock is still held. This prevents an old trigger from accepting a Live
-- enable between guard installation and the later archive-schema statements.
...
BEGIN
    IF NEW.live_stakes
       AND (
        ...
        IF to_regprocedure(
            'public.restore_archived_strategy_runs_for_dependency_core(uuid,text,text,boolean,boolean,timestamp with time zone)')
            IS NOT NULL THEN
            EXECUTE
                'SELECT public.restore_archived_strategy_runs_for_dependency_core($1, NULL, NULL, false, true, $2)'
            USING NEW.id, live_boundary_utc;
        ELSIF to_regprocedure(
            'public.restore_archived_strategy_runs_for_dependency(uuid,text,text,boolean,boolean)')
            IS NOT NULL THEN
            -- First compatibility upgrade: the prior exact v1 restorer exists,
            -- while the v2-aware core is installed later in this initializer.
            IF EXISTS (
                SELECT 1
                FROM public.strategy_market_paper_skip_tombstones tombstone
                WHERE tombstone.strategy_id = NEW.id
                  AND tombstone.archive_format_version IS DISTINCT FROM 1
            ) THEN
                RAISE EXCEPTION
                    'Cannot safely enable Live while a possible legacy/incomplete tombstone exists.'
                    USING ERRCODE = '55000';
            END IF;

            FOR archived_market_id IN
                SELECT tombstone.market_id
                FROM public.strategy_market_paper_skip_tombstones tombstone
                WHERE tombstone.strategy_id = NEW.id
                  AND tombstone.archive_format_version = 1
                  AND tombstone.run_updated_at_utc >= live_boundary_utc
                ORDER BY tombstone.market_id
            LOOP
                EXECUTE
                    'SELECT public.restore_archived_strategy_runs_for_dependency($1, NULL, $2, true, true)'
                USING NEW.id, archived_market_id;
            END LOOP;
        END IF;
...
CREATE TABLE IF NOT EXISTS strategy_skip_archive_market_identities (
    ...
    CONSTRAINT ux_strategy_skip_archive_market_identities_market UNIQUE (market_id)
);
...
CREATE TABLE IF NOT EXISTS strategy_skip_archive_market_metadata_versions (
    ...
    market_identity_id integer NOT NULL
        REFERENCES strategy_skip_archive_market_identities(market_identity_id),
    ...
    market_start_utc timestamptz NULL,
    market_end_utc timestamptz NULL,
    CONSTRAINT ux_strategy_skip_archive_market_metadata_version_identity
        UNIQUE (metadata_version_id, market_identity_id),
    CONSTRAINT ux_strategy_skip_archive_market_metadata_versions_exact
...
CREATE TABLE IF NOT EXISTS strategy_skip_archive_reasons (
    ...
    CONSTRAINT ux_strategy_skip_archive_reasons_text UNIQUE (skip_reason),
    CONSTRAINT ck_strategy_skip_archive_reasons_nonempty CHECK (
        NULLIF(btrim(skip_reason), '') IS NOT NULL)
);
...
CREATE TABLE IF NOT EXISTS strategy_market_paper_skip_tombstones_v2 (
    ...
    skip_reason_id smallint NOT NULL
        REFERENCES strategy_skip_archive_reasons(skip_reason_id),
    run_updated_at_utc timestamptz NOT NULL,
    PRIMARY KEY (strategy_id, market_identity_id),
    CONSTRAINT ux_strategy_market_paper_skip_tombstones_v2_run
        UNIQUE (archived_run_id),
    CONSTRAINT fk_strategy_market_paper_skip_tombstones_v2_metadata
        FOREIGN KEY (metadata_version_id, market_identity_id)
        REFERENCES strategy_skip_archive_market_metadata_versions(
            metadata_version_id, market_identity_id)
);

CREATE INDEX IF NOT EXISTS ix_skip_archive_metadata_condition
ON strategy_skip_archive_market_metadata_versions(
    condition_id, metadata_version_id, market_identity_id);

CREATE INDEX IF NOT EXISTS ix_skip_tombstones_v2_metadata_strategy
ON strategy_market_paper_skip_tombstones_v2(
    metadata_version_id, strategy_id, archived_run_id);

CREATE INDEX IF NOT EXISTS ix_skip_tombstones_v2_rollup_group
ON strategy_market_paper_skip_tombstones_v2(
    strategy_id,
    ((run_updated_at_utc AT TIME ZONE 'UTC')::date),
    skip_reason_id,
    run_updated_at_utc,
    archived_run_id);

CREATE INDEX IF NOT EXISTS ix_skip_tombstones_v2_strategy_recent
ON strategy_market_paper_skip_tombstones_v2(
    strategy_id, run_updated_at_utc DESC, archived_run_id)
INCLUDE (stake_usd, entry_due_at_utc, skip_reason_id);

CREATE INDEX IF NOT EXISTS ix_skip_tombstones_v2_global_recent
ON strategy_market_paper_skip_tombstones_v2(
    run_updated_at_utc DESC, strategy_id, archived_run_id)
INCLUDE (stake_usd, entry_due_at_utc, skip_reason_id);

CREATE OR REPLACE FUNCTION public.prevent_strategy_skip_archive_dimension_mutation()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS trg_prevent_skip_archive_market_identity_mutation
ON public.strategy_skip_archive_market_identities;
CREATE TRIGGER trg_prevent_skip_archive_market_identity_mutation
BEFORE UPDATE OR DELETE ON public.strategy_skip_archive_market_identities
FOR EACH ROW
EXECUTE FUNCTION public.prevent_strategy_skip_archive_dimension_mutation();

DROP TRIGGER IF EXISTS trg_prevent_skip_archive_metadata_mutation
ON public.strategy_skip_archive_market_metadata_versions;
CREATE TRIGGER trg_prevent_skip_archive_metadata_mutation
BEFORE UPDATE OR DELETE ON public.strategy_skip_archive_market_metadata_versions
FOR EACH ROW
EXECUTE FUNCTION public.prevent_strategy_skip_archive_dimension_mutation();

DROP TRIGGER IF EXISTS trg_prevent_skip_archive_reason_mutation
ON public.strategy_skip_archive_reasons;
CREATE TRIGGER trg_prevent_skip_archive_reason_mutation
BEFORE UPDATE OR DELETE ON public.strategy_skip_archive_reasons
FOR EACH ROW
EXECUTE FUNCTION public.prevent_strategy_skip_archive_dimension_mutation();

CREATE OR REPLACE FUNCTION public.prevent_strategy_skip_archive_v2_update()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
    RAISE EXCEPTION 'strategy_market_paper_skip_tombstones_v2 rows are immutable.'
        USING ERRCODE = '55000';
END;
$$;

DROP TRIGGER IF EXISTS trg_prevent_strategy_skip_archive_v2_update
ON public.strategy_market_paper_skip_tombstones_v2;
CREATE TRIGGER trg_prevent_strategy_skip_archive_v2_update
BEFORE UPDATE ON public.strategy_market_paper_skip_tombstones_v2
FOR EACH ROW
EXECUTE FUNCTION public.prevent_strategy_skip_archive_v2_update();

CREATE OR REPLACE FUNCTION public.prevent_strategy_skip_archive_cross_version_collision()
RETURNS trigger
LANGUAGE plpgsql
AS $$
DECLARE
...
BEGIN
    IF TG_TABLE_NAME = 'strategy_market_paper_skip_tombstones' THEN
        IF EXISTS (
            SELECT 1
            FROM public.strategy_market_paper_skip_tombstones_v2 tombstone
            WHERE tombstone.archived_run_id = NEW.archived_run_id
        ) OR EXISTS (
            SELECT 1
            ...
            INNER JOIN public.strategy_market_paper_skip_tombstones_v2 tombstone
                ON tombstone.strategy_id = NEW.strategy_id
               AND tombstone.market_identity_id = market_identity.market_identity_id
            WHERE market_identity.market_id = NEW.market_id COLLATE "C"
        ) THEN
            RAISE EXCEPTION
                'Cannot persist v1 skipped-run archive: logical identity already exists in v2.'
                USING ERRCODE = '23505';
        END IF;
    ELSIF TG_TABLE_NAME = 'strategy_market_paper_skip_tombstones_v2' THEN
        ...
        IF EXISTS (
            ...
            RAISE EXCEPTION
                'Cannot persist v2 skipped-run archive: logical identity already exists in v1.'
                USING ERRCODE = '23505';
        END IF;
    ELSE
        RAISE EXCEPTION 'Unsupported skipped-run archive collision table: %.', TG_TABLE_NAME;
    END IF;
...
DROP TRIGGER IF EXISTS trg_prevent_strategy_skip_archive_v1_v2_collision
ON public.strategy_market_paper_skip_tombstones;
CREATE TRIGGER trg_prevent_strategy_skip_archive_v1_v2_collision
BEFORE INSERT OR UPDATE OF strategy_id, market_id, archived_run_id
ON public.strategy_market_paper_skip_tombstones
FOR EACH ROW
EXECUTE FUNCTION public.prevent_strategy_skip_archive_cross_version_collision();

DROP TRIGGER IF EXISTS trg_prevent_strategy_skip_archive_v2_v1_collision
ON public.strategy_market_paper_skip_tombstones_v2;
CREATE TRIGGER trg_prevent_strategy_skip_archive_v2_v1_collision
BEFORE INSERT OR UPDATE OF strategy_id, market_identity_id, archived_run_id
ON public.strategy_market_paper_skip_tombstones_v2
FOR EACH ROW
EXECUTE FUNCTION public.prevent_strategy_skip_archive_cross_version_collision();
...
SELECT
    ...
    NULL::integer AS metadata_version_id,
    NULL::smallint AS skip_reason_id
FROM public.strategy_market_paper_skip_tombstones tombstone
...
SELECT
    ...
    tombstone.metadata_version_id,
    tombstone.skip_reason_id
FROM public.strategy_market_paper_skip_tombstones_v2 tombstone
...
BEGIN
    IF EXISTS (
        SELECT 1
        FROM public.strategy_market_paper_skip_tombstones tombstone
        WHERE tombstone.archived_run_id = NEW.id
    ) OR EXISTS (
        SELECT 1
        FROM public.strategy_market_paper_skip_tombstones tombstone
        WHERE tombstone.strategy_id = NEW.strategy_id
