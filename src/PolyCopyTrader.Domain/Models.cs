...
public static class StrategyIds
{
    ...
    private static IReadOnlyList<BtcUpDown5mStrategyVariant> CreateCryptoUpDown5mVariants()
    {
        ...
        foreach (var asset in assets)
        {
            ...
            if (string.Equals(asset.Symbol, "ETH", StringComparison.OrdinalIgnoreCase))
            {
                variants.AddRange(CreateEthLossDiffMirrorVariants(variants));
            }
        ...
        int entryDelaySeconds = -30)
    {
        ...
        return new BtcUpDown5mStrategyVariant(
            ...
            $"{secondsBeforeOpen.ToString(CultureInfo.InvariantCulture)} seconds before {normalizedAsset} 5m market open, select the {boundarySelectionName} usable in-memory reference average ({boundaryName} boundary) across every configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m window. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. If the current price is {boundaryRelation} that {boundaryName} boundary by at least {thresholdName} bps, BUY {targetOutcomeName} from current premarket executable ask depth using the worst-price cap. Otherwise skip. Paper entry simulates the same taker BUY, while Live-shadow submits a market BUY amount so available liquidity is taken immediately and any remainder is cancelled.",
            BtcUpDown5mStrategyDirection.Dynamic,
            entryDelaySeconds,
        ...
        int entryDelaySeconds = -30)
    {
        ...
        return new BtcUpDown5mStrategyVariant(
            ...
            $"{secondsBeforeOpen.ToString(CultureInfo.InvariantCulture)} seconds before {normalizedAsset} 5m market open, form an envelope from every usable in-memory 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m reference average. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. If the current price is above the maximum average by at least {thresholdName} bps, BUY Down; if it is below the minimum average by at least {thresholdName} bps, BUY Up. Otherwise skip. Paper entry simulates the same taker BUY, while Live-shadow submits a market BUY amount so available liquidity is taken immediately and any remainder is cancelled.",
            BtcUpDown5mStrategyDirection.Dynamic,
            entryDelaySeconds,
            BtcUpDown5mStrategyBehavior.ReferenceAverageBpsThresholdFakPremarket,
            thresholdBps,
        ...
        BtcUpDown5mStrategyVariant sourceVariant)
    {
        ...
        return sourceVariant with
        {
            ...
            Description =
                $"Paper-only Maker GTD clone of {sourceVariant.Name}. It preserves the source ETH reference-average envelope signal, including Max/Min boundaries selected from every usable configured window whether complete or incomplete; gaps and incomplete coverage alone do not block the signal. Bps use the oldest available real bucket in the 24h window even when that denominator window is incomplete; no other denominator is substituted. Direction, timing, stake sizing, and risk checks are unchanged. Before market open, make at most ten attempts to create one post-only GTD BUY using the maximum-resting formula floor_to_tick(min(bestAsk - tick, cap)), capped at {ReferenceAverageMakerGtdMaximumOrderPrice.ToString("0.00", CultureInfo.InvariantCulture)}; for a tick-aligned venue book this is the highest tick-aligned price strictly below the current best ask. After the first accepted order, stop retrying. The resting order expires one minute before market end. By explicit user approval, this exact family and maximum-resting price policy are a closed ordinary-Paper exception. The optimistic, non-Live-equivalent TouchNoDepth model fills the full order only on later authoritative exact-token evidence: last_trade_price or current best ask at or below the BUY limit; equality counts, and this may overstate fills. Its records intentionally contribute to ordinary Paper orders, PnL, win rate, and performance. Every result must be labeled optimistic TouchNoDepth Paper; not Live-equivalent; may overstate fills. Live submission is disabled, and no alias, clone, descendant, future strategy, different execution source, predicate mismatch, or changed execution semantic inherits this exception.",
        ...
        int entryDelaySeconds = -30)
    {
        ...
        return new BtcUpDown5mStrategyVariant(
            ...
            $"{secondsBeforeOpen.ToString(CultureInfo.InvariantCulture)} seconds before {normalizedAsset} 5m market open, apply the neutral Reference Average envelope signal: above the maximum usable in-memory reference average by at least {thresholdName} bps buys Down, while below the minimum usable average by at least {thresholdName} bps buys Up; otherwise skip. Every configured window with at least one valid sample and a positive calculated average participates whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. Simulate a Paper FAK taker BUY with a maximum order price of {maximumOrderPrice.ToString("0.00", CultureInfo.InvariantCulture)}; fill only immediately executable asks at or below that price and cancel the remainder. Live execution is not supported for this Paper experiment.",
            BtcUpDown5mStrategyDirection.Dynamic,
            entryDelaySeconds,
        ...
        int entryDelaySeconds = -30)
    {
        ...
        {
            BtcUpDownFixedOutcome.Up => "select the largest usable in-memory reference average as the maximum boundary",
            BtcUpDownFixedOutcome.Down => "select the smallest usable in-memory reference average as the minimum boundary",
            _ => "select both the maximum and minimum usable in-memory reference-average boundaries"
        };
        ...
        return new BtcUpDown5mStrategyVariant(
            ...
            $"{secondsBeforeOpen.ToString(CultureInfo.InvariantCulture)} seconds before {normalizedAsset} 5m market open, {boundarySelectionDescription} across every configured 24h, 12h, 6h, 3h, 90m, 45m, 20m, and 10m window. A window participates whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. {signalDescription} Enter only when the direction-relevant selected boundary uses the {requiredWindow} window; otherwise skip. Paper entry simulates the same taker BUY from executable ask depth using the worst-price cap. Live execution is not supported for this optimized Paper experiment.",
            BtcUpDown5mStrategyDirection.Dynamic,
            entryDelaySeconds,
        ...
        int entryDelaySeconds = -30)
    {
        ...
        var description = lowEnter
            ? $"{secondsBeforeOpen.ToString(CultureInfo.InvariantCulture)} seconds before ETH 5m market open, compare the latest Binance ETH/USDT reference price with the usable in-memory 3h reference average only; other configured windows do not participate. The 3h average is eligible whenever it has at least one valid sample and a positive calculated average, whether complete or incomplete; gaps and incomplete coverage alone do not block calculation. Measure bps from the oldest available real bucket in the 24h window; that 24h window may be incomplete, but no other denominator is substituted. If the current price is above that 3h average by at least {thresholdName} bps, BUY Down; if it is below that 3h average by at least {thresholdName} bps, BUY Up. Otherwise skip. Simulate a Paper FAK taker BUY with a maximum order price of {maximumOrderPrice.ToString("0.00", CultureInfo.InvariantCulture)}; fill only immediately executable asks at or below that price and cancel the remainder. Live execution is not supported for this Paper experiment."
    ...
    private static int GetOptimizedReferenceAverageBpsPremarketIdGroup(
        string assetSymbol,
        BtcUpDownFixedOutcome? triggerOutcome)
    {
...
public static class StrategyChildParentAssignmentModes
{
    ...
    public const string LossDiffReset = "LossDiffReset";
    public const string LossDiffPositive = "LossDiffPositive";
}

public sealed record StrategyLossDiffState(
    ...
    int CurrentValue,
    DateTimeOffset StartedAtUtc,
    DateTimeOffset? LastParentEnteredAtUtc,
    Guid? LastParentRunId,
