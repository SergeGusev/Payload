...
public static class EthUp8LossDiffHistoryBackfillCommand
{
    public const string CommandFlag = "--backfill-eth-up8-lossdiff-history";
    public const string ApplyFlag = "--apply";
    public const string ApprovalDigest = "sha256:b83c8ba3ee3b432a551aeeb1ed76d33d30e76ff4765c63831341ad9f12fe7be5";
    public const string AppliedHistoryContractDigest = "sha256:ab25b0935aea29c666905707a6b3487135336142ec93f8c195ad65954e123cb8";
    public const string SourceDigest = "sha256:61847ea1dcd44eebad4e8614492c465d051b3a92326b893400bc2c3487888c53";
    public const string FullSourceDigest = "sha256:72783d933d4c8e404696389bc4fb369c6febdb3331f93fd5c191ca357a7efcdf";
    public const string ExpectedResetParentRunIdDigest =
        "sha256:8b7428c0d5f700e6c7b2f726b9f5fb5a587921948744ce0952f7fad466c07c50";
    public const string ExpectedPositiveParentRunIdDigest =
        "sha256:497e0580dab35d9694f2bcec24479dc37fe691e93683ccc6fdaf18a572c19dc2";
    public const string MarkerKey = "20260828_eth_up8_lossdiff_history_backfill_batched_v1";
    public const string EvidenceVersion = "eth_up8_lossdiff_parent_mirror_history_v1";
    public const string CommandBuild = "eth_up8_lossdiff_history_backfill_batched_v1+83e3f246";
    internal const string TransactionSafetySql =
        "SET LOCAL TIME ZONE 'UTC'; SET LOCAL lock_timeout = '2s'; SET LOCAL statement_timeout = '15s';";
    public const string DeployedServiceVersion =
        "info=1.0.0+7a0b967d9c610166975a57b066a5154ff2499cf9; assembly=1.0.0.0; mvid=50215310ddd6";
    private const long AdvisoryLockKey = 8229202608280001;
    private static readonly TimeSpan BatchDelay = TimeSpan.FromSeconds(1);

    internal static readonly Guid ParentStrategyId = Guid.Parse("b7c50005-0000-4000-8137-000000000108");
    internal static readonly DateTimeOffset CutoffUtc = DateTimeOffset.Parse(
        "2026-08-27T21:18:38.378192Z",
        CultureInfo.InvariantCulture,
        DateTimeStyles.AssumeUniversal);
    ...
    [
        new(
            Guid.Parse("b7c50005-0000-4000-8229-000000000003"),
            ...
            20),
        new(
            Guid.Parse("b7c50005-0000-4000-8229-000000000016"),
        ...
        CancellationToken cancellationToken)
    {
        ...
        var factory = new PostgresConnectionFactory(
            configuration.Storage,
            apply ? "eth_up8_lossdiff_history_backfill_apply" : "eth_up8_lossdiff_history_backfill_preview");
        await using var connection = factory.CreateConnection();
        await connection.OpenAsync(cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        var baseline = BuildOperationalBaseline(initial);
        var committed = checked((int)(initial.Snapshot.ExactTargetChainCount ?? 0));
        foreach (var entry in initial.Plan)
        {
            var beforeBatch = await ReadOperationalSnapshotAsync(
                connection, builder, baseline, cancellationToken);
            if (beforeBatch.Problems.Count != 0)
            {
                await WriteBatchStopAsync(output, committed, initial.Plan.Count, beforeBatch.Problems);
                return 1;
        ...
        var final = await ReadVerifiedSnapshotAsync(connection, builder, cancellationToken);
        AddOperationalBaselineProblems(baseline, final, final.Problems);
        if (final.Snapshot.MarkerDetails is null)
        {
            RequireCompleteProgress(final.Snapshot, final.Plan, final.Problems);
        }
        ...
        var post = await ReadVerifiedSnapshotAsync(connection, builder, cancellationToken);
        AddOperationalBaselineProblems(baseline, post, post.Problems);
        if (post.Problems.Count != 0 || post.Snapshot.MarkerDetails is null)
        {
            throw new InvalidOperationException("Post-marker verification failed: " + string.Join("; ", post.Problems));
        }

        await output.WriteLineAsync("APPLY_OK: exactly 46 Reset and 21 Positive complete Paper chains plus one marker committed in short batches.");
        return 0;
    }
    ...
    private static async Task<OperationalSnapshot> ReadOperationalSnapshotAsync(
        ...
        CancellationToken cancellationToken)
    {
        var current = await ReadVerifiedSnapshotAsync(connection, builder, cancellationToken);
        AddOperationalBaselineProblems(baseline, current, current.Problems);
        return new OperationalSnapshot(current.Snapshot, current.Problems);
    }
    ...
    private static OperationalBaseline BuildOperationalBaseline(VerifiedSnapshot initial) => new(
        ComputeSourceDigest(initial.Source.Select(row => row.CanonicalLine)),
        ComputeSourceDigest(initial.Source.Select(row => row.FullChainCanonicalLine)),
        BuildMembershipDigest(initial.Plan),
        initial.Snapshot.InvariantDigest);

    private static void AddOperationalBaselineProblems(
        OperationalBaseline baseline,
        VerifiedSnapshot current,
        ICollection<string> problems)
    {
        foreach (var problem in CompareOperationalBaseline(
                     baseline.SourceDigest,
                     baseline.FullSourceDigest,
                     baseline.MembershipDigest,
                     baseline.InvariantDigest,
                     ComputeSourceDigest(current.Source.Select(row => row.CanonicalLine)),
                     ComputeSourceDigest(current.Source.Select(row => row.FullChainCanonicalLine)),
                     BuildMembershipDigest(current.Plan),
                     current.Snapshot.InvariantDigest))
        {
            problems.Add(problem);
        }
    }

    internal static IReadOnlyList<string> CompareOperationalBaseline(
        string expectedSourceDigest,
        string expectedFullSourceDigest,
        string expectedMembershipDigest,
        string expectedInvariantDigest,
        string currentSourceDigest,
        string currentFullSourceDigest,
        string currentMembershipDigest,
        string currentInvariantDigest)
    {
        var problems = new List<string>();
        if (!string.Equals(currentSourceDigest, expectedSourceDigest, StringComparison.Ordinal))
            problems.Add($"source digest changed during apply: {currentSourceDigest}");
        if (!string.Equals(currentFullSourceDigest, expectedFullSourceDigest, StringComparison.Ordinal))
            problems.Add($"full source-chain digest changed during apply: {currentFullSourceDigest}");
        if (!string.Equals(currentMembershipDigest, expectedMembershipDigest, StringComparison.Ordinal))
            problems.Add($"causal selected membership/run-ID plan changed during apply: {currentMembershipDigest}");
        if (!string.Equals(currentInvariantDigest, expectedInvariantDigest, StringComparison.Ordinal))
            problems.Add($"protected flags/states/events invariant changed during apply: {currentInvariantDigest}");
        return problems;
    }
...
SELECT
    current_database(),
    NOT pg_is_in_recovery(),
    (SELECT count(*) FROM public.schema_migration_history WHERE migration_id = '0003-eth-up8-lossdiff-gated-children'
        AND semantic_checksum = '5e4effb4845ee9896f64f5ebbbed41eef13ecb2d6bc25b61caf0b0e417b6d267'),
    (SELECT count(*) FROM public.strategies s WHERE
        s.id = 'b7c50005-0000-4000-8137-000000000108'
        AND s.code = 'eth_up_down_5m_up_bps_8_fak_premarket'
        AND s.name = 'ETH Up or Down 5m Up 8 bps Reference Average Premarket'
        AND s.enabled AND NOT s.live_stakes AND NOT s.paused AND NOT s.auto_live_paused),
    (SELECT count(*) FROM public.strategies s WHERE
        (s.id = 'b7c50005-0000-4000-8229-000000000003' AND s.code = 'eth_up_down_5m_up_bps_8_fak_premarket_lossdiff_3_plus' AND s.name = 'ETH 5m Up 8 bps Reference Average Premarket LossDiff 3+' OR
         s.id = 'b7c50005-0000-4000-8229-000000000016' AND s.code = 'eth_up_down_5m_up_bps_8_fak_premarket_lossdiff_16_plus_positive' AND s.name = 'ETH 5m Up 8 bps Reference Average Premarket LossDiff 16+ Positive')
        AND s.enabled AND NOT s.live_stakes AND NOT s.paused AND NOT s.auto_live_paused),
    (SELECT count(*) FROM public.strategies s WHERE
        s.id IN ('b7c50005-0000-4000-8137-000000000108', 'b7c50005-0000-4000-8229-000000000003', 'b7c50005-0000-4000-8229-000000000016')
        OR s.code IN ('eth_up_down_5m_up_bps_8_fak_premarket', 'eth_up_down_5m_up_bps_8_fak_premarket_lossdiff_3_plus', 'eth_up_down_5m_up_bps_8_fak_premarket_lossdiff_16_plus_positive')
        OR s.name IN ('ETH Up or Down 5m Up 8 bps Reference Average Premarket', 'ETH 5m Up 8 bps Reference Average Premarket LossDiff 3+', 'ETH 5m Up 8 bps Reference Average Premarket LossDiff 16+ Positive')),
    (SELECT count(*) FROM public.strategy_loss_diff_states state WHERE
        (state.child_strategy_id = 'b7c50005-0000-4000-8229-000000000003' AND state.mode = 'LossDiffReset' AND state.threshold = 3 OR
         state.child_strategy_id = 'b7c50005-0000-4000-8229-000000000016' AND state.mode = 'LossDiffPositive' AND state.threshold = 16)
        AND state.parent_strategy_id = 'b7c50005-0000-4000-8137-000000000108'
        AND state.started_at_utc = @CutoffUtc),
    (SELECT count(*) FROM public.strategy_child_parent_assignments assignment WHERE
        (assignment.id = 'b7c50005-0000-4000-8230-000000000003' AND assignment.child_strategy_id = 'b7c50005-0000-4000-8229-000000000003' AND assignment.child_mode = 'LossDiffReset'
         OR assignment.id = 'b7c50005-0000-4000-8230-000000000016' AND assignment.child_strategy_id = 'b7c50005-0000-4000-8229-000000000016' AND assignment.child_mode = 'LossDiffPositive')
        AND assignment.parent_strategy_id = 'b7c50005-0000-4000-8137-000000000108'
        AND assignment.asset_symbol = 'ETH' AND assignment.lookback_hours = 0
        AND assignment.parent_pnl_usd = 0 AND assignment.parent_roi_pct = 0
        AND assignment.assigned_at_utc = @CutoffUtc AND assignment.ended_at_utc IS NULL),
    (SELECT count(*) FROM public.strategy_market_paper_runs r
        WHERE r.strategy_id = 'b7c50005-0000-4000-8137-000000000108'
          AND r.entered_at_utc < @CutoffUtc AND r.status = 'Settled'
          AND r.realized_pnl_usd IS NOT NULL AND r.realized_pnl_usd <> 0),
    (SELECT count(*) FROM public.strategy_market_paper_runs WHERE strategy_id = ANY(@ChildIds) AND entered_at_utc < @CutoffUtc),
    (SELECT details FROM public.schema_data_migrations WHERE migration_key = @MarkerKey),
...
""";
        ...
        IReadOnlyList<PlannedEntry> plan)
    {
        ...
        problems.AddRange(ValidateSourceFacts(
            ...
            ComputeSourceDigest(source.Select(row => row.FullChainCanonicalLine)),
            CutoffUtc));
        Require(snapshot.BaseSourceRows == 1022, $"base_source_rows={snapshot.BaseSourceRows}");
        Require(snapshot.HealthyHeartbeatCount == 1, $"exact_healthy_heartbeat_count={snapshot.HealthyHeartbeatCount}");
        Require(snapshot.WaitingLockCount == 0, $"waiting_locks={snapshot.WaitingLockCount}");
        Require(snapshot.ImportedPreCutoffEventCount == 0, $"imported_pre_cutoff_events={snapshot.ImportedPreCutoffEventCount}");
        ...
        Require(
            string.Equals(BuildParentRunIdDigest(plan, Children[0].Id), ExpectedResetParentRunIdDigest,
                StringComparison.Ordinal),
            $"{Children[0].Code} exact parent run-ID membership drift");
        Require(
            string.Equals(BuildParentRunIdDigest(plan, Children[1].Id), ExpectedPositiveParentRunIdDigest,
                StringComparison.Ordinal),
            $"{Children[1].Code} exact parent run-ID membership drift");
    ...
    {
        var problems = new List<string>();
        if (count != 1022) problems.Add($"source_count={count}");
        if (wins != 574) problems.Add($"source_wins={wins}");
        if (losses != 448) problems.Add($"source_losses={losses}");
        if (!string.Equals(sourceDigest, SourceDigest, StringComparison.Ordinal))
            problems.Add("source_digest drift");
        if (!string.Equals(fullSourceDigest, FullSourceDigest, StringComparison.Ordinal))
            problems.Add("full_source_digest drift");
...
WITH parent AS (
    ...
    SELECT
        @SignalId, s.leader_trade_id, @ChildWallet, s.condition_id, s.asset_id, s.outcome, s.leader_price,
        s.best_bid, s.best_ask, s.spread_abs, s.spread_pct, s.lag_seconds, s.score, true,
        'accepted_eth_up8_lossdiff_history_backfill', s.proposed_paper_price, s.proposed_size_shares,
        s.proposed_notional_usd, s.created_at_utc,
        COALESCE(s.raw_context_json, '{}'::jsonb) || jsonb_build_object(
            'version', @EvidenceVersion, 'child_mode', @ChildMode, 'threshold', @Threshold,
            'pre_entry_loss_diff', @PreEntryValue, 'parent_run_id', p.id, 'parent_signal_id', p.parent_signal_id,
...
WHERE r.status = 'Settled' AND r.retention_scope = 'PaperOnly'
  ...
  AND o.copied_trader_wallet = 'strategy:' || e.child_code
  AND s.trader_wallet = 'strategy:' || e.child_code AND s.accepted
  AND s.decision = 'accepted_eth_up8_lossdiff_history_backfill'
  AND s.raw_context_json = COALESCE(parent_signal.raw_context_json, '{}'::jsonb) || e.audit
  AND o.raw_decision_json = COALESCE(parent_order.raw_decision_json, '{}'::jsonb) || e.audit
  AND f.evidence::jsonb = jsonb_build_object(
      'version', @EvidenceVersion, 'parent_evidence', parent_fill.evidence,
...
""";
    ...
    internal static string BuildMembershipDigest(IReadOnlyList<PlannedEntry> plan) =>
        ComputeSourceDigest(plan
            .OrderBy(entry => entry.Child.Id)
            .ThenBy(entry => entry.Source.EnteredMicros)
            .ThenBy(entry => entry.Source.RunId)
    ...
    internal static string BuildParentRunIdDigest(IReadOnlyList<PlannedEntry> plan, Guid childId) =>
        ComputeSourceDigest(plan
            .Where(entry => entry.Child.Id == childId)
            .OrderBy(entry => entry.Source.EnteredMicros)
            .ThenBy(entry => entry.Source.RunId)
            .Select(entry => entry.Source.RunId.ToString("D")));
    ...
    internal static string BuildMarkerDetails(IReadOnlyList<PlannedEntry> plan) => JsonSerializer.Serialize(new
    {
        ...
        full_source_chain_digest = FullSourceDigest,
        selected_membership_digest = BuildMembershipDigest(plan),
        contract_digest = AppliedHistoryContractDigest,
        command_build = CommandBuild,
        batch_size = 1,
        completed_chain_count = plan.Count,
        reset = Children[0].ExpectedMetrics,
    ...
    private sealed record OperationalBaseline(
        string SourceDigest,
        string FullSourceDigest,
