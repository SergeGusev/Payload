    ...
    IPolymarketFeeAccountingService? feeAccountingService = null) : IBtcUpDown5mPaperStrategyProcessor
{
    ...
    private static readonly IReadOnlyList<string> ObservationMarketAssetSymbols = CryptoReferenceAssetSymbols
        .Append("BTC")
    ...
    private readonly SemaphoreSlim observationMarketSnapshotRefreshLock = new(1, 1);
    ...
    private static bool UsesFastDiffReservedEntryDecisionSlots(string cycleKind)
    {
        return string.Equals(cycleKind, "fast_diff_due", StringComparison.Ordinal);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var snapshotMarkets = await GetObservationMarketSnapshotAsync(nowUtc, cancellationToken);
        var btcVariants = variants
            .Where(IsBtcReferenceVariant)
            .ToArray();
        if (btcVariants.Length > 0)
        {
            var btcMarkets = snapshotMarkets
        ...
        if (cryptoVariants.Length > 0)
        {
            var assetSymbols = cryptoVariants
                ...
                .Distinct(StringComparer.OrdinalIgnoreCase)
                .ToArray();
            var allowedAssetSymbols = assetSymbols.ToHashSet(StringComparer.OrdinalIgnoreCase);
            var cryptoMarkets = snapshotMarkets
                .Where(market => CryptoUpDown5mMarketAnalyzer.TryGetAssetSymbol(
                    market,
                    allowedAssetSymbols,
                    out _))
                .ToArray();
    ...
    private async Task<IReadOnlyList<PolymarketGammaMarket>> GetObservationMarketSnapshotAsync(
        DateTimeOffset nowUtc,
        CancellationToken cancellationToken)
    {
        var snapshot = Volatile.Read(ref observationMarketSnapshot);
        if (IsObservationMarketSnapshotFresh(snapshot, nowUtc))
        {
            return snapshot.Markets;
        }

        await observationMarketSnapshotRefreshLock.WaitAsync(cancellationToken);
        try
        {
            nowUtc = GetUtcNow();
            snapshot = Volatile.Read(ref observationMarketSnapshot);
            if (IsObservationMarketSnapshotFresh(snapshot, nowUtc))
            {
                return snapshot.Markets;
            }

            var combinedLimit = (int)Math.Min(
                int.MaxValue,
                Math.Max(1L, (long)options.MaxMarketsPerCycle * 2L));
            var markets = await repository.GetUpDownStrategyGammaMarketsForObservationAsync(
                ObservationMarketAssetSymbols,
                nowUtc.Subtract(MarketObserveBehindWindow),
                nowUtc.Add(MarketObserveAheadWindow),
                combinedLimit,
                cancellationToken);
            snapshot = new ObservationMarketSnapshot(GetUtcNow(), markets);
            Volatile.Write(ref observationMarketSnapshot, snapshot);
            return snapshot.Markets;
        }
        finally
        {
            observationMarketSnapshotRefreshLock.Release();
        }
    }

    private bool IsObservationMarketSnapshotFresh(
        ObservationMarketSnapshot snapshot,
        DateTimeOffset nowUtc)
    {
        if (snapshot.LoadedAtUtc == DateTimeOffset.MinValue)
        {
            return false;
        }

        var age = nowUtc - snapshot.LoadedAtUtc;
        return age >= TimeSpan.Zero &&
            age < TimeSpan.FromMilliseconds(options.ObservationMarketSnapshotTtlMilliseconds);
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var markets = (await GetObservationMarketSnapshotAsync(nowUtc, cancellationToken))
            .Where(BtcUpDown5mMarketAnalyzer.IsStrategyCandidate)
            .ToArray();
        var observed = 0;
        var entriesPlaced = 0;
        ...
        CancellationToken cancellationToken)
    {
        ...
        Guid? correlationId = isPaperLiveShadowTest ? Guid.NewGuid() : null;
        ...
        FakBuyExecutionIntent? shadowFakExecutionIntent = null;
        ...
        if (isPaperLiveShadowTest)
        {
            ...
            shadowFakExecutionIntent = FakBuyExecutionIntent.Create(
                variant.Id,
                correlationId!.Value,
                market.ConditionId,
                selectedOutcome.AssetId,
                ...
                shadowFakSizing.TargetSizeShares,
                shadowFakOrderBook,
                nowUtc);
            shadowFakSizing = shadowFakSizing with
            {
                TargetNotionalUsd = shadowFakExecutionIntent.TargetNotionalUsd,
                TargetSizeShares = shadowFakExecutionIntent.TargetSizeShares
            };
            shadowFakRawDecisionJson = AttachOpeningLimitStakeSizingJson(
                rawDecisionJson,
                paperLiveShadowStakeUsd,
                shadowFakSizing,
                expiration);
            shadowFakEstimate = EstimatePaperFakFill(shadowFakExecutionIntent, shadowFakOrderBook);
            rawDecisionJson = AttachFakPaperFillSimulationJson(
                ...
                nowUtc,
                executionIntent: shadowFakExecutionIntent);
        ...
        {
            if (isPaperLiveShadowTest &&
                correlationId is { } createdShadowCorrelationId &&
                shadowSnapshot?.OrderBook is { } shadowOrderBook)
            {
        ...
        SemaphoreSlim? dueEntryLock = null)
    {
        ...
        var tasks = runs.Select(async run =>
        {
            ...
            using var decisionCapacityLease = await entryDecisionConcurrencyGate.EnterAsync(
                priority: false,
                cancellationToken);
        ...
        CancellationToken cancellationToken)
    {
        ...
        remainingRuns = OrderDueEntryRunsForPlacement(
            ...
            variantsById,
            strategySettings);
        var usesFastDiffReservedSlots = UsesFastDiffReservedEntryDecisionSlots(cycleKind);
        var effectiveConcurrency = Math.Min(
            remainingRuns.Count,
            usesFastDiffReservedSlots
                ? entryDecisionConcurrencyGate.MaxConcurrency
                : entryDecisionConcurrencyGate.NonPriorityCapacity);
        ...
        await TrackStrategyStageAsync(
            ...
            cancellationToken);
        _ = await TrackStrategyStageAsync(
            cycleId,
            cycleKind,
            flowName,
            stageName + ".previous_result_signal_warmup",
            detail: null,
            variantsById.Count,
            remainingRuns.Count,
            GetEarliestEntryDueAtUtc(remainingRuns),
            GetLatestEntryDueAtUtc(remainingRuns),
            async token => await WarmUpPreviousResultSignalsAsync(
                remainingRuns,
                variantsById,
                marketLookupTasks,
        ...
        var tasks = remainingRuns.Select(async run =>
        {
            ...
            IDisposable decisionCapacityLease;
            var concurrencyWaitStarted = Stopwatch.GetTimestamp();
            try
            {
                decisionCapacityLease = await entryDecisionConcurrencyGate.EnterAsync(
                    usesFastDiffReservedSlots,
                    cancellationToken);
            }
            finally
        ...
        try
        {
            results = await TrackStrategyStageAsync(
                ...
                $"EffectiveConcurrency={effectiveConcurrency.ToString(CultureInfo.InvariantCulture)};" +
                $"TotalConcurrency={entryDecisionConcurrencyGate.MaxConcurrency.ToString(CultureInfo.InvariantCulture)};" +
                $"FastDiffReservedSlots={entryDecisionConcurrencyGate.ReservedPrioritySlots.ToString(CultureInfo.InvariantCulture)};" +
                $"Priority={usesFastDiffReservedSlots.ToString(CultureInfo.InvariantCulture)}",
                variantsById.Count,
                remainingRuns.Count,
                GetEarliestEntryDueAtUtc(remainingRuns),
                GetLatestEntryDueAtUtc(remainingRuns),
    ...
    private async Task<int> WarmUpPreviousResultSignalsAsync(
        IReadOnlyList<StrategyMarketPaperRun> runs,
        IReadOnlyDictionary<Guid, BtcUpDown5mStrategyVariant> variantsById,
        System.Collections.Concurrent.ConcurrentDictionary<string, Lazy<Task<PolymarketGammaMarket?>>> marketLookupTasks,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var warmupTasks = new List<Task<BtcPreviousMarketMoveSignal>>();
        var nowUtc = GetUtcNow();
        foreach (var run in runs)
        {
            if (!variantsById.TryGetValue(StrategyIds.Normalize(run.StrategyId), out var variant) ||
            ...
            var market = await GetPolymarketGammaMarketForEntryAsync(
                ...
                run.MarketId,
                cancellationToken);
            var marketStartUtc = market is null ? null : GetMarketWindowStartUtc(market, variant);
            if (market is null || marketStartUtc is null)
            {
                continue;
            }

            var cacheKey = GetSkipPreviousResultBpsStreakMoveSignalCacheKey(variant, marketStartUtc.Value);
            if (!cacheKeys.Add(cacheKey))
            {
                continue;
            }

            warmupTasks.Add(GetCachedSkipPreviousResultBpsStreakMoveSignalAsync(
                signalTasks,
                market,
                variant,
        ...
        foreach (var warmupTask in warmupTasks)
        {
            ...
            {
                // The ordinary run path re-awaits the cached task and preserves its existing error handling.
            }
        }

        return warmupTasks.Count;
    }
                ...
                try
                {
                    ...
                    if (UsesOpeningLimitEntry(variant))
                    {
                        ...
                        var orderPrice = IsFakOrderEntry(variant)
                            ? ResolveFakMaximumOrderPrice(variant, limitPricing.OrderBookLookup?.OrderBook)
                            : isPaperLiveShadowTest
                        ...
                        if (usePaperFakFillModel)
                        {
                            ...
                            var fakExecutionIntent = FakBuyExecutionIntent.Create(
                                variant.Id,
                                run.Id,
                                market.ConditionId,
                                limitSelectedOutcome.AssetId,
                                orderPrice,
                                paperFakSizing.TargetNotionalUsd,
                                paperFakSizing.TargetSizeShares,
                                fakOrderBook,
                                nowUtc);
                            paperFakSizing = paperFakSizing with
                            {
                                TargetNotionalUsd = fakExecutionIntent.TargetNotionalUsd,
                                TargetSizeShares = fakExecutionIntent.TargetSizeShares
                            };
                            var paperFakStakeUsd = fakExecutionIntent.TargetNotionalUsd;
                            paperFakRawDecisionJson = AttachOpeningLimitStakeSizingJson(
                                limitPricing.RawDecisionJson,
                                stakeMultiplier,
                                paperFakSizing,
                                expiration,
                                paperLostCounterAdjustment);
                            var fakEstimate = EstimatePaperFakFill(fakExecutionIntent, fakOrderBook);
                            ...
                            try
                            {
                                ...
                                if (FakExecutionRules.IsPartialNotionalFill(
                                    fakEstimate.NotionalUsd,
                                    paperFakStakeUsd))
                                {
                                    fakOrder = fakOrder with
                                    {
                                        Status = PaperOrderStatus.PartiallyFilledExpired,
                                        FilledAtUtc = null,
                        ...
                        try
                        {
                            ...
                            {
                                var createdShadowCorrelationId = correlationId ??
                                    throw new InvalidOperationException("Paper live-shadow correlation id was not created.");
                                var quoteAgeMs = (int)Math.Round(GetSnapshotAge(shadowOrderBook.SnapshotAtUtc).TotalMilliseconds);
                                var shadowDecision = new PaperLiveShadowDecision(
                                    createdShadowCorrelationId,
                                    variant.Id,
        ...
        CancellationToken cancellationToken)
    {
        ...
        var worstPrice = ResolveFakMaximumOrderPrice(variant, orderBook);
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (maximumAverage is null || minimumAverage is null)
        {
            const string reason = "reference_average_boundaries_missing";
            return BtcOpeningLimitDecision.Reject(
                reason,
                BuildReferenceAverageBpsRawDecisionJson(
                    market,
        ...
        CryptoReferencePriceAverage? selectedAverage = configuredTriggerDirection switch
        {
            BtcPriceDirection.Up => maximumAverage,
            BtcPriceDirection.Down => minimumAverage,
            _ => null
        };
        ...
        var triggerDirection = configuredTriggerDirection ??
            ...
            {
                (> 0m, _) => BtcPriceDirection.Up,
                (_, < 0m) => BtcPriceDirection.Down,
                _ => null
            });
        selectedAverage = triggerDirection switch
        {
            BtcPriceDirection.Up => maximumAverage,
            BtcPriceDirection.Down => minimumAverage,
            _ => null
        };
        var moveFromAverageBps = triggerDirection switch
        {
            BtcPriceDirection.Up => moveAboveMaximumBps,
            BtcPriceDirection.Down => moveBelowMinimumBps,
            _ => (decimal?)null
        };
        if (triggerDirection is null || selectedAverage is null || moveFromAverageBps is null)
        {
        ...
        var thresholdMet = triggerDirection.Value == BtcPriceDirection.Up
            ? evaluatedMoveFromAverageBps >= thresholdBps
            : evaluatedMoveFromAverageBps <= -thresholdBps;
        if (!thresholdMet)
        {
        ...
        if (IsFilteredReferenceAverageBpsFakPremarketEntry(variant) &&
            IsFilteredReferenceAverageAbsMoveSkipped(Math.Abs(evaluatedMoveFromAverageBps)))
        {
    ...
    private Task<BtcPreviousMarketMoveSignal> GetCachedSkipPreviousResultBpsStreakMoveSignalAsync(
        ...
        CancellationToken cancellationToken)
    {
        var cacheKey = GetSkipPreviousResultBpsStreakMoveSignalCacheKey(variant, marketStartUtc);
        var lazy = signalTasks.GetOrAdd(
            cacheKey,
            _ => new Lazy<Task<BtcPreviousMarketMoveSignal>>(
    ...
    private static string GetSkipPreviousResultBpsStreakMoveSignalCacheKey(
        BtcUpDown5mStrategyVariant variant,
        DateTimeOffset marketStartUtc)
    {
        return string.Concat(
        ...
        CancellationToken cancellationToken)
    {
        ...
        var expectedMarketStartsToResolve = GetExpectedMarketStartsThroughKnownOutcomeChange(
            expectedMarketStarts,
            ledgerResultsByStart);
        var missingMarketStarts = expectedMarketStartsToResolve
            .Where(marketStart => !ledgerResultsByStart.ContainsKey(marketStart.ToUnixTimeSeconds()))
            .ToArray();
        IReadOnlyList<PolymarketGammaMarket> markets = [];
        if (missingMarketStarts.Length > 0)
        {
            var marketLimit = Math.Max(options.MaxMarketsPerCycle, missingMarketStarts.Length * 4);
            if (variant.MarketInterval == BtcUpDownMarketInterval.FiveMinutes)
            {
                var intervalDuration = BtcUpDown5mMarketAnalyzer.GetIntervalDuration(variant.MarketInterval);
                markets = await repository.GetCryptoUpDown5mGammaMarketsEndingBetweenAsync(
                    [referenceAssetSymbol],
                    missingMarketStarts.Min().Add(intervalDuration),
                    missingMarketStarts.Max().Add(intervalDuration),
                    marketLimit,
                    cancellationToken);
            }
            else
            {
                markets = IsBtcReferenceVariant(variant)
        ...
        var selected = new List<BtcSkipMarketResult>(expectedMarketStartsToResolve.Count);
        var diagnostics = new List<BtcSkipCloseBookDiagnostic>();
        foreach (var expectedMarketStart in expectedMarketStartsToResolve)
        {
    ...
    private static IReadOnlyList<DateTimeOffset> GetExpectedMarketStartsThroughKnownOutcomeChange(
        IReadOnlyList<DateTimeOffset> expectedMarketStarts,
        IReadOnlyDictionary<long, BtcSkipMarketResult> ledgerResultsByStart)
    {
        if (expectedMarketStarts.Count <= 1 ||
            !ledgerResultsByStart.TryGetValue(
                expectedMarketStarts[0].ToUnixTimeSeconds(),
                out var firstResult))
        {
            return expectedMarketStarts;
        }

        for (var index = 1; index < expectedMarketStarts.Count; index++)
        {
            if (ledgerResultsByStart.TryGetValue(
                    expectedMarketStarts[index].ToUnixTimeSeconds(),
                    out var result) &&
                !string.Equals(
                    result.WinningOutcome,
                    firstResult.WinningOutcome,
                    StringComparison.OrdinalIgnoreCase))
            {
                return expectedMarketStarts.Take(index + 1).ToArray();
            }
        }
        ...
        IReadOnlyList<CryptoReferencePriceAverage> averages)
    {
        ...
        var minimum = validAverages
            .OrderBy(average => average.AveragePriceUsd.GetValueOrDefault())
            .ThenByDescending(average => average.WindowSeconds)
            .ThenBy(average => average.WindowLabel, StringComparer.OrdinalIgnoreCase)
            .First();
    ...
    private TakerBuyFillEstimate EstimatePaperFakFill(
        ...
        OrderBookSnapshot orderBook)
    {
        return FakBuyExecutionParity.SimulatePaper(
            intent,
            orderBook,
            options.PaperTakerMaxSpreadAbs);
    }
        ...
        decimal targetSizeShares)
    {
        ...
        foreach (var level in orderBook.Asks
            ...
            .OrderBy(level => level.Price))
        {
            if (targetSizeShares > 0m && shares >= targetSizeShares)
            {
                break;
            }
    ...
    private static string AttachFakPaperFillSimulationJson(
        ...
        string? rejectionReason,
        DateTimeOffset nowUtc,
        decimal? maximumOrderPrice = null,
        FakBuyExecutionIntent? executionIntent = null)
    {
        JsonObject root;
        try
        {
        ...
        root["paper_fak_maximum_order_price"] = maximumOrderPrice;
        root["paper_fak_order_price_cap_applied"] = maximumOrderPrice is not null;
        ...
        {
            root["execution_intent_strategy_id"] = executionIntent.StrategyId.ToString();
            root["execution_intent_decision_id"] = executionIntent.DecisionId.ToString();
            root["execution_intent_condition_id"] = executionIntent.ConditionId;
            root["execution_intent_asset_id"] = executionIntent.AssetId;
            root["execution_intent_side"] = executionIntent.Side.ToString();
            root["execution_intent_order_type"] = FakBuyExecutionIntent.TimeInForce;
            root["execution_intent_time_in_force"] = FakBuyExecutionIntent.TimeInForce;
            root["execution_intent_post_only"] = executionIntent.PostOnly;
            root["execution_intent_maximum_order_price"] = executionIntent.MaximumOrderPrice;
            root["execution_intent_requested_notional_usd"] = executionIntent.RequestedNotionalUsd;
            root["execution_intent_requested_size_shares"] = executionIntent.RequestedSizeShares;
            root["execution_intent_target_notional_usd"] = executionIntent.TargetNotionalUsd;
            root["execution_intent_target_size_shares"] = executionIntent.TargetSizeShares;
            root["execution_intent_tick_size"] = executionIntent.TickSize;
            root["execution_intent_min_order_size"] = executionIntent.MinOrderSize;
            root["execution_intent_negative_risk"] = executionIntent.NegativeRisk;
            root["execution_intent_created_at_utc"] = executionIntent.CreatedAtUtc.ToString("O", CultureInfo.InvariantCulture);

            if (lookup?.OrderBook is { } intentOrderBook)
            {
                root["execution_intent_order_book_snapshot"] = JsonSerializer.SerializeToNode(new
                {
                    source = lookup.Source,
                    age_ms = lookup.Age?.TotalMilliseconds,
                    asset_id = intentOrderBook.AssetId,
                    condition_id = intentOrderBook.ConditionId,
                    snapshot_at_utc = intentOrderBook.SnapshotAtUtc,
                    min_order_size = intentOrderBook.MinOrderSize,
                    tick_size = intentOrderBook.TickSize,
                    negative_risk = intentOrderBook.NegativeRisk,
                    last_trade_price = intentOrderBook.LastTradePrice,
                    bids = intentOrderBook.Bids.Select(level => new { price = level.Price, size = level.Size }).ToArray(),
                    asks = intentOrderBook.Asks.Select(level => new { price = level.Price, size = level.Size }).ToArray()
                });
            }
        }
        ...
        CryptoReferencePriceAverage? bpsDenominatorAverage = null)
    {
        ...
        {
            BtcPriceDirection.Up => "maximum",
            BtcPriceDirection.Down => "minimum",
            _ => null
        };
        return JsonSerializer.Serialize(new
        {
            ...
            reference_average_selected_boundary = selectedBoundary,
            reference_average_maximum_window = maximumAverage?.WindowLabel,
            reference_average_maximum_window_seconds = maximumAverage?.WindowSeconds,
            reference_average_maximum_price_usd = maximumAveragePriceUsd,
            ...
            reference_average_move_from_selected_boundary_bps = moveFromAverageBps,
            reference_average_move_above_maximum_bps = moveAboveMaximumBps,
            reference_average_move_below_minimum_bps = moveBelowMinimumBps,
            ...
            fak_maximum_order_price = variant.FakMaximumOrderPrice,
    ...
    private static void AddFakExecutionIntentValidation(
        ICollection<string> validation,
        FakBuyExecutionIntent executionIntent,
        Signal signal,
        BtcUpDown5mOutcomeQuote outcome,
        BtcUpDown5mStrategyVariant variant,
        ...
        Guid correlationId)
    {
        var sharedValidation = FakBuyExecutionParity.Validate(executionIntent);
        foreach (var error in sharedValidation.Errors)
        {
            validation.Add("FAK execution intent is not Live-valid: " + error);
        }

        if (executionIntent.StrategyId != variant.Id || paperOrder.StrategyId != executionIntent.StrategyId)
        {
            validation.Add("Paper and Live FAK strategy identifiers do not match the shared execution intent.");
        }

        if (executionIntent.DecisionId != correlationId || paperOrder.CorrelationId != executionIntent.DecisionId)
        {
            validation.Add("Paper and Live FAK decision identifiers do not match the shared execution intent.");
        }

        if (paperOrder.SignalId != signal.Id)
        {
            validation.Add("Paper FAK signal identifier does not match the Live signal.");
        }

        if (!string.Equals(executionIntent.ConditionId, signal.LeaderTrade.ConditionId, StringComparison.Ordinal) ||
            !string.Equals(paperOrder.ConditionId, executionIntent.ConditionId, StringComparison.Ordinal))
        {
            validation.Add("Paper and Live FAK condition identifiers do not match the shared execution intent.");
        }

        if (!string.Equals(executionIntent.AssetId, outcome.AssetId, StringComparison.Ordinal) ||
            !string.Equals(paperOrder.AssetId, executionIntent.AssetId, StringComparison.Ordinal))
        {
            validation.Add("Paper and Live FAK asset identifiers do not match the shared execution intent.");
        }

        if (executionIntent.Side != TradeSide.Buy || paperOrder.Side != executionIntent.Side)
        {
            validation.Add("Paper and Live FAK sides do not match the shared execution intent.");
        }

        if (paperOrder.Price != executionIntent.MaximumOrderPrice)
        {
            validation.Add("Paper FAK maximum order price does not match the shared Live execution intent.");
        }

        if (paperOrder.NotionalUsd != executionIntent.TargetNotionalUsd)
        {
            validation.Add("Paper FAK target notional does not match the shared Live execution intent.");
        }

        if (paperOrder.SizeShares != executionIntent.TargetSizeShares)
        {
            validation.Add("Paper FAK target size does not match the shared Live execution intent.");
        }
    }
        ...
        CancellationToken cancellationToken)
    {
        ...
        var price = executionIntent.MaximumOrderPrice;
        var sizeShares = executionIntent.TargetSizeShares;
        var liveNotional = executionIntent.TargetNotionalUsd;
        var postOnly = executionIntent.PostOnly;
        var validation = new List<string>();
        AddFakExecutionIntentValidation(
            validation,
            executionIntent,
            signal,
            outcome,
        ...
        {
            validation.Add("Live preflight clock check failed: " + ex.Message);
        }
        ...
        var liveOrderIntent = CreatePaperLiveShadowLiveOrder(
            signal,
            outcome,
            variant,
            price,
        ...
        try
        {
            await repository.AddLiveOrderAsync(liveOrderIntent, cancellationToken);
            exposureCache.ApplyLiveOrder(liveOrderIntent);
            await repository.UpdatePaperLiveShadowDecisionLinksAsync(
                correlationId,
                signal.Id,
                paperOrder.Id,
        ...
        var request = CreateFakMarketBuyRequest(executionIntent);
        ...
        {
            var errorOrder = liveOrderIntent with
            {
        ...
        var updatedLiveOrder = liveOrderIntent with
        {
            Status = status,
    ...
    private ClobV2OrderRequest CreateFakMarketBuyRequest(FakBuyExecutionIntent intent)
    {
        return FakBuyExecutionParity.CreateLiveRequest(
            intent,
    ...
    private static decimal ResolveFakMaximumOrderPrice(
        BtcUpDown5mStrategyVariant variant,
        OrderBookSnapshot? orderBook = null)
    {
        return variant.FakMaximumOrderPrice ?? ResolveFakGuaranteedWorstPrice(orderBook);
    }
    ...
    private sealed record ObservationMarketSnapshot(
        DateTimeOffset LoadedAtUtc,
        IReadOnlyList<PolymarketGammaMarket> Markets)
    {
        public static ObservationMarketSnapshot Empty { get; } = new(
